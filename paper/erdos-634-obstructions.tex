...
for regions assembled from cells of a \emph{fixed} regular lattice, and no such lattice is available
here. (An earlier version of this item asserted that the vertices ``lie in no lattice''. That is
\emph{false} as stated and is corrected here: a finite tiling uses only finitely many directions
$k\gamma$, say $k$ in a window $W$ of width $w$, and its vertices lie in the rank-two lattice
$\Lambda_W=\sum_{k\in W}\mathbb Z\zeta^k$, of covolume $\sqrt{D}/(2f^{\,w-1})$ --- verified exactly on
six certificates. What is dense is the $\mathbb Z$-module generated by \emph{all} directions, or
equivalently the projection of $\Lambda_W$ to a line, since $\alpha/\pi$ is irrational. The correct
obstruction is that $\Lambda_W$ \emph{depends on the tiling} and is not fixed a priori, so there is
no ambient cell structure for the method to run on.) Second, tilings here need not be
...
\begin{lemma}[Endpoint angles]\label{lem:endpoints}\lab{VERIFIED}
...
refutation is the same at every $f$. At $f=12,14,16,18,20,22$ the word $(6,5)$ gives $506$ nodes with
depth $13$ in every case, and $(5,4)$, $(7,6)$, $(8,7)$, $(9,8)$, $(10,9)$ give $299$, $807$, $1315$,
$2124$, $3441$ likewise.

The agreement is stronger than the counts. Reading the engine's node trace at $f=12$ and $f=14$ for
$(6,5)$, the two refutations are the \emph{same tree}: the $506$ nodes carry identical paths in
identical depth-first order, not merely identical totals. The placements differ only by the
$f$-instantiation of the coordinates --- the first apex sits at $d_{\mathrm{BG}}=431/24$ at $f=12$
and $587/28$ at $f=14$, both $(3f^2-1)/2f$, at heights with numerator $f^2-1$ over $2f$ and radicand
$4f^2-1$.

By Proposition~\ref{prop:testsuniform} no test the search performs can tell the members apart, which
is why. What remains measured rather than proved is that the search performs only those tests.
\end{proposition}
...
\emph{(i) The march's steps land on vertices of the above kind.} On a side of the target this is
now proved, and without bridge~(c). The chain is: a placed tile presents $\beta$ or $\gamma$ at
either endpoint of its $a$-edge, since $\alpha$ is opposite $a$ and the angles order as their
opposite sides (\texttt{MarchFlank.presents\_beta\_or\_gamma}); two $\gamma$'s cannot meet at a
straight-edge point, the system $x+2z=3$, $y+z=2$ giving $x=-1$ at $z=2$
(\texttt{MarchRun.no\_two\_gammas}), so the junction pair is never $(\gamma,\gamma)$
(\texttt{.junction\_cases}); a word in which $\mathrm{BG}$ is never followed by $\mathrm{GB}$ is
monotone, hence $\mathrm{GB}^{j}\mathrm{BG}^{L-j}$, hence carries at most one change
(\texttt{MarchMonotone}); and the $\{3\alpha,2\beta\}$ junction is exactly that change, being the
only pair without a $\gamma$ (\texttt{.transitions\_card\_le\_one},
\texttt{.exceptional\_is\_transition}). Hence all but at most one junction of the run is a march
junction (\texttt{MarchRun.all\_but\_one\_is\_march\_junction}), and
Proposition~\ref{C-prop:orientmonobdy} records the monotonicity half as VERIFIED.

The run itself is an object: \texttt{MarchRunObject.aRun} selects the wall edges of length $a$ on a
line and indexes them, and \texttt{.run\_exceptional\_le\_one} supplies the junction bound for that
object. Two hypotheses remain unproved there --- that the selected run is contiguous along the line,
which is \texttt{BaseChain.base\_chain\_consecutive\_meet}'s business, and that the orientation word
is the one read off the tiles rather than an abstract $\mathrm{Bool}$ sequence. Until those close,
(i) is proved for the junctions of a run but not yet for \emph{the} run of a hypothetical tiling.
...
\medskip
The coordinates also expose the recursion. The fillers' top edges $[A_1,A_2],[A_2,A_3],\dots$ each
have length $a=f$ and lie at the common height, so \emph{the fillers form a new $a$-run one level
up}: the configuration is self-similar, which is the mechanism behind the recurrence. The derived
run starts at the first apex, at $x$-coordinate $d_{\mathrm{GB}}$ or $d_{\mathrm{BG}}$ according to
the chirality, so the chirality does not change the derived run's shape --- it translates it, by
\[
d_{\mathrm{BG}}-d_{\mathrm{GB}}=2f-\tfrac1f
\]
(\texttt{derived\_run\_shift}). Two positions of the original run measure $2f$, so the shift falls
short of exactly two positions by $1/f$ (\texttt{derived\_shift\_defect}); and in units where the
run's $a$-edges are integers, that shortfall is exactly $1$
(\texttt{defect\_scales\_to\_one}). That is the residue killed by
Lemma~\ref{C-lem:stubgap}, and it is what \texttt{MarchStep.offset\_terminal\_dies} consumes at the
end of the march. So the two chiralities are the one that advances flush and the one that advances
by two positions less $1/f$, the defect being precisely what the terminal argument kills.

\medskip
The trace also identifies the two children, which settles what the chiralities \emph{are}. At
$f=12$ the engine places the first $a$-tile as $(0,0)\to(12,0)\to(431/24,\,143\sqrt{575}/24)$ and
the second as $(12,0)\to(24,0)\to(719/24,\,\cdot\,)$, then the filler on the junction and the two
apexes. Every number is the model above: $d_{\mathrm{BG}}=431/24$, the apex height is $h$, the
radicand $575$ is $4f^2-1$, and $719/24=12+d_{\mathrm{BG}}$
(\texttt{MarchCoords.model\_matches\_trace\_f12}, \texttt{.second\_apex\_f12}). The model is not a
reconstruction; it is what the search does.

At the branch the two children are both triangles on the junction with sides $\{a,b,c\}$, as
\texttt{filler\_forced} requires, and they differ exactly as \texttt{chirality\_swaps\_sides}
predicts: one has its far vertices \emph{at the two existing apexes} --- the flush filler --- and the
other does not, its far vertices sitting at different heights because $b$ and $c$ are exchanged on
the two rays. Matching them to the subtree sizes gives the correspondence:
\[
\text{flush}\ \longrightarrow\ a(bp-1)-3,\qquad
\text{crossed}\ \longrightarrow\ a(bp-2).
\]
So the flush chirality advances by one position and the crossed by two. This is obligation (ii)
identified and measured, at $bp=6$ and $bp=7$; it is not yet proved, because a trace decides
instances and the claim is over all $bp$ and all $f$.
...
system over the nonnegative integers gives $17$ solutions at $N=11$, $172$ at $N=23$ and $2096$ at
...
\texttt{SectorArea.volume\_sector} (the unit sector of angle $\theta$
...
statement transported to \texttt{EuclideanSpace} $\R$ \texttt{(Fin 2)}), then
\texttt{VertexSector} and \texttt{AngleSumDissection.Dissection.sum\_localAngle\_eq} (the tiles'
local angles at a point sum to the target's), and finally
\texttt{Geometry.Dissection.hasAngleSums} with
\texttt{PinPlumbing.pin\_angle\_sum\_interior}. Every step is axiom-clean.
