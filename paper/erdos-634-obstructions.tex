...
\emph{What is left.} Three statements, all about the \emph{run} rather than about a vertex:
\begin{enumerate}
\item[\textup{(i)}] that the march's steps land on vertices of the above kind --- frontier, not a
target vertex, carrying a $\gamma$;
\item[\textup{(ii)}] that the two placements advance the configuration by exactly one and by
exactly two positions;
\item[\textup{(iii)}] that both advances reduce to the same problem at smaller $bp$.
\end{enumerate}
With these, the entire $cp=bp-1$ family is excluded for every $bp$ and every $f$, with no search ---
the first finite obligation this branch has offered that covers infinitely many members. Note that
(i)--(iii) are all statements about \emph{where} tiles lie, so none is excluded by
Proposition~\ref{prop:ninetools}: that no-go closes exactly the relocation-invariant tools.
\end{remark}
