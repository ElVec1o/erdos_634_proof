...
\begin{lemma}[The mirrored piece is charged]\label{lem:charge}\lab{VERIFIED}
...
$14\,930$ to $109$ at $f=8$, a wall-clock factor of about $1700$; the orbit $(3,2)$ at $f=8$, which
...
$385$, $N=11$'s from $135$ to $40$, and the certified $44$-tiling is found in $145\,506$ rather
than $831\,344$, every verdict unchanged. On the word-constrained instances it is a severe loss:
...
\begin{proposition}[The adjacent family is independent of the member]\label{prop:findep}\lab{VERIFIED}
...
every prune counter. At $f=12,14,16,18,20$ the word $(6,5)$ gives $506$ nodes with depth $13$ in
every case, and $(5,4)$, $(7,6)$, $(8,7)$, $(9,8)$, $(10,9)$ give $299$, $807$, $1315$, $2124$,
$3441$ likewise. Every other shape grows exponentially in $f$.
\end{proposition}

\begin{proposition}[Its size is Fibonacci in $bp$]\label{prop:fib}\lab{VERIFIED}
...
An inductive proof needs the base cases $bp=3,4$ together with one step: that at an $a\mid a$
junction of the run the $\alpha$-filler admits exactly two chiralities, advancing the configuration
by one and by two positions, and that both reduce to the same problem at smaller $bp$. That step is
geometry and is not proved here. With it, the entire $cp=bp-1$ family is excluded for every $bp$ and
every $f$, with no search --- the first finite obligation this branch has offered that covers
...
\begin{remark}[No higher nilpotent layer]\label{rem:nilptower}\lab{VERIFIED}
