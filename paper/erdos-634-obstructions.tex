...
Attempting to state (ii) and (iii) without naming the geometric objects produces refutable
statements, not merely unproved ones. Writing (ii) as ``for every advance function $\mathrm{adv}$,
$\mathrm{adv}(\text{flush})=1$ and $\mathrm{adv}(\text{crossed})=2$'' is false, witnessed by
$\mathrm{adv}\equiv0$; writing (iii) as ``$R(bp-1)\Rightarrow R(bp)$ for every $R$'' is false,
witnessed by $R(n)=[n\le1]$ at $bp=2$. Both refutations are machine-checked. The advance function is
exactly what the geometry is supposed to \emph{define}, so quantifying over it universally inverts
the statement. Hence (ii) and (iii) require the tile-placement layer before they can be written
down, let alone proved: they are not one lemma short, and no reformulation avoiding the coordinates
will help.
\end{remark}
