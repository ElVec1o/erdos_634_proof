...
\begin{lemma}[Anchored-end clearance]\label{lem:anchorclear}
Write $m=\min(a,b,c)$ for the shortest tile side. Let $[X,Y]$ be an edge of a tile of the tiling with
$X\in\partial ABC$. Then no vertex of the tiling lies in the relative interior of $[X,Y]$ within
distance $m$ of $X$. In particular, if $|XY|=m$ then $[X,Y]$ carries no interior vertex at all: it is
not crossed.
\end{lemma}
...
The opposite side is covered by tiles abutting $[X,Y]$, each contributing a subsegment of one of its
own edges, and consecutive contributions meet at points of $[X,Y]$; a vertex in the relative interior
is exactly such a meeting point. Now $ABC$ is convex and $X\in\partial ABC$, so the continuation of
the segment beyond $X$ leaves $ABC$; hence no covering edge overruns $X$, and the first covering
edge begins exactly at $X$. That edge is a whole tile edge, so its length is $a$, $b$ or $c$, in
particular at least $m$. Its far endpoint --- the first meeting point --- therefore lies at distance
at least $m$ from $X$, and no meeting point lies nearer. If $|XY|=m$ the whole relative interior is
at distance less than $m$ from $X$, so it contains no meeting point.
\end{proof}

\begin{remark}[The constant is attained]\label{lem:anchorclearsharp}
The bound $m$ cannot be improved. Across all eight kernel-checked tilings, edges of minimal length
with exactly one anchored end are crossed $0$ times out of $74$; and among the $52$ crossings that do
occur on one-anchored edges, the least distance from the anchored end to the crossing equals $m$
\emph{exactly} in every tiling that has such a crossing (squared distances $256$, $1024$, $1024$,
$256$, $256$, $64$, $324$ against squared minimal sides $256$, $1024$, $1024$, $256$, $256$, $64$,
$324$). So the lemma is sharp, and no crossing was found violating it.
