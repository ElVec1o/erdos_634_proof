...
\begin{theorem}[The inflation is rigid at $(1,3)$, $(2,5)$ and $(3,7)$]\label{thm:inflrigid}
The inflated tile of Proposition~\ref{prop:inflbdy} admits no tiling by $f^2$ copies whose $c$-side
carries the $p=1$ word, at $(1,3)$, $(2,5)$ and $(3,7)$. Its only boundary is therefore the standard
one, $a^f$, $b^f$, $c^f$.
\end{theorem}
...
in $297$ nodes and $(0,0,5)$ gives \texttt{FOUND\_TILING} in $2\,212$.

The arithmetic behind the two-word dichotomy is machine-checked at all three members
(\texttt{Inflation.middle\_nine}, \texttt{.residual\_75}, \texttt{.residual\_245}), as is the
uniqueness of the $a$- and $b$-side compositions.

The two \texttt{FOUND} runs are negative controls: a search exhausting both profiles would indicate a
malformed instance rather than a theorem. Instances, certificates and node counts are in
...
At $(1,3)$, $(2,5)$ and $(3,7)$: if the west corner block is complete, then the equal side on that side
carries no $a$-edge.
\end{corollary}
...
computation has been run; $(4,9)$, at $81$ tiles, is in progress.
