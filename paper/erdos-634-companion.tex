...
exactly $jb$, divided into $j$ segments of length $b$ by $Q_i=i\,c\,u+(j-i)(f,0)$;
...
\]
and $Q_i$ is the affine combination dividing the segment in ratio $i:(j-i)$, so consecutive $Q_i$
...
\emph{(iii)} $\beta$ is opposite $b$, so its incident edges are $a$ and $c$: the tile in the
$\beta$-slot lays an $a$-edge (direct) or a $c$-edge (mirrored), never a $b$-edge. Both runs left on
...
are both multiples of $f$. Concretely, if the tile is direct its $a$-edge does not reach $Q_{j-1}$
and the remainder $b-a$ is a gap, so no edge ends at $Q_{j-1}$ and some edge crosses it; if it is
mirrored its $c$-edge overshoots $Q_{j-1}$ by $c-b=e^2=1$ and crosses it outright. Either way a
...
$jb-c=f(f^2-f-1)$ are multiples of $f$ and lie in the semigroup. What the proposition does not
supply is the step that forces the west $\alpha$-slot tile $X=T_1+(f,0)$ once an edge crosses
$Q_{j-1}$; at $f=4$ the mirrored $X$ is excluded by base overshoot, and we have not checked that
exclusion for general $f$. Escape route 2 of Conjecture~\ref{conj:advance} closes exactly when it is.
\end{remark}
