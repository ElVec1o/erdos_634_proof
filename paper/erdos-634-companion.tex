...
(c) $T_2$ presents $\alpha$ at $A$ with edges $b$ and $c$ there, the $b$ along $AP$; let $Q$ be its far
vertex, at drop $c\cos\frac{\alpha}{2}$. The chord meets $AQ$ at the point $R$ with
$|AR|/|AQ|=\cos\frac{3\alpha}{2}/\cos\frac{\alpha}{2}=b/c$ by Lemma~\ref{lem:apexid}, so $|AR|=b$ and
$R=P'$; and $T_2$ is split by the line $PR$ through its vertex $P$, so the area ratio is also $b/c$
(\texttt{middle\_fraction}). The length of $PP'$ is $2b\sin\frac{\alpha}{2}=e(f^{2}-e^{2})/f$.
...
\begin{corollary}[No $T$-junction at $P$]\label{cor:noTP}
$T_1$ and $T_2$ each present $\gamma$ at $P$, and $2\gamma=\pi+\alpha$, so the angle left at $P$ is
$\pi-\alpha<\pi$. Hence no tile has $P$ interior to one of its edges: every tile meeting $P$ has a
vertex there, and the residual figure is $\{2\alpha,2\beta\}$ or $\{\beta,\gamma\}$.
\end{corollary}

\begin{proof}
$T_1$ has sides $c,b,a$ at $A,P,J$ as in Theorem~\ref{thm:apexconfig}, so its angle at $P$ is opposite
$c$, namely $\gamma$; likewise for $T_2$. A tile with $P$ interior to an edge would contribute $\pi$,
exceeding the residual $\pi-\alpha$. The residual is $2\alpha+2\beta$, and the vertex arithmetic of
Lemma~\ref{lem:anglecalc} at $k=1$ leaves only the two figures named.
\end{proof}
