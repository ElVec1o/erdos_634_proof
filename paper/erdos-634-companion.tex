...
That is forcing, not exclusion. A gap run contradicts only when the run is blocked at both ends; it
is blocked at $J_{2f}$, which lies on the boundary, but nothing proved here blocks the far end.
Supplying that is the tile-interior blocking of Theorem~\ref{thm:walls13}, and it is the one step
between this analysis and the exclusion of $p=2$ on the whole subfamily. It is open.

The point at which $e\ge2$ is \emph{easier} than $e=1$ is worth recording: $c-a$ is a gap exactly for
$e\ge2$.
\end{remark}
