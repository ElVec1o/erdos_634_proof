...
both tilings we possess: the $44$-tiling of $(16,16,22)$ has base word $a^4c^2ac$ with west side
$c^4$ and east side $a^4c^2$, and the $99$-tiling of $(24,24,33)$ has base $a^2b^7c^2$ with west side
$c^6$ and east side $a^2b^2c^2b^2$. Both are at $m>1$.
...
Both tilings we possess agree: west side all $c$-edges, east side beginning with $a$-edges. Every
...
Theorem~\ref{thm:secondc} uses $e^{2}+ef<f^{2}$ at exactly one step: that $Y$'s edge from $P$ towards
...
Proposition~\ref{prop:nogolden} removes the hypothesis from Theorem~\ref{thm:secondc}, and with it
from Corollary~\ref{cor:pbound}: the bound $pe+2\le f$ holds at every coprime member. It does
\emph{not} remove it from Theorem~\ref{thm:nobothmirror}, whose third cross product is a positive
...
The family $e=f-1$ was outside the reach of \S\ref{sub:apex} until Proposition~\ref{prop:nogolden}
...
Theorem~\ref{thm:basebeta-e1}, and $59$ by the frontier results for $N\le80$. Among the remainder the
...
The theorem discharges Hypothesis~\ref{hyp:walls} at these members; it does not by itself exclude the
...
and the vertex budget of Corollary~\ref{cor:noTP} in both branches
...
found a genuine defect --- Corollary~\ref{cor:pbound} was stated under a hypothesis that fails at
...
circularity, and that Theorem~\ref{thm:apexconfig} and Corollary~\ref{cor:noTP} use nothing beyond
family-wide facts; it found no further defect.

The identification with Hypothesis~\ref{hyp:walls} is the one discussed in
Remark~\ref{rem:wallsfix} and is not settled; the theorem is stated in terms of $p$ for that reason. Theorem~\ref{thm:efminus1} is therefore unconditional as a statement about $p$, and
...
question ``is $p=0$'' are the same question, and at $(1,3)$ Theorem~\ref{thm:walls13} already answers
...
same answer for the west side. Proposition~\ref{prop:inflbdy} carries no unproved geometric hypothesis:
...
Remark~\ref{rem:sidenoa} draws exactly this inference --- ``the complete west block is the tile scaled
by $f$, so its far side \dots\ is subdivided into exactly $f$ $c$-edges'' --- and asserts the middle
step rather than proving it. Corollary~\ref{cor:sidenoa-proved} proves it at the three members where the
...
complete, which is the west half of Hypothesis~\ref{hyp:walls} itself; the corollary is conditional on
...
At $(1,3)$ the theorem also reproves Theorem~\ref{thm:walls13} by an independent route, on $9$ tiles
rather than $26$, which is the regression check that the reformulation of \S\ref{sub:inflation}
...
On the inflation, The junction analysis of \S\ref{sub:inflation} (machine-checked as \texttt{junction\_three}) leaves three completions at each junction of the $k=2$ word
$c\,a^{f}c$, hence $3^{f-1}$ patterns a priori. Proposition~\ref{prop:orientmono} leaves $f+1$, one for
...
particular to the runs enumerated in the second gap of Conjecture~\ref{conj:advance}. That enumeration
...
assume it, and the T-junction whose exclusion is the whole difficulty of \S\ref{sub:apex} is exactly an
...
such straddles. Proposition~\ref{prop:inflparity} is what survives once straddles are hypothesised away,
which is why it proves less than it appears to.

So the uniform proof of Theorem~\ref{thm:inflrigid} remains open. What Proposition~\ref{prop:inflparity}
shows is that any such proof must use more than counting on the both-odd members, since the counting
invariant is identically satisfied there. \textup{(}Machine-checked, \texttt{parity\_25},
\texttt{parity\_49}, \texttt{parity\_silent}.\textup{)}
...
Remark~\ref{rem:parityscope} was retracted after passing a control that could not see its defect.
...
generated by all admissible edge vectors is dense (Remark~\ref{rem:chordscope}). Of the four ways to
...
\begin{proposition}[The general form]\label{prop:straddlegen}
