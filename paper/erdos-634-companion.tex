...
the $198$ members carrying survivors. Since the enumeration also shows exactly one surviving column
for each $k$ in range (observed on all $655$ columns; we do not prove it), the surviving set at a
member is described completely by
\[
k=1,\dots,\Bigl\lfloor\frac{2ef-3e-f}{b}\Bigr\rfloor .
\]
This is a bound and a description, not the required $k=0$. What it buys is that the residual
question is one linear equation per $k$, with $k$ in an explicit finite range.
\end{remark}
