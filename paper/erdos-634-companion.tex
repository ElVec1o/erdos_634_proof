...
Two inputs of Proposition~\ref{prop:flankline} are supplied by the dissection in general
(\texttt{covered\_of\_mem\_target}, \texttt{approach\_covered},
\texttt{above\_line\_of\_below\_tile}) but have not been instantiated at the escape configuration
itself: that the approach points to $V$ exist and lie above the line, and that the tile below the
wall occupies the lower side near $V$. Until they are, the chain above is a proof \emph{given} the
configuration, not a proof about it. No further geometric mechanism is missing; what is missing is
the identification of the configuration's tiles with the hypotheses' $T$ and its neighbour.
\end{remark}
