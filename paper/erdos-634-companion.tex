...
Second, and unexplained, $a$-edges with one anchored end are crossed in none of them --- $0$ out of
$54$. A whole-edge covering of an $a$-edge is a single $a$ by uniqueness of the representation, so a
crossing would need the far side's first edge to be a $b$ or a $c$, overrunning the interior end by
$b-a$ or $c-a$. We have no proof that this cannot happen and record the observation only as that.
\end{remark}
