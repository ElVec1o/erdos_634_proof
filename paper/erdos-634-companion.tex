...
soundness. The assembled conditional statement is
\texttt{FanStep.word42\_junction\_dies}: the only exact fill of the $\beta$-wedge is a single
$\beta$-tile (\texttt{beta\_fill\_unique}), which dies on the two-gap contract, and every
non-exact fill stalls at the sub-$\alpha$ fan remainder, which no tile angle fits.
...
excluded (\texttt{no\_interior\_tile\_at\_pin}); and the flank identification is derived
combinatorially (\texttt{west\_flank\_gamma}, \texttt{east\_flank\_cases}). The congruence layer is
