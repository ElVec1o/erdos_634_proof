...
combinatorially (\texttt{west\_flank\_gamma}, \texttt{east\_flank\_cases}). What remains:
identifying a congruent tile's corner angles with $\{\alpha,\beta,\gamma\}$ through the
congruence layer, the covering data (\texttt{wall\_partition}) along the flanking rays, and the
mirrored ladder's termination, which is the per-word content --- the T-vertex step of that ladder
being already forced (\texttt{t\_vertex\_fill}). An adversarial
