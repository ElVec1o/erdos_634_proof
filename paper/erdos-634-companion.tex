...
\begin{proposition}[The orientation of a boundary $a$-run is monotone]\label{prop:orientmono}\lab{PROVED: the alphabet lemmas VERIFIED}
Let a maximal run of consecutive $a$-edges lie on a straight side of the target, or on a side of the
inflated tile, and let $T_1,\dots,T_L$ be the tiles laying them. Each $T_i$ presents $\beta$ at one end
of its $a$-edge and $\gamma$ at the other; write its orientation as $\mathrm{BG}$ or $\mathrm{GB}$
according to which end carries $\beta$. Then no $\mathrm{BG}$ is followed by a $\mathrm{GB}$: the
sequence is $\mathrm{GB}^{j}\mathrm{BG}^{L-j}$ for a single $j$. Consequently at most one junction of
the run carries the figure $\{3\alpha,2\beta\}$, and it is the transition.
\end{proposition}
