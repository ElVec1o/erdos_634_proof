...
$\gamma$-trap forces $z\ge1$ (Lemma~\ref{lem:basedi}), deleting $j=2$.
...
To reach the walls form one further needs Theorem~\ref{thm:n1}, whose statement assumes separation.
That hypothesis is inert, as the next proposition records.
\end{remark}

\begin{proposition}[Theorem~\ref{thm:n1} does not need separation]\label{prop:n1general}
The separation hypothesis may be deleted from Theorem~\ref{thm:n1}: for all $e\ge1$, $f\ge2$ with
$\gcd(e,f)=1$, no tiling of the base-$\beta$ target at $m=1$ has base walk $(0,e,2e)$.
\end{proposition}

\begin{proof}
The proof of Theorem~\ref{thm:n1} applies verbatim; separation is used at no step, as follows. The
corner angle is $\beta$ and the corner tile is unique with flanks $\{a,c\}$, which is the base-$\beta$
corner structure and holds for every member. Since the base carries no $a$-edge, $T_0$ lays $c$ on
the base and $a$ on the side; the side has length $f^3>ef=a$, so the junction at $a\,u$ is interior
and straight, and the wedge arithmetic $\pi-\gamma=\alpha+\beta$ is a consequence of
$3\alpha+2\beta=\pi$ and $\gamma=2\alpha+\beta$ alone. The chord from $(c,0)$ to $a\,u$ is a whole
edge of $T_0$, so no tile crosses it, and both of its ends lie on the target boundary, so no covering
edge overruns them; its far side is therefore a union of whole edges of total length $b$, and
$b=xa+yb+zc$ forces $(x,y,z)=(0,1,0)$ (\texttt{partner\_unique}), which is general in $(e,f)$. In
the induction, the exclusion of a $c$ laid along $T_k$'s $b$-ray uses only $c>b$, i.e.\ $e^2>0$, and
the slot fills use only the wedge arithmetic. The induction then forces every base edge to be a
$c$-edge, which already contradicts the assumed column, since $(0,e,2e)$ carries $e\ge1$ $b$-edges;
independently it forces $f^2\mid e(3f^2-e^2)$, hence $f^2\mid e^3$ and $f=1$ by coprimality
(\texttt{base\_not\_all\_c}, whose Lean hypotheses are only $2\le f$, $e<f$, $\gcd(e,f)=1$).
\end{proof}

\begin{corollary}[The walls form at $f>2e$]\label{cor:wallsf2e}
Let $\gcd(e,f)=1$ and $f>2e$. Then the base walk is the walls form $(f,e,e)$.
\end{corollary}

\begin{proof}
Corollary~\ref{cor:basedi2e} leaves $(0,e,2e)$ and $(f,e,e)$; Proposition~\ref{prop:n1general}
deletes the first.
\end{proof}

\begin{remark}[Status of Proposition~\ref{prop:n1general}]\label{rem:n1status}
The three arithmetic ingredients above were checked on all $36896$ coprime members with
$e\le200$, $e<f<4e+2$, of which $17296$ are close pairs: $b$ has no decomposition other than a single
$b$-edge, $c>b$ holds, and $f^2$ never divides $e(3f^2-e^2)$. The remaining steps are the base-$\beta$
corner structure and the wedge arithmetic, neither of which refers to the separation inequality.
Corollary~\ref{cor:wallsf2e} accordingly extends Corollary~\ref{cor:basewalls} from
$f>(1+\sqrt2)e$ to $f>2e$. Being a deletion of a hypothesis from an existing geometric argument
rather than a new one, Proposition~\ref{prop:n1general} carries the label PROVED, not machine-checked;
its geometric steps are the same ones Theorem~\ref{thm:n1} already rests on.
\end{remark}
...
\begin{proposition}[The side parameter]\label{prop:sidep}
Let $\gcd(e,f)=1$, $1\le e<f$, $m=1$, and let $n_a,n_b,n_c$ be the edge counts of an equal side.
Then $n_b=0$; moreover $f\mid n_a$, and writing $n_a=pf$ one has
\[
n_c+pe=f .
\]
The $\gamma$-trap gives $n_c\ge1$, so $0\le p\le(f-1)/e$, and Hypothesis~\ref{hyp:walls} is exactly
the assertion $p=0$.
\textup{(\texttt{SideNoB.side\_no\_b\_uncond}, \texttt{side\_p\_parametrisation}.)}
\end{proposition}

\begin{proof}
That an equal side carries no $b$-edge is \texttt{side\_no\_b\_uncond}, whose hypotheses are
$\gcd(e,f)=1$, $e<f$ and $n_c\ge1$; no size condition is involved. The side has length $f^3$, so
$n_a\,ef+n_c f^2=f^3$; dividing by $f$ gives $n_a e+n_c f=f^2$, whence $f\mid n_a e$ and so
$f\mid n_a$ by coprimality. Writing $n_a=pf$ and dividing once more gives $pe+n_c=f$. Finally
$n_c\ge1$ is the $\gamma$-trap.
\end{proof}

\begin{corollary}[A uniform two-case side problem]\label{cor:ptwo}
Suppose in addition that $2e<f$ and that $(e,f)$ is a close pair, $f^2\le2ef+e^2$. Then
...
Corollary~\ref{cor:wallsf2e} pins the base walk exactly on the close pairs with $f>2e$, which is
where Lemma~\ref{lem:basetri} did not apply; there are $203$ such members with $e\le40$, the
smallest being $(3,7)$ with $N=138$, then $(4,9)$ with $N=227$ and $(5,11)$ with $N=338$. On the same
family Corollary~\ref{cor:ptwo} bounds the side parameter by $2$. Checked on all $126003$ close
pairs with $f>2e$ and $e\le1000$: $\lfloor(f-1)/e\rfloor=2$ in every one.
