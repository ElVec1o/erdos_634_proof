...
Proposition~\ref{prop:sharpcolumn} rules them out under the weaker hypothesis $f\ge2e$, so the
trichotomy and the dichotomy both extend.

\begin{corollary}[The base trichotomy and dichotomy without separation]\label{cor:basedi2e}
Let $\gcd(e,f)=1$ and $f\ge2e$. Then the base admits exactly the three decompositions
...
$-ye^2\equiv-e^3$, so $f\mid e^2(y-e)$ and hence $f\mid y-e$ by coprimality. Write $y=e+kf$ with
$k\ge0$. If $k\ge1$ then Proposition~\ref{prop:sharpcolumn}(iii) forces $f\le2e-1$, contrary to
$f\ge2e$; so $y=e$. Then $f\mid x$, say $x=jf$, and the base equation becomes $je+z=2e$, i.e.\
...
covers the band $2e\le f\le(1+\sqrt2)e$ that Lemma~\ref{lem:basetri} left to a per-member
computation: $5065$ of the $17296$ close pairs with $e\le200$ lie in it. For those members the base
dichotomy now holds for the same reason it holds at a separated member.

It does \emph{not} follow that the base walk is the walls form there, because the last step of
Corollary~\ref{cor:basewalls} is Theorem~\ref{thm:n1}, whose statement assumes separation.
Inspecting its proof, separation is not visibly used: the chord's far side is a single $b$ by
\texttt{partner\_unique}, which is general in $(e,f)$ and is checked at the close pair $(2,3)$; the
overshoot step needs only $c>b$; and the final contradiction is $f^2\mid e^3$, i.e.\
\texttt{base\_not\_all\_c}, whose Lean statement assumes only $2\le f$, $e<f$ and
$\gcd(e,f)=1$. Whether the separation hypothesis of Theorem~\ref{thm:n1} can simply be deleted is
therefore a concrete open question, and an affirmative answer would give the walls form throughout
$f\ge2e$. We do not assert it here.
\end{remark}
