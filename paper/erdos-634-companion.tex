...
(\texttt{sortedPositions} and its three lemmas). What remains of bridge (c) is therefore the
single step of reading the tile back from its position --- immediate from the key's injectivity
--- and the input that an
$a$-edge's two ends carry $\beta$ and $\gamma$, which is the corner theory applied to a real
tile. No further geometric obstacle stands in the way, but until those two are supplied the
