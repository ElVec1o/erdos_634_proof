...
when $f\le 2e-1$ (\texttt{base\_column\_k\_one}). And since $x,z\ge1$ the equation forces
\[
kb\;\le\;2ef-e-f
\qquad(\texttt{base\_column\_k\_bound}),
\]
which is the size bound $kb\le 2ef$ of Remark~\ref{rem:closepairs} sharpened by the two positivity
constraints. It is a bound, not the required $k=0$; what the reduction buys is that the residual
question is now one linear equation per $k$, with $k$ ranging over an explicitly bounded set.
\end{remark}
