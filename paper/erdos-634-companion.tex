...
Let $\mathcal T$ be a tiling of the base-$\beta$ target at $m=1$. Then both base-corner tiles lay an
$a$-edge on the base and a $c$-edge on the equal side. Equivalently, by Lemma~\ref{lem:ccorner},
neither equal side carries an $a$-edge: $p=0$ on both.
\end{hypothesis}

\begin{remark}[Correction to the statement of Hypothesis~\ref{hyp:walls}]\label{rem:wallsfix}
Earlier versions stated the hypothesis as the completeness of two corner \emph{blocks}, ``$f$ $a$-feet
at the west corner, $e$ $c$-feet at the east''. That form is inconsistent with
Theorem~\ref{thm:e1reduce}, which proves, for $e=1$, $m=1$, $f\ge3$, that the base walk begins
\emph{and} ends with an $a$-edge and carries its single $c$-edge in the interior. The block form
places a $c$-edge at the east corner and concentrates all $f$ $a$-edges at the west, contradicting
both halves of that theorem; at $(1,2)$ it would give $p=0$ on the west side and $p=1$ on the east,
contradicting Lemma~\ref{lem:ple} as well.

The form above is what every downstream result consumes --- \S\ref{sub:sidep}, Lemma~\ref{lem:ple},
Corollary~\ref{cor:ptwo}, Theorem~\ref{thm:ptwodead} and all of \S\ref{sub:apex} --- and it agrees with
Theorem~\ref{thm:e1reduce}. The $e$ $c$-edges of the walls base walk $(f\,a,\,e\,b,\,e\,c)$ sit in the
interior of the base, not at a corner. Remark~\ref{rem:blockbreaks}'s identification of the west block
as the tile scaled by $f$ stands; its dual east block does not, and the remark should be read for its
$m$-dependence argument only.
\end{remark}
...
\emph{The east block described below does not occur; see Remark~\ref{rem:wallsfix}. The identification
of the west block and the $m$-dependence argument that follows are unaffected.}
...
n_c+pe=f ,
\]
and the $\gamma$-trap's $n_c\ge1$ gives $0\le p\le(f-1)/e$. Hypothesis~\ref{hyp:walls} is, in the form
stated there, exactly the assertion $p=0$ on both equal sides; see Remark~\ref{rem:wallsfix} for why
that form replaces an earlier one. Lemma~\ref{lem:ccorner} already records this profile in the form ``$a$-count
...
Since Hypothesis~\ref{hyp:walls} is now stated as $p=0$ on both equal sides
(Remark~\ref{rem:wallsfix}), this establishes the hypothesis at every such member.
\end{theorem}
...
The identification with Hypothesis~\ref{hyp:walls} is now immediate, the hypothesis being stated as
$p=0$ on both equal sides. That restatement was forced: the earlier corner-block form contradicted
Theorem~\ref{thm:e1reduce}, as Remark~\ref{rem:wallsfix} records. Theorem~\ref{thm:efminus1} is therefore unconditional as a statement about $p$, and
