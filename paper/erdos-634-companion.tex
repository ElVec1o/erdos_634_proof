...
What it does not yet give is rigidity: the cross-section of a straddling tile varies continuously with
its position, so no individual value is forced. Closing $p=1$ by this route requires the missing
ingredient to be supplied by the $\gamma$-grading of \S\ref{sub:gammagrading} --- a straddling tile's
edges have directions that are integer multiples of $\gamma$ modulo $\pi$, so its cross-section is not
free but drawn from a countable set determined by the tile's label. Whether that set meets the
required totals is open, and is the next question we intend to settle.
\end{remark}
