...
The $e=1$ case of this parametrisation is already the opening step of the proof of
Theorem~\ref{thm:e1reduce}, where $n_a=fk$ and $n_c=f-k$ are obtained after ruling out $n_b\in\{f\}$
by a size count together with the $\gamma$-trap. What is new here is only that the elimination of
$n_b$ is available at every member, via \texttt{side\_no\_b\_uncond}, so the parametrisation and the
resulting parameter $p$ exist in general and not merely at $e=1$.
...
Proposition~\ref{prop:sidep} and Corollary~\ref{cor:ptwo} are independent of
Theorem~\ref{thm:n1} and of the base walk: they constrain the side at any member. On the close pairs
