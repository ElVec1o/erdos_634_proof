...
The trap does not transport, and the reason is exact. On a side the two ends are a base corner and
the apex, both $\gamma$-free for boundary reasons. On the interior stretch neither is: an endpoint's
figure from above may be $\{\alpha,\beta,\gamma\}$, which closes at $\pi$ since
$\gamma+\alpha+\beta=3\alpha+2\beta$. It is moreover typical, since the $a$-run orientation word
is monotone, $M^iD^{f-i}$, and the $\gamma$s then occupy every junction but the $i$-th, so both ends
carry one unless $i\in\{0,f\}$. And \texttt{gamma\_count\_tight} shows the hypothesis cannot be
weakened: $f$ $\gamma$s do fit in $f+1$ junctions at one apiece, so the count is exactly tight rather
than merely unproved.

The next input must therefore come from outside the run: a reason the stretch's endpoints cannot
present $\gamma$ from above, which is a boundary-like fact at an interior line. That is the smallest
open statement the $e=1$ hole now rests on, and it concerns two vertex figures.
\end{remark}
