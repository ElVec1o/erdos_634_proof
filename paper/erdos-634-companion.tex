...
Theorem~\ref{thm:e1reduce} then says something sharper than a wording problem: at $e=1$, $m=1$,
$f\ge3$ it proves the base begins \emph{and} ends with an $a$-edge, so no tiling there has $c$-feet at
the east corner. The east half of the hypothesis is thus unsatisfiable at those members, and any
result quoted as ``establishing Hypothesis~\ref{hyp:walls}'' there establishes its west half only.
This does not affect \S\ref{sub:apex}, whose results are statements about $p$ on a given equal side and
stand on their own proofs, but it does mean the hypothesis should be split into its two halves before
being used further.
\end{remark}
