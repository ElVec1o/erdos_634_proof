...
to be dead: the whole class of linear direction invariants and the metric bounds derived from them;
the hope of closing $e=1$ by proving that every side carries at least two $c$-edges; Euler and
angle-sum counting, which collapse to a single relation; the non-abelian Conway--Lagarias tiling
group, which collapses to its rank-two abelianisation; the reach ladder, which costs one geometric
step per member; and two rounds of purpose-built machinery, both of which reached a true theorem
whose hypothesis is never satisfied. The common cause is worth stating: every invariant the sweep
produces \emph{counts}, and what the open step needs is one that \emph{locates}.

One further correction belongs in the abstract rather than the body. The complete-corner-wall
hypothesis under which the main paper states its conditional theorem is, at $e=1$, equivalent to its
own conclusion: a complete west block requires the base word to begin $a^f$, while the reduction
theorem for that subfamily makes it a permutation of $(a^f,b,c)$ ending in $a$, so the hypothesis
holds there exactly when no tiling exists. It is therefore not a weaker assumption at $e=1$, and no
exclusion of an $e=1$ member may cite it. At $e\ge2$ it is a genuine positional strengthening and
remains open.
\end{abstract}
...
\begin{theorem}[The base-$\beta$ family at $e=1$, $f=2$]\label{thm:basebeta-e1}\lab{PROVED}
...
\begin{theorem}[Boundary walks of the base-$\beta$ target at $m=1$]\label{thm:walks}\lab{PROVED}
...
\begin{theorem}[Apex mismatch: the pierced corner]\label{thm:pierce}\lab{PROVED}
...
\begin{proposition}[Rung two: the pre-piercer chain]\label{prop:rung2}\lab{PROVED}
...
\begin{theorem}[Alignment of the mismatch ray]\label{thm:align}\lab{VERIFIED}
...
\begin{theorem}[The mismatch ray is completely determined]\label{thm:ray}\lab{PROVED}
...
\begin{theorem}[Unique representation of $k\,b$]\label{thm:kbrep}\lab{VERIFIED}
...
\begin{theorem}[The angle $(\pi-\alpha)/2$]\label{thm:halfangle}\lab{VERIFIED}
...
\begin{theorem}[The $b$-run orientation lemma]\label{thm:chain}\lab{VERIFIED}
...
\begin{theorem}[The far region is a scaled tile]\label{thm:farregion}\lab{PROVED}
...
\begin{corollary}[The far side can never give a contradiction]\label{cor:farvacuous}\lab{PROVED}
...
\texttt{code/rust/walls} (binary \texttt{straddle}), and it is recorded here because a negative result of
...
\begin{theorem}[Walk structure at $m=1$]\label{thm:walkstruct}\lab{PROVED: the walk arithmetic VERIFIED}
...
\begin{theorem}[The $e=1$ subfamily reduces to one walk]\label{thm:e1reduce}\lab{PROVED}
...
\begin{theorem}[The $e=1$ base-$\beta$ family]\label{thm:e1family}\lab{PROVED for $f\ge3$ (machine-checked); $f=2$ by exhaustive search}
...
For $f\ge3$ this is machine-checked
(\texttt{lean/Erdos634/E1BaseWord.lean}, \texttt{e1\_family\_f\_ge\_3\_congruent}), axiom-free
beyond \texttt{propext}, \texttt{Classical.choice} and \texttt{Quot.sound}, over exactly the class
of tilings above (any congruent dissection, isometries including reflections, tile count
unconstrained). The route is not the strip-and-column chain below but a direct argument on the
base layer: every junction of the forced base word is a boundary point of the target, hence a
finite vertex figure, which pins each tile's orientation (\texttt{MarchInduction.lean},
\texttt{MarchSlots.lean}); an exact-partition lemma for a straight run clamped between two placed
tiles or the target boundary (\texttt{RunPartition.lean}, formalising the argument of
Proposition~\ref{prop:cornerpara}) then excludes every remaining case
(\texttt{MarchCompose.lean}). The case $f=2$ ($N=11$) is settled by exhaustive computational search
(Table~\ref{tab:basebeta}) and is not part of the Lean theorem, which requires $f\ge3$.

The strip-and-column chain below predates the Lean route above and is kept for its architecture;
we state it and prove the three
...
\begin{remark}[At $e=1$ the completion branch is empty, so the column is unconditional]\label{rem:nocomplete}
Remark~\ref{rem:sidenoa} already establishes that complete blocks are unavailable at $e=1$, and
concludes from it that the hypothesis there \emph{is} the case rather than a step towards it. What
follows is the structural dividend of that observation, which was not taken.

The chain above carries, at each corner, the alternative ``either the quadratic growth completes
($k=f$) or a first break occurs''. At $e=1$ the first alternative never occurs, and --- unlike the
argument of Remark~\ref{rem:sidenoa}, which reads the contradiction off the east corner's $c$-foot
--- this can be seen at the west corner from the letter budget alone, which is what closes the
west-only retreat proposed in Remark~\ref{rem:wallsfix}. A complete west block has $f$ $a$-feet on the base; feet are
consecutive from the corner, so it requires the base word to begin with $a^f$. A complete east block
at $e=1$ has a single $c$-foot, so it requires the base word to end with $c$. But
Theorem~\ref{thm:e1reduce} makes the base word a permutation of $(a^f,b,c)$ beginning \emph{and}
ending with $a$: the first requirement asks for $f+1$ letters $a$ out of $f$, and the second
contradicts the final $a$ outright.

So both corners break. A base-break consumes the walk's $c$, and $(f,1,1)$ has only one, so at most
one of the two breaks is on the base; hence at least one corner breaks on a \emph{side}, and the
forced horizontal column of $f$ tiles exists in every tiling of every $e=1$ member. The column is
therefore not one branch of a case split at $e=1$ --- it is unconditional, and the subfamily rests
on the single deferred step that the column reaches the base.

This also settles the status of Hypothesis~\ref{hyp:walls} at $e=1$, since the hypothesis is
precisely the assertion that the completion branch is the one that occurs: it holds there exactly
when no tiling exists, so no exclusion of an $e=1$ member may cite it. Formalised in
\texttt{lean/Erdos634/WallsCircular.lean} (\texttt{west\_block\_never\_complete},
\texttt{both\_corners\_break}, \texttt{some\_side\_break}), axiom-clean.
\end{remark}

\begin{lemma}[Filler identity]\label{lem:fillerid}\lab{VERIFIED}
With $J_i$ the $i$-th column junction and $\mathrm{ap}_i$ the $i$-th apex, the two identities
$\bigl(\tfrac{3f^2-1}{2f}\bigr)^2+c^2\sin^2\beta=f^4$ and
$\bigl(\tfrac{f^2-1}{2f}\bigr)^2+c^2\sin^2\beta=(f^2-1)^2$
are consistent --- the same $c^2\sin^2\beta$ serves both --- and eliminating it leaves an identity
in $f$ alone, valid for every $f\neq0$ (\texttt{FillerGeneral.filler\_consistent}). In the
denominator-cleared form these read
$(f^2-1)^2+4f^6-(3f^2-1)^2=4f^2(f^2-1)^2$ and $(3f^2-1)^2+(4f^6-(3f^2-1)^2)=4f^2(f^2)^2$,
identities over any commutative ring (\texttt{.filler\_b\_general}, \texttt{.filler\_c\_general}).
\end{lemma}

\begin{lemma}[What the filler identity buys]\label{lem:filler}\lab{PROVED}
Hence the triangle $(J_i,\mathrm{ap}_i,\mathrm{ap}_{i+1})$ has sides exactly $c,b,a$, so the column
and its fillers tile the full inter-level strip and lay a run of $a$-edges one level down, whose far
sides decompose only as single $a$-edges. The strip, the column and its fillers are not objects of
the Lean development.
\end{lemma}

\begin{lemma}[Offset congruence]\label{lem:offsets}\lab{PROVED}
...
\begin{lemma}[Interior multiples]\label{lem:interior}\lab{CONJECTURE: effective, resting on the deferred column}
...
$c,b,a$; \texttt{lean/GeneralPillars.lean} per member, and general in $(e,f)$ over any
commutative ring as \texttt{FillerGeneral.filler\_b\_general\_ef},
\texttt{.filler\_c\_general\_ef}, \texttt{.filler\_consistent\_ef}, of which those
per-member checks are instances); the sector inequalities $\gamma-(\alpha+\beta)=\alpha$
...
\begin{hypothesis}[Complete corner walls]\label{hyp:walls}\lab{CONJECTURE: open at $e\ge2$, and at $e=1$ equivalent to its own conclusion}
...
\begin{remark}[Hypothesis~\ref{hyp:walls} is asymmetric; the $p=0$ reading is its west half]\label{rem:wallsfix}
Two readings of the hypothesis are in circulation in this development and they are not equivalent.

The block form above is asymmetric: $a$-feet west, $c$-feet east. By Lemma~\ref{lem:ccorner} a corner
tile laying $c$ on the base lays $a$ on the equal side, so the block form puts an $a$-edge at the foot
of the east side and gives $p_{\mathrm{west}}=0$, $p_{\mathrm{east}}\ge1$. That asymmetry is visible in
both tilings we possess: the certified $44$-tiling (\texttt{Tiling44.lean}, the tiling
\texttt{N44B}) has base walk $a^4c^3a$ with west side $c^4$ and east side $ca^2c^2$ read from the
base corner, and the $99$-tiling of $(24,24,33)$ has base $a^2b^7c^2$ with west side $c^6$ and
east side $a^2b^2c^2b^2$. Both are at $m>1$. (The words are the exact sweep of
\texttt{code/interface\_census.py}; an earlier version of this remark misquoted the $44$-tiling's
words as $a^4c^2ac$ / $a^4c^2$.)

Against this, \S\ref{sub:sidep} reads the hypothesis as ``exactly the assertion $p=0$'', which is
symmetric in the two sides, and Theorem~\ref{thm:e1reduce} proves that at $e=1$, $m=1$, $f\ge3$ the
base walk begins \emph{and} ends with an $a$-edge --- so its single $c$-edge is interior and neither
corner carries $c$-feet. The block form and that theorem cannot both describe an $m=1$ tiling at
$e=1$.

The discrepancy is localised, and Remark~\ref{rem:sidenoa} localises it. Its derivation reads: ``the
complete \emph{west} block is the tile scaled by $f$, so its far side --- which at $m=1$ is the whole
equal side --- is subdivided into exactly $f$ $c$-edges, i.e.\ $p=0$.'' That argument uses the west
block only, and the conclusion is drawn for the west side; it is then stated for ``the sides''. The
east block, being the tile scaled by $e$, has far side $e\,a$ and therefore places $e$ $a$-edges at the
foot of the east side.

That resolution does not survive, and the reason is that the decisive clash is at the \emph{west}
corner, not the east. The complete west block has $f$ $a$-feet on the base; feet are consecutive and
begin at the corner, so the block form asserts that the base word begins with $a^f$. By
Theorem~\ref{thm:e1reduce} the base word at $e=1$, $m=1$, $f\ge3$ is a permutation of $(a^f,b,c)$
whose \emph{last} letter is also an $a$, and $f+1$ letters $a$ do not exist. Hence at every $e=1$
member the west corner of every tiling is starved or broken. Since the west block is the sole
premise of the derivation of $p=0$ quoted above, the west-only reading is not a weaker surviving
form of the hypothesis: it has no premise at all at $e=1$. The correct conclusion is that at $e=1$
the hypothesis holds exactly when no tiling exists, so any exclusion of an $e=1$ member citing it is
circular. This is \texttt{lean/Erdos634/WallsCircular.lean}, axiom-clean; the counting step is
\texttt{BaseWordBlock.no\_f\_plus\_one\_a}. The $e=1$ members remain excluded by the
reduction of Theorem~\ref{thm:e1family}, which nowhere mentions the hypothesis; that reduction
rests in turn on the deferred strip-and-column step, so the subfamily stands reduced rather than
closed, and the hypothesis is of no help in closing it.

So the hypothesis is genuinely asymmetric, and for $e\ge2$ the identification with $p=0$ holds for the west side.
Both tilings we possess have west side all $c$-edges; the $99$-tiling's east side begins with
$a$-edges as the block form predicts, while the $44$-tiling's east side begins with a $c$ ---
consistent with Remark~\ref{rem:blockbreaks}, whose east corner blocks are broken in exactly that
tiling, so it is no counterexample to the block form's prediction for \emph{complete} corners. Every
statement of the form ``Hypothesis~\ref{hyp:walls} is exactly $p=0$'' should be read as applying to the
west side; on the east the hypothesis gives $p\ge1$.

That the block reading fails at $e=1$ is not new here: Remark~\ref{rem:whyhyp} already records that
Theorem~\ref{thm:e1reduce}, forcing the base walk to begin and end with $a$-edges, ``is incompatible
with the complete-block reading of the base, so \emph{every} hypothetical $e=1$ tiling falls into the
starved-or-broken clause''. What the present remark adds is only where the two readings part company
--- Remark~\ref{rem:sidenoa} argues from the west block and states its conclusion for both sides ---
together with the confirmation from the two certified tilings.

The practical consequence is that the hypothesis should be split into its west and east halves before
being used further, and that a result quoted as ``establishing Hypothesis~\ref{hyp:walls}'' should say
which half.

Having split them, it is worth recording how unequal the two halves are, since the split might be read
as doubling the work and it does not. The base accounts as
\[
  e(3f^2-e^2)\;=\;\underbrace{f\,a}_{\text{west foot}}\;+\;\underbrace{e\,b}_{\text{middle}}\;+\;\underbrace{e\,c}_{\text{east foot}},
\]
an identity in $(e,f)$. The west block is the tile scaled by $f$ and carries $f^2$ tiles; the east block
is the tile scaled by $e$ and carries only $e^2$. Three consequences.

First, \emph{at $e=1$ the east block is a single tile}, and the east half of the hypothesis has no
content beyond Lemma~\ref{lem:ccorner}: a base corner tile laying $c$ lays $a$ on the side. So on the
whole thin family the east half is free, and what remains there is the base-form question of
Lemma~\ref{lem:basedi}, which the two halves share.

Second, the east block is smaller than the west at every member --- $e^2$ against $f^2$, a factor
between $4.8$ and $49$ on the members tabulated in \S\ref{sub:inflation} --- so any method that settles
the west settles the east \emph{a fortiori} in size, if not automatically in content.

Third, and most usefully, the side the east block presents to the equal side is its $a$-side, of length
$e\,a$; and Proposition~\ref{prop:inflbdy} makes the $a$-side of an inflation \emph{rigid}, namely
$a^{e}$. The $p=0$ versus $p=1$ ambiguity that is the whole difficulty of the west half lives on the
$c$-side, which for the east block lies along the base rather than along an equal side. The east half is
therefore not a second copy of the crux: it is a smaller problem, and the feature that makes the west
hard is absent from it. This does not affect \S\ref{sub:apex}, whose results are statements about $p$ on a given
equal side and stand on their own proofs.
\end{remark}

\begin{theorem}[The base-$\beta$ branch, conditional on Hypothesis~\ref{hyp:walls}]\label{thm:basebeta-full}\lab{CONJECTURE}
...
block would need the footprint $ef^2=4$ to be cleanly composed. Of the other two, the $99$-tiling
of $(24,24,33)$ has both unit-scale corner blocks complete (zero wall straddlers, exact sweep,
\texttt{code/interface\_census.py}); \texttt{N44C} has its east block complete but its west
$f$-block broken --- six tiles straddle that wall. Among the three genuine tilings exactly one
realises both complete blocks.
...
\begin{lemma}[Angle calculus]\label{lem:anglecalc}\lab{PROVED}
For every member $(e,f)$:
\begin{enumerate}
\item \textup{(No right angle.)} $\pi/2$ has no representation: it would force $2x=3$.
...
\begin{remark}[No piece has a right angle]\label{rem:norightangle}\lab{VERIFIED}
Since every piece is congruent to the tile, its corner angles are $\alpha,\beta,\gamma$, none of
which is $\pi/2$ by clause~(1). Hence no piece of any dissection of a base-$\beta$ region has a
right angle: the target cannot be split along its altitude, and no perpendicular cut is available
...
\begin{proposition}[$\gamma$-grading]\label{prop:gammagrading}\lab{PROVED}
...
\begin{proposition}[The direction group of a branch]\label{prop:dirgroup}\lab{PROVED}
...
\begin{lemma}[Terminal wedge]\label{lem:termwedge}\lab{PROVED}
...
\begin{lemma}[Pentagon]\label{lem:pentagon}\lab{PROVED}
...
\begin{lemma}[The second stub is a gap]\label{lem:stubgap}\lab{VERIFIED}
...
($12\,231$ pairs, \texttt{code/rust/stubgap}) finds that \emph{both} initial stubs are gaps at the first
...
with $f\le400$ (\texttt{code/rust/stubgap}). Note that $(0,\min(a,b))$ and $\{|a-b|\}$ are genuinely
...
\begin{lemma}[Two $c$-edges are forced; $p\le f-2$]\label{lem:ple}\lab{CONJECTURE}
...
\begin{lemma}[The equal sides carry no $b$-edge]\label{lem:sidenob}\lab{VERIFIED}
...
\begin{remark}[Correction to Remark~\ref{rem:sidenoa}: $p=0$ is not the open step, and fails for one side]\label{rem:pnotzero}
Both supports of the previous remark's identification have to be withdrawn, and the target replaced.

The \emph{derivation} of $p=0$ there is ``the complete west block is the tile scaled by $f$, so its
far side --- the whole equal side at $m=1$ --- is subdivided into exactly $f$ $c$-edges''. A complete
west block puts $f$ consecutive $a$-feet at the corner, so it needs the base word to begin with
$a^f$; with exactly $f$ letters $a$ and a final $a$ forced by Theorem~\ref{thm:e1reduce}, that is
$f+1$ of them. The premise never holds, so the derivation is vacuous.

The \emph{sufficiency} of $p=0$ --- ``note that $p=0$ closes the subfamily outright'' --- is
justified there by an argument every step of which is about complete blocks: ``complete blocks put
$f$ $a$-feet at one base corner and $e=1$ $c$-foot at the other, so the base reads $a^f\,b\,c$''.
That silently substitutes complete blocks for $p=0$, which requires the converse implication
$p=0\Rightarrow$ (walls). The converse appears only as part of the statement of
Conjecture~\ref{conj:advance}, proved at $f=2,3,4$ alone.

Moreover the symmetric reading is not merely unsupported but false. By
Remark~\ref{rem:nocomplete} both corners break; a base-break consumes the walk's single $c$, so at
most one break is on the base, and some corner breaks on a side. A corner block breaks on a side by
failing to read a $c$ where the block demands one, and the block's partial far side lies on the
adjacent equal side. Hence
\[
  \textup{at $e=1$, $m=1$, $f\ge3$: at least one equal side carries an }a\textup{-edge,}
\]
so no tiling has $p=0$ on both sides, and a single symmetric $p$ satisfies $p\ge1$.
Formalised as \texttt{Erdos634.WallsCircular.some\_side\_carries\_a} and
\texttt{symmetric\_p\_zero\_false}.

What survives as the open step is the \emph{cascade}, not the parameter: whether the chain from such
an $a$-edge terminates in a collision, which is Conjecture~\ref{conj:advance}. Its premise,
``suppose an equal side carries an $a$-edge'', is now discharged rather than hypothetical --- it
holds in every tiling of every member. In the language of the crossing statement, $\ell$, the
initial $c$-block of the side, is exactly the level at which the corner block breaks, and the open
inequality $\ell\ge b_p-1$ is a race between where the block gives out and where the base $b$ sits.
It was never the claim that the block does not give out.
\end{remark}

\begin{lemma}[First-run orientation]\label{lem:firstrun}\lab{CONJECTURE}
...
\begin{lemma}[Top junction; $p\le f-3$ for $f\ge4$]\label{lem:topjunction}\lab{CONJECTURE}
...
\begin{lemma}[c-chord dichotomy]\label{lem:cchord}\lab{VERIFIED}
...
\begin{theorem}[Hypothesis~\ref{hyp:walls} holds at $(1,3)$]\label{thm:walls13}\lab{CONJECTURE}
...
$(1,2)$ (Lemma~\ref{lem:ple}) and $(1,3)$; $f=4$ is settled by Theorem~\ref{thm:walls14}, so on the
thin family the hypothesis is open for $f\ge5$, where $p\le f-3$, and it is open for $e\ge2$
generally --- see Conjecture~\ref{conj:advance} for the thin case and \S\ref{sub:apex} for what is
known at $e\ge2$.
\end{remark}
...
\begin{proposition}[The double-$c$ kill at any initial block length]\label{prop:doublec}\lab{PROVED}
...
\section{Route 1's chain, formalised}\label{sec:routeone}

Conjecture~\ref{conj:advance}'s remaining gap is route 1, reduced by
Remark~\ref{rem:route1uniform} to the question of what covers the segment $[V,E]$ of length exactly
$a$. This section records the chain that answers it, together with the two inputs that remain
unproved. Every statement below is machine-checked in \texttt{lean/Erdos634/RouteOne.lean} unless
marked otherwise.

\begin{proposition}[The flank at $V$ lies along the line]\label{prop:flankline}\lab{VERIFIED}
Let $T$ be a tile with vertex $V$ whose two edge directions at $V$ point weakly upward, and suppose
$T$ contains points approaching $V$ tangentially from the right --- for every $\delta>0$ a point $q$
of $T$ with $q-V=(t,u)$, $t>0$ and $u\le\delta t$. Then one of the two edges at $V$ is horizontal
and points rightward (\texttt{flank\_along\_line}, \texttt{flank\_along\_line'}).

The proof decomposes $q-V$ barycentrically at $V$, where the coefficient of $V$ itself drops out
(\texttt{sub\_vertex\_eq\_combo}) and the other two are nonnegative on the carrier; the cone
statement \texttt{cone\_forces\_horizontal} then forces a horizontal flank. The hypothesis that the
two directions are not \emph{both} horizontal is not needed as an assumption: their cross product is
$\det T\neq0$ (\texttt{not\_both\_horizontal}).
\end{proposition}

\begin{proposition}[The overshoot trichotomy]\label{prop:overshoot}\lab{VERIFIED}
The covering of $[V,E]$ therefore begins with a tile edge from $V$ along the line, of length $a$, $b$
or $c$. Since $a<b<c$, a $b$- or $c$-edge contains $E$ strictly in its interior
(\texttt{overshoot\_dichotomy}, \texttt{E\_interior\_of\_long}); that is precisely the
tile-interior blocking whose failure defined the escape, so those branches die. Only the $a$-edge
survives, and it makes $E$ a junction one $a$-length along.
\end{proposition}

\begin{proposition}[The advance is a descent]\label{prop:descent}\lab{VERIFIED}
Each advance consumes a distinct $a$-edge of the wall, since successive positions differ by $n\,a$
(\texttt{advance\_injective}); hence the number of advances is bounded by the number of the wall's
$a$-edges (\texttt{advance\_count\_le\_run}), and the escape at step $L+1$ is unreachable when the
wall carries $L$ such edges (\texttt{terminus\_of\_run\_length}). This is a combinatorial terminus,
independent of the wall's metric length. The descent itself is \texttt{wall\_descent}, and
\texttt{route\_one\_closes'} chains it: with the trichotomy regenerating and the terminus blocked, no
escape exists at any distance from the exit.
\end{proposition}

\begin{proposition}[The serving tile is an upper tile]\label{prop:serving}\lab{VERIFIED}
Take the approach points to $V$ \emph{strictly above} the wall. Each is covered, and a tile
containing a point of positive height cannot be one whose carrier lies weakly below $V$; so the
tile the pigeonhole selects is not the straight tile beneath the wall
(\texttt{not\_below\_of\_contains\_above}, \texttt{serving\_tile\_is\_upper}). With that,
\texttt{route\_one\_flank\_identified} states the flank conclusion for a named tile of the
dissection at a named vertex: the serving tile has a horizontal rightward edge at $V$.
\end{proposition}

\begin{proposition}[The serving tile has $V$ as a vertex]\label{prop:servingvertex}\lab{VERIFIED}
A tile's local angle at $V$ is a corner angle, $2\pi$, $\pi$ or $0$
(\texttt{PinPlumbing.localAngle\_cases}). For the serving tile the last three are excluded: $0$
because it contains $V$, $2\pi$ by the figure's $u=0$, and $\pi$ because the $\pi$-count at $V$ is
exactly one and the tile below carries it, so no other tile can
(\texttt{not\_straight\_of\_unique}). Hence $V$ is one of its vertices
(\texttt{serving\_has\_vertex}), and \texttt{route\_one\_flank\_composed} states the flank
conclusion with that hypothesis discharged.
\end{proposition}

\begin{remark}[What route 1 still assumes]\label{rem:routeoneopen}\lab{OPEN}
The chain of Propositions~\ref{prop:flankline}--\ref{prop:servingvertex} is complete as a chain: from
a wall, an interior point $V$ on it, a tile below carrying the straight angle, and approach points
strictly above, it derives that the covering of $[V,E]$ begins with a tile edge from $V$, that a $b$-
or $c$-edge dies by tile-interior blocking, and that the surviving $a$-advance descends to a
terminus.

What it does not do is \emph{exhibit} that data in a hypothetical tiling of the base-$\beta$ target.
Each theorem takes the configuration as hypotheses --- the wall, the below tile, the interiority of
$V$, the figure's counts at $V$ --- and the escape of Remark~\ref{rem:route1uniform} must be shown
to present them. That is the remaining obligation: not a step of the argument, but the argument's
attachment to the object it is about. Route 1 is therefore a proof scheme, machine-checked, awaiting
its instance.
\end{remark}

\begin{conjecture}[Advance-and-collide]\label{conj:advance}\lab{CONJECTURE, proved at $f=2,3,4$}
...
The second gap is closed: Proposition~\ref{prop:doublec} kills the double-$c$ configuration at
every initial block length $j\ge2$ and every $f\ge3$, with no per-word enumeration
(Remark~\ref{rem:doublecscope}). What stands between this conjecture and
Hypothesis~\ref{hyp:walls} at $e=1$ is therefore the first gap alone, and
Remark~\ref{rem:route1uniform} reduces it to a single question: what covers the segment $[V,E]$ of
length exactly $a$. Two of that question's three cases are now closed at the vertex figure: some
tile lays an edge from $V$ rightward along the line, of length $a$, $b$ or $c$, and since $b$ and
$c$ both exceed $a$, either choice places $E$ strictly interior to that edge --- which is precisely
the tile-interior blocking whose failure defined the escape, so the chord dichotomy applies and the
branch dies (\texttt{RouteOne.overshoot\_dichotomy}, \texttt{.E\_interior\_of\_long}). Only the
$a$-edge case advances, making $E$ a junction one $a$-length along: the march, on an interior wall.
The figure at $V$ is pinned by \texttt{RouteOne.alpha\_wall\_figure\_real}: an interior point
carrying an $\alpha$ (the second side tile's corner at $V$ faces $AB=a$) and a straight angle has
the figure $\{\pi,3\alpha,2\beta\}$ or $\{\pi,\alpha,\beta,\gamma\}$, no second straight
angle and no covering tile. Unproved: that a through-edge, rather than a junction, runs below the
line at $V$, and that the exactly-filled wedge's last flank lies along the line --- the two edges on
which the $[V,E]$ question now sits.
\end{conjecture}

\begin{theorem}[Hypothesis~\ref{hyp:walls} holds at $(1,4)$]\label{thm:walls14}\lab{CONJECTURE}
...
\begin{theorem}[The cascade closes every initial-block-1 configuration, for all $f$]\label{thm:e1cascade}\lab{CONJECTURE}
...
\begin{lemma}[The $jb$-line partition]\label{lem:jbline}\lab{VERIFIED}
...
\begin{lemma}[The east fan at the fork is forced]\label{lem:eastfan}\lab{PROVED}
...
\begin{theorem}[The row fork kill]\label{thm:forkkill}\lab{PROVED}
...
\begin{proposition}[The $A_2$ branch dies]\label{prop:a2branch}\lab{PROVED}
...
\begin{lemma}[The wall climb]\label{lem:wallclimb}\lab{PROVED}
...
\begin{theorem}[L2 at every reached slot]\label{thm:l2slot}\lab{PROVED}
...
\begin{theorem}[The block-two chain runs to arbitrary depth]\label{thm:elltwo}\lab{PROVED}
...
\begin{theorem}[Reach three behind a thick block, and the pincer window]\label{thm:depthwindow}\lab{PROVED}
...
\begin{corollary}\label{cor:walls15}\lab{PROVED via the certified exhaustion \texttt{w74}; the argument-only route inherits Theorem~\ref{thm:e1cascade}'s effective label}
...
the first member closed with no engine certificate under the chain's own hypotheses; the member is
independently certified by the exhaustion \texttt{w74} ($132\,519$ nodes), so its exclusion does
not rest on the chain.
\end{corollary}

\begin{lemma}[The apex]\label{lem:apex}\lab{PROVED: the two cited cores VERIFIED}
...
\begin{lemma}[Descent identities and the ladder]\label{lem:ladder}\lab{PROVED}
...
\begin{lemma}[Termination is one condition, boundary or interior]\label{lem:termination}\lab{PROVED}
...
\begin{theorem}[Straddlers need three strips, so rows $1,2,3$ are clean]\label{thm:strippbound}\lab{CONJECTURE}
...
What the repair still owes is now smaller than it was. The proposition's Lean skeleton carried two
geometric hypotheses about the descent --- that the feet reach the base, and that each lands on an
$a$-junction --- and the second is close to the conclusion being sought. It is removable. The south
cover is $f$ whole $a$-edges, so its $f+1$ endpoints span a length $f\cdot a=f^2$; they descend to
$f+1$ distinct base points, which are base-word junctions since no vertex of a tile lies interior to
a boundary edge; and they therefore carve a run of $k\ge f$ whole letters of total length $f^2$.
The three edge lengths then decide the run by themselves: a single $c$ would exhaust $f^2$ in one
letter, leaving $k=1<f$, and a $b$ would leave a remainder of $1$, below the shortest letter, so
every letter is an $a$ and $k=f$ exactly. That is $f$ consecutive $a$-letters, which the base word
cannot carry, its $f$ letters $a$ forming one block while its first and last slots are both $a$.
So the descent needs only to \emph{land}; what it lands on is forced arithmetically
(\texttt{A2BranchRow3.span\_all\_a}, \texttt{row\_three\_dies\_of\_span}, axiom-clean).
\end{theorem}

\begin{corollary}\label{cor:walls16}\lab{CONJECTURE: effective, resting on reach $4$, which is open}
...
The $(1,5)$, $(1,6)$ and $(1,7)$ cases are additionally certified by independent exhaustions of
the full $m=1$ targets ($132{,}519$, $1{,}251{,}382$ and $13{,}729{,}561$ nodes, instances
\texttt{w74}, \texttt{w107}, \texttt{w146}). These are exhaustion certificates for the targets
themselves and are labelled as computations; they are not a reading of
Hypothesis~\ref{hyp:walls} at those members, which at $e=1$ would require the arrangement check
of Remark~\ref{rem:whyhyp}.
\end{corollary}

\begin{remark}[The window is mirror-symmetric, and what that leaves at $N=191$]\label{rem:mirrorwindow}
The target is isosceles, so reflecting it swaps the two base corners and reverses the base word:
position $i$ goes to $f+3-i$, and a configuration $(bp,cp)$ to $(f+3-bp,\,f+3-cp)$. The admissible
box $3\le bp\le f$, $2\le cp\le f+1$ is carried to itself. The four kills are stated
asymmetrically --- direct-$b$ from the $b$-first corner, L2 from the $c$-first corner --- but the
reflection swaps those corners, and it permutes the kills in two transpositions, exchanging the
first with the second and the third with the fourth (\texttt{PincerLadder.kill\_mirror},
\texttt{omega}, axiom-clean). So the escape set is closed under the reflection and its members come
in mirror pairs.

That halves the residue. Enumerating the admissible box against the \emph{proved} reach $3$
($R=4$; reach $4$ is open at \texttt{prop:a2branch} row $3$) gives
\[
\begin{array}{rrrrr}
f & N & \text{words} & \text{escapes} & \text{up to mirror}\\\hline
5 & 74 & 12 & 0 & 0\\
6 & 107 & 20 & 2 & 1\\
7 & 146 & 30 & 6 & 3\\
8 & 191 & 42 & 14 & 7
\end{array}
\]
with representatives at $f=8$ of $(4,2),(5,2),(5,3),(5,6),(5,7),(5,8),(5,9)$, and at $f=7$ of
$(4,2),(5,2),(5,3)$. So $N=191$, the smallest unsettled prime, stands seven configurations from an
argument-only exclusion at today's reach, and three at reach $4$ --- not forty-two. Generated by
\texttt{code/analysis/base\_word\_residue.py}, which reproduces the ladder independently.

The count is a measurement, not a route: Corollary~\ref{cor:walls16} derives $f\le6$ from reach
$4$, which is open, so at $e=1$ that corollary is conditional on it; the members $f\le7$ are settled
regardless by the certified exhaustions quoted there. And by Remark~\ref{rem:pnotzero} the phrase
``Hypothesis~\ref{hyp:walls} holds at $(1,f)$'' should be read at $e=1$ as ``the member is
excluded'', those being equivalent there --- it is not evidence for the hypothesis as a structural
fact.
\end{remark}

\begin{remark}[The reach ladder, uniform in the reach]\label{rem:ladder}
Theorem~\ref{thm:depthwindow} and Corollary~\ref{cor:walls16} instantiate one statement twice.
If the four kills cover depth $\le R$, then no admissible $(bp,cp)$ escapes for any $f\le R+1$;
since reach $r$ makes the kills cover depth $\le r+1$, this reads
\[
  \text{reach } r \;\Longrightarrow\; f\le r+2 .
\]
Reach $3$ gives $R=4$ and $f\le5$, reach $4$ gives $R=5$ and $f\le6$, recovering both statements
(\texttt{pincer\_ladder}, with \texttt{ladder\_reach\_three} and \texttt{ladder\_reach\_four} as
its instances). The arithmetic half of the ladder is therefore settled at every height at once:
extending Corollary~\ref{cor:walls16} past $f=6$ requires only the geometric reach step, and no
further case analysis in $(bp,cp)$ at any $f$.

The bound is sharp. At $f=R+2$ the configuration $(bp,cp)=(4,2)$ escapes all four kills
(\texttt{pincer\_sharp}), and enumerating the escapes at that $f$ gives exactly $(4,2)$ and
$(f-1,f+1)$ and nothing else (\texttt{first\_failure\_escapes}).

Those are not two families. They are one configuration seen from the two base corners:
$(f-1,f+1)$ is the mirror $(f+3-4,\,f+3-2)$ of $(4,2)$, and by Remark~\ref{rem:mirrorwindow} the
kills are invariant under that reflection. An earlier version of this remark concluded that
excluding $(4,2)$ alone ``changes nothing''; on the contrary, a proof that $(4,2)$ cannot occur,
stated generally in $f$, transports to its mirror by reflecting the tiling, and so advances the
bound by the full step. After the gain two further orbits appear, $(5,2)$ and $(5,3)$ with their
mirrors. What survives is that each $f$ costs its own reach step; what does not is the count --- the
step is a single configuration.

That makes the route to $N=191$ a named target rather than a programme. It needs the ladder driven
from $R=4$ to $R=7$, and each step is the exclusion of the base word $a\,c\,a\,b\,a^{f-2}$,
which is what $(bp,cp)=(4,2)$ spells.
\end{remark}

\begin{remark}[The $(4,2)$ configuration has nowhere for the descent to land]\label{rem:word42}
That target is smaller than it looks. The word $(bp,cp)=(4,2)$ spells is
\[
  a\,c\,a\,b\,a^{f-2},
\]
$f+2$ letters with exactly $f$ of them $a$, and its $a$-runs have lengths $1$, $1$ and $f-2$. The
longest is $f-2<f$, so the word carries \emph{no} block of $f$ consecutive $a$-letters
(\texttt{A2BranchRow3.word42\_no\_a\_block}).

Now recall the span dichotomy of Theorem~\ref{thm:strippbound}'s repair: a base run of $k\ge f$
whole letters of total length $f^2$ must consist of $f$ consecutive $a$s, those and a single $c$
being the only ways to make $f^2$ from $\{f,\,f^2-1,\,f^2\}$. This word has no such run; and a
descent landing on the single $c$ instead would put its $f-1$ interior feet interior to a boundary
edge, which no tiling vertex may be. In this configuration the south cover therefore has
\emph{nowhere to land at all} (\texttt{word42\_no\_landing}, axiom-clean).

So $(4,2)$ dies the moment the descent is known to reach the base, and needs nothing else: no reach
step, no fan analysis, no case split on the row. With Remark~\ref{rem:mirrorwindow} supplying the
mirror and Remark~\ref{rem:ladder} the ladder, discharging the single hypothesis ``the feet reach
the base'' advances the bound by one level. Three such advances, $R=4$ to $R=7$, reach $N=191$.

The three open steps of the prime hole's smallest member are thus one step applied three times, and
that step is now carried in its weakest form --- the descent must land, and nothing is asked about
what it lands on.
\end{remark}

\subsection{The $(4,2)$ word dies by an $\alpha$-fan: a quantitative law}\label{sub:fanlaw}

The refutation of the $(4,2)$ base word was mined from full traces of the seeded engine
(placements and prune sites, node-identical to the untraced runs) at $f=4,\dots,7$, and its
structure is uniform in $f$. At the base junction between the word's first $a$ and its $c$, the
west corner tile presents $\gamma$ and the $c$-tile presents $\alpha$, leaving a wedge of exactly
$\pi-\gamma-\alpha=\beta$. The growing part of the tree is a fan of $\alpha$-tiles pinned at that
junction, each pivoting on the previous tile's $b$-edge; the tree's binary choices are chiralities
that do not alter the angle, which is why every branch terminates identically. After
$k=\lfloor\beta/\alpha\rfloor$ fan steps the remainder $\beta-k\alpha$ is a corner smaller than
every tile angle, and the branch dies.

\begin{proposition}[The fan law]\label{prop:fanlaw}\lab{HEURISTIC}
For the $(4,2)$-seeded instance at $(1,f)$, the engine's refutation consists of two such fans
(behind the two chiralities of the $c$-tile, the second four levels deeper), a set of local side
cases whose number is independent of $f$, and one secondary chain that advances one block per unit
of $f$ along the trailing $a$-run of the base word. The fan depth is exactly
$\lfloor\beta/\alpha\rfloor$, with $\beta/\alpha=\pi/(2\alpha)-3/2$.
\end{proposition}

The law is verified at seven consecutive members:
\[
\begin{array}{r|ccccccc}
f & 4 & 5 & 6 & 7 & 8 & 9 & 10\\\hline
\lfloor\beta/\alpha\rfloor & 4 & 6 & 7 & 9 & 11 & 12 & 14\\
\text{measured fan depth} & 4 & 6 & 7 & 9 & 11 & 12 & 14\\
\text{nodes} & 139 & 517 & 1025 & 4019 & 15953 & 31873 & 127183
\end{array}
\]
the last two columns as pre-registered predictions scored out of sample (research protocol: the
$f=10$ row was frozen, with the exact leaf count $2^{15}$ and depth $20$, before that run
returned; all five registered quantities matched). The side-case counts are constant --- the
traces at $f=4$ and $f=5$ agree leaf for leaf on all $21$ of them --- and the secondary chain is
the march along the base word, its side branches dying by the overrun identity $c-b=1$, a leftover
of length $1$ lying below every generator of $\langle f,f^2-1,f^2\rangle$. Instrumenting the
semigroup prune to print its failing run settles the side-case arithmetic completely: at
$f=4,\dots,7$ all seven $f$-independent semigroup kills fire on the run $f^2-f-1=b-a$ and every
chain kill on the run $1=c-b$, and both lengths were already formalized as gaps
(\texttt{east\_cover\_gap}; \texttt{FanKill.one\_is\_gap}, packaged as
\texttt{two\_gap\_contract}). The $P2$ layer of the refutation therefore consumes exactly two
verified identities.

The step itself is also closed arithmetically (\texttt{lean/Erdos634/FanStep.lean}, axiom-clean):
in the remaining wedge after $k\ge1$ steps, a tile at the pin presents $\alpha$, $\beta$ or
$\gamma$, or passes an edge through it; $\gamma$ and the through-edge never fit, $\beta$ fits
only at $k=0$ where it dies on the two gaps, and the $\alpha$-step advances by exactly $\alpha$
with both chirality stubs equal to the same gap $c-b=1$ (\texttt{step\_exhaustion},
\texttt{stub\_is\_gap}). The termination arithmetic is formalized in
\texttt{lean/Erdos634/FanKill.lean} (axiom-clean):
$k\alpha=\beta$ is impossible for every $k$, since it would force $\alpha=\pi/(3+2k)$ against the
irrationality of $\alpha/\pi$, which holds at every member; the remainder at
$k=\lfloor\beta/\alpha\rfloor$ is trapped strictly in $(0,\alpha)$; and nothing whose angle is at
least the minimal tile angle fits in it. What remains outside arithmetic has shrunk to two instantiations of theorems the corpus already
holds in full strength: \texttt{Dissection.wall\_partition} (a straight interior wall segment is
measure-partitioned by the tile edges along it) at the pin's flanking rays, and
\texttt{Dissection.vertex\_multiplicities} (the angle arithmetic at a vertex) at the pin itself
--- together with the completeness of the candidate enumeration, which is the engine's certified
soundness. There is, we must record, \emph{no} assembled conditional statement: an earlier draft carried one
and adversarial review found it vacuous (its hypothesis negated the multiplicity system, which is
satisfiable, rather than the geometric completion), so it has been withdrawn. What is established
is the arithmetic --- the only exact fill of the $\beta$-wedge is a single $\beta$-tile, which
dies on the two-gap contract, and every non-exact fill stalls at the sub-$\alpha$ fan remainder,
which no tile angle fits --- together with the sketch that composes them. The composition itself
is not formalized, and three obligations it needs are unbuilt, which we name rather than leave
implicit.

\begin{remark}[Where the pin configuration is expensive, and what it forces]\label{rem:pinbuffer}\lab{PROVED}
The base words that resist exhaustive search are sharply identified: those in which a
corner-anchored $a$-run meets an $a\mid c$ junction whose $c$ is immediately followed by $b$. At
$f=12$ every orbit of another shape exhausts within a few thousand nodes, while these do not, and
the pattern was confirmed on a prediction made before the runs.

The configuration forces more than the search sees. At the junction the west $a$-tile presents
$\gamma$ and the $c$-tile $\beta$, leaving a wedge of exactly $\alpha$ filled by a single
$\alpha$-tile $X$ (\texttt{PinLemma.pin\_forces\_single\_alpha}). $X$'s flanks are $b$ and $c$, but the rays it
lies along are the $c$-tile's $a$-edge, of length only $a$, and the west tile's $b$-edge. Its flank
along the short ray therefore \emph{overflows} that tile's apex, by $L-a$ with $L\in\{b,c\}$, and
the two cases are decided arithmetically: $b-a$ is a semigroup gap, so the $b$-chirality admits no
whole-edge covering beyond the apex, while $c-a=f^{2}-f=(f-1)a$ is exactly $f-1$ $a$-edges, and the
remaining flank $b$ is then flush with the west tile's $b$-edge. One chirality dies algebraically;
the other is rigidly pinned with an overflow that is an exact multiple of the shortest edge
(\texttt{PinBuffer.buffer\_overflow\_b\_is\_gap}, \texttt{buffer\_dichotomy}).

The surviving chirality lands in machinery already in place. At the apex $A$ the $c$-tile shows
$\gamma$ --- its angles are $\beta$ at the junction and $\alpha$ at the far base end --- while
$X$'s $c$-edge passes straight through, contributing $\pi$; the pocket that remains is
$2\pi-\gamma-\pi=\alpha+\beta$, whose figure is unique, one $\alpha$ and one $\beta$, and no
further edge passes through $A$. That is exactly the $\alpha+\beta$ pocket of the $T$-vertex
ladder, so the hard configuration reduces to the ladder's termination --- the same residue this
programme has carried throughout --- rather than to new geometry
(\texttt{pocket\_at\_apex}, \texttt{pocket\_at\_apex\_figure}).

The runway is moreover crystalline. The overflow has length exactly $c-a=(f-1)a$, and both longer
edges exceed it, so any covering confined to the runway is exactly $f-1$ $a$-edges: no choice of
edge type arises, and with the orientation propagating from the pocket the run carries no branching
at all (\texttt{runway\_forces\_a\_edges}). What this does not yield is a contradiction at the
far end. If the run is confined, the terminal vertex carries $X$'s $\beta$ against the last tile's
$\gamma$, leaving $4\alpha+2\beta$ with three admissible figures; and the run need not be
confined, since a $b$ laid from the runway's start overruns the terminal vertex by $f-1$ and a $c$
by $f$ (\texttt{overrun\_amounts}). The first of those is strictly below the minimal edge, which invites the thought that it is
therefore impossible; it is not. That remainder is part of the covering edge rather than a segment
awaiting coverage, and at the terminal vertex exactly one tile has the point interior to an edge
while $X$ has a vertex there, so the three-tile exclusion does not apply and the budget is
satisfied, $\beta+\pi$ leaving $3\alpha+\beta$ with two admissible figures. What does follow is
weaker and one step further out: past that vertex the edge continues for only $f-1$ while every
tile edge is at least $a=f$, so the first tile of the wedge must overrun its far endpoint
(\texttt{sub\_a\_forces\_overrun}). The branch is displaced, not closed, and the $c$-overrun
leads to the same wedge with remainder exactly one $a$-edge
(\texttt{c\_overrun\_is\_flush}).

The displacement does not reach the boundary. It is tempting to identify the far endpoint of the
overrunning edge with the $b\mid a$ junction of the base word, which would make the configuration
funnel into the rigid $a$-run; but that conflates the $b$-tile of the base with the covering
$b$-edge laid along $X$'s $c$-edge, and the latter lives on the ray from the pin at angle $\beta$.
At $f=12$ its far endpoint sits at height $153.79$, some $308.70$ from the $b\mid a$ junction. The
residue therefore recurses at another interior point, with no boundary anchor --- which is the
character of the crossing question, and the reason the $V_1$ repair, which worked precisely because
both of its anchors lay on the boundary, has no analogue here.

The recursion is nevertheless collinear. Every overrunning edge lies along $X$'s $c$-edge, so the
successive endpoints march along one fixed ray from the pin, in steps of $f-1$ or $a$. Since the
$c$-tile presents $\beta$ at the pin, that ray is parallel to the target's left side and leaves
through the right side, at distance $T = f^{3}(N_0 - pf)/N_0$ with $N_0 = 3f^{2}-1$ and the pin at
$J = pf$. This is never an integer: $\gcd(f,N_0)=1$, so $N_0 \mid f^{3}(N_0-pf)$ would force
$N_0 \mid p$, while $1\le p\le f-1<N_0$ (\texttt{PinRay.exit\_not\_integral}). Tile edges have
integer lengths, so if the chain of edges along the ray ever reaches the boundary its length would
have to be an integer, and the configuration dies at once. What is not available is the chain's
propagation, and this is not merely unproved but false in general. One might argue that at each
endpoint the angle budget forbids a second through-edge, so the wedge beyond it must be bounded by
the ray and its tile must lay an edge along the ray; but the wedge's tile may instead have a
\emph{vertex} at that point with the ray passing strictly inside its angle, whereupon the ray runs
through that tile's interior and the chain simply stops. Walking every edge-ray from the base of
our certified $44$-tiling, nine of fifteen die in the interior, one of them after a single edge. So
the non-integrality of $T$ is a genuine obstruction only for configurations in which propagation
can be forced separately --- and that forcing is the crossing question itself.
\end{remark}

\begin{remark}[The unbuilt obligations of the pin programme]\label{rem:pingaps}\lab{OPEN}
\emph{(a) The model identification.} The dissection layer's output is phrased in the corner
angles of a model triangle. Nothing in the formal development identifies those with
$\alpha,\beta,\gamma=2\alpha+\beta$ satisfying $3\alpha+2\beta=\pi$ with $\alpha/\pi$
irrational, for the tile of sides $(f,f^2-1,f^2)$; that identification is the precondition for
feeding the multiplicity theorems, and it is assumed at every point of use.
\emph{(b) The units bridge.} The run equation concludes in extended-real distances, while the
semigroup gap lemmas consume natural-number equations over $\langle f,f^2-1,f^2\rangle$. No
theorem converts between them; doing so requires pinning the tiling's absolute scale.
\emph{(c) The combinatorics--geometry bridge.} The orientation and flank machinery
(\texttt{Interface}, \texttt{Inflation}, \texttt{RunOrientation}, \texttt{MarchAssembly}) is
self-contained finite combinatorics that never mentions a dissection. Its theorems therefore
constrain no tiling until a functor from tilings to orientation words is built, and the seed
hypothesis --- that the run begins at a corner tile presenting $\beta$ --- is likewise an
assumption at the combinatorial level rather than a consequence of the corner theory.
These three, not bookkeeping, are what separate the sketch from a theorem.

Of these, (c)'s mathematical content is now discharged, though its bookkeeping is not. The whole
combinatorial layer rests on one forbidden transition --- two $\gamma$s cannot meet at a junction
--- and that is a theorem about actual dissections: at a boundary junction the tiles' local angles
sum to $\pi$, while $2\gamma=\pi+\alpha$ exceeds it
(\texttt{OrientBridge.no\_two\_gamma\_at\_boundary\_junction}). The same file records why the
constraint is a boundary phenomenon: at an interior point the budget is $2\pi$ and $2\gamma<2\pi$,
which is exactly the reason the corner-chain induction fails at interior $V_k$. The obligation is discharged in indexed form too: given the two readings the indexing must supply
--- that a `BG` tile meets the junction at its east end and a `GB` tile at its west --- consecutive
base tiles satisfy the admissibility relation of the combinatorial layer
(\texttt{adjacent\_admissible}). The reading itself is now a definition rather than an assumption: a tile is called $BG$ when it
presents $\beta$ at the west end of its base edge, and given only that the edge's two ends carry
$\beta$ and $\gamma$ in some order, showing $\beta$ west is equivalent to showing $\gamma$ east
(\texttt{tileOrient}, \texttt{orient\_reading}), which delivers both hypotheses of
\texttt{adjacent\_admissible}. What remains of (c) is therefore two things and no more: the
\emph{ordering} --- \texttt{lineChain} is a finite set, and turning it into a list requires
sorting the chain along the base together with a contiguity fact, a measure identity saying
nothing about gaps on its own. That fact is now available: the chain edges are \emph{closed}, so
their union is closed, and a closed subset of an interval carrying the interval's whole measure is
the interval --- its complement is open and null, while a nonempty open subset of $\mathbb R$ has
positive measure (\texttt{Contiguity.closed\_full\_measure\_eq},
\texttt{no\_gap\_between}). What is left of the ordering is therefore the sort itself, and the two facts it rests on are also
in hand: the sort key is injective on the chain, since two non-degenerate edges sharing a left
endpoint would have overlapping interiors (\texttt{distinct\_left\_endpoints}), and consecutive
keys leave no gap, since a point between them would be covered by neither while full coverage says
it must be covered (\texttt{gap\_forces\_meet}). The sort is now built: the chain's positions form a finite set of reals, and sorting them yields
a strictly increasing list with one entry per chain edge, every edge represented
(\texttt{sortedPositions} and its three lemmas). Reading the tile back is done too: every sorted position comes from a chain edge, and by
injectivity from exactly one, so mapping back recovers the chain in base order with one entry per
edge (\texttt{orderedChain} and its lemmas). What remains of bridge (c) is therefore the input
that an
$a$-edge's two ends carry $\beta$ and $\gamma$. Its arithmetic is now in place: if a tile's three
vertex angles are $\alpha,\beta,\gamma$ as a multiset and the vertex opposite the edge carries
$\alpha$, then --- the tile being scalene, so that its angles are distinct --- each endpoint
carries $\beta$ or $\gamma$ (\texttt{endpoints\_avoid\_alpha}, with
\texttt{sides\_strict} supplying $f<f^2-1<f^2$). The side--angle correspondence, which places the $a$-edge opposite $\alpha$, is proved as well.
It is not in Mathlib in this form, but follows from the law of cosines by one identity,
$2abc(\cos A-\cos B)=(b-a)\bigl((a+b)^2-c^2\bigr)$, whose right side is positive when $a<b$ and
the triangle inequality holds; cosine being strictly decreasing on $[0,\pi]$, the angle opposite
the shorter side is the smaller (\texttt{cos\_gt\_of\_side\_lt},
\texttt{smallest\_angle\_opposite\_shortest\_side}).

Every mathematical fact bridge (c) requires is therefore proved. What is not yet done is the
\emph{instantiation}: defining the position key on an actual line chain, deriving its injectivity
from the tiles' disjointness, and deriving the coverage hypothesis of the no-gap lemma from the
wall partition. That is assembly against the geometric objects rather than new mathematics, but it
is not trivial, and we state it as outstanding rather than as a formality --- an earlier draft of
this programme described a comparable step as bookkeeping and was wrong. No further geometric obstacle stands in the way, but until those two are supplied the
combinatorial results still do not apply to tilings.
\end{remark}

In fact the fan induction is not needed for the mathematics at all. Since the angle sums are a
theorem of the dissection layer (\texttt{sum\_localAngle\_eq}, the discharged $G2$), a
\emph{completed} tiling has tile angles summing to exactly $\pi$ at every base junction; between a
$\gamma$-flank and an $\alpha$-flank the interior multiset must sum to exactly $\beta$, and the
multiplicity system forces it to be a single $\beta$-tile
(\texttt{PinLemma.pin\_forces\_single\_beta}, via \texttt{vertex\_multiplicities}); a tile
holding the junction inside one of its edges is excluded outright, contributing $\pi$ against two
positive flanks (\texttt{no\_through\_tile}); and the forced $\beta$-tile dies on the two gaps
(\texttt{beta\_tile\_dies}). The engine's fans are what this impossibility looks like to a
solver: partial states advancing by $\alpha$ that can never close, their depth
$\lfloor\beta/\alpha\rfloor$ being the law measured above. The argument applies to the direct
orientation of the $c$-tile in \emph{every} word with $c$ in the second position, $(3,2)$
included; what remains per word is the mirrored orientation, where the forced fill is a single
$\alpha$-tile (\texttt{pin\_forces\_single\_alpha}) and the configuration advances to the next
junction --- the head of the engine's second cascade. The instantiations themselves have begun landing: the pin equation --- at any boundary point of
the target that is not a target vertex, the tiles' local angles sum to exactly $\pi$ --- is now a
two-line composition of the dissection layer's theorems
(\texttt{PinPlumbing.pin\_angle\_sum}); the per-tile case split of the local angle is proved
from the definition (\texttt{localAngle\_cases}); a tile covering the pin in its interior is
excluded (\texttt{no\_interior\_tile\_at\_pin}); and the flank identification is reduced to a
combinatorial statement about which corners meet which sides (\texttt{west\_flank\_gamma},
\texttt{east\_flank\_cases}) --- reduced, not established: what these prove is that \emph{given}
a west flank that is not $\beta$ it is $\gamma$, and that a $c$-side end is $\alpha$ or $\beta$.
Nothing in the formal development establishes the antecedent at a junction of an actual tiling. The congruence layer is
closed as well: congruent tiles have equal corner angles at matched vertices, by the law of cosines
from the matched distances (\texttt{angle\_of\_sss}, \texttt{congruent\_corner\_angles}), and
matching edge lengths (\texttt{congruent\_edge\_lengths}) --- so every tile of a congruent
dissection contributes one of $\{\alpha,\beta,\gamma\}$ at any vertex and one of $\{a,b,c\}$
to any wall run, which is what the multiplicity and semigroup arguments consume. The right half of that conversion is done: a wall segment's edge measures sum to its length
(\texttt{wall\_partition\_length}, via Mathlib's \texttt{hausdorffMeasure\_segment}); the left
half is now closed as well: a nonempty convex compact subset of a segment is itself a segment
(\texttt{convex\_compact\_in\_segment}, by pulling back along the line's parametrization and
applying the connected-compact classification on $\mathbb R$), so each edge-segment intersection
term of the partition is the distance between two points
(\texttt{inter\_measure\_is\_dist}). The run equation itself is now delivered: for any dissection whose tiles are congruent to a
model, a wall segment whose chain edges lie wholly inside it has length
$x\,A+y\,B+z\,C$ in the model's three edge lengths
(\texttt{wall\_run\_equation}, assembling the partition, the segment classification,
\texttt{congruent\_edge\_lengths} and a counting lemma) --- precisely the hypothesis form the
semigroup gap kills consume. The partial-edge variant, for runs met by a `T`-vertex from the far
side, is the remaining case of the conversion. The ladder's invariant is now measured in exact
form: every stub has length exactly $1$, the stub directions are $\beta+k\alpha$, turning
$\alpha$ per block, and each block's $c$-edge carries the previous structure's tip as a
$T$-vertex at parameter exactly $b$ --- one pattern at every block and every member probed. The
escape in which a covering edge sails past an apex is excluded by theorem: at an interior point
three distinct tiles cannot each hold the point interior to an edge, since they would contribute
$3\pi$ against the interior budget of $2\pi$
(\texttt{at\_most\_two\_through}), so transversal edge crossings are impossible and an edge
along the stub line cannot cross the next block's $c$-edge. The invariant is verified exactly: in $\mathbb Q(\sqrt D)$ arithmetic over the engine's exact
coordinates, $\mathrm{apex}=\mathrm{pin}+(b/c)(\mathrm{tip}-\mathrm{pin})$ holds identically at
both members traced, and its algebra is formalized
(\texttt{flush\_overrun\_mem}, \texttt{flush\_split\_lengths}: the apex lies on the $c$-edge
at parameter $b/c$ and the split lengths are exactly $b$ and $c-b$). The termination therefore
reduces to the invariant induction: when the trailing $a$-letters are exhausted, no continuation
tile exists, the final stub must be covered by whole edges, and $x\,a+y\,b+z\,c=1$ is the gap.
Every step ingredient of that induction is now machine-checked, including the junction
catalogue: the march visits exactly two kinds of base junction, and every flank combination's
forced fill is a proved multiplicity statement --- one $\alpha$ (the partner), three $\alpha$s,
one $\beta$, the $\gamma$-wedge dichotomy, or the impossibility of two $\gamma$-flanks
($2\gamma=\pi+\alpha>\pi$) --- so no junction type the march can meet is missing
(\texttt{MarchJunctions}). The landing itself is the tile's own law of cosines: the apex lies at
distance exactly $b$ from the next pin because $a^2+c^2-2ac\cos\beta=b^2$
(\texttt{ladder\_step\_identity}). The catalogue's hypothesis form is itself now a theorem of the dissection layer: at any base
junction of a congruent dissection with two flank tiles of positive angle, the remaining tiles'
angles are each zero or a model corner angle, and their multiset satisfies
$x\,\alpha+y\,\beta+z\,\gamma=\pi-\mathrm{flank}_W-\mathrm{flank}_E$
(\texttt{junction\_equation}, composing the pin equation, the case split, the interior-tile and
through-tile exclusions, the congruence layer, and a counting lemma). The composition's spine is also closed: an $a$-edge's two ends carry $\beta$ and $\gamma$, one
each, and since two $\gamma$-flanks are impossible at a junction, a $\gamma$-east orientation at
one tile of the trailing run forces $\gamma$-east at every subsequent tile
(\texttt{march\_orientation\_propagates}, by induction; axioms \texttt{propext} alone) --- the
uniform orientation observed in every trace is a theorem given its seed. What remains of the
assembly: the seed orientation from the head's forced structure, the per-junction instantiation of
the junction equation with the propagated flanks, and the terminal kill --- bookkeeping over
proved pieces. Also open: the
mirrored ladder's termination, the per-word content --- its T-vertex step being already forced
(\texttt{t\_vertex\_fill}).

The consequence worth stating for the branch as a whole: the direct-orientation kill at the
$a|c$ junction uses no property of the word beyond $c$ in second position. Every escape orbit of
the pincer window with $cp=2$ --- $(4,2),(5,2),(6,2),\dots$ at every level --- shares it, and the
engine confirms the shared fan law on the three orbits probed. The uniform statement, that every
$(bp,2)$ word with $bp\ge4$ admits no tiling at any $f$, is the current target; its direct branch
is the assembled argument above, and its mirrored branch is the ladder whose step is forced and
whose termination is the one open ingredient. An adversarial
control is recorded: the $(3,2)$ word shares the junction but not the fate --- its tree grows
sevenfold per unit of $f$ --- because the fan governs only the wedge-west branches, and the rest of
a word's tree depends on the word; the window's $L2$ kill, not the fan, is what covers
$(3,2)$-type words. Under those, the $(4,2)$ word dies at every
$f$ simultaneously; since $(4,2)$ and its mirror are the only escapes of the pincer window at each
level (Remark~\ref{rem:ladder}, Remark~\ref{rem:mirrorwindow}), this would advance the reach
ladder uniformly. The signature is not special to $(4,2)$. The $(5,2)$-seeded instances show the same structure at
$f=5,\dots,8$: a constant side-case count (18), three fans in place of two, and binary fan depth
again exactly $\lfloor\beta/\alpha\rfloor$ ($6,7,9,11$), with leaf counts $3\cdot2^{\lfloor
\beta/\alpha\rfloor}$. The $(5,3)$ instances replicate it a third time, with identical fan counts
$3\cdot2^{\lfloor\beta/\alpha\rfloor}$ and their own constants ($9$ area, $30$ corner-type);
$(6,2)$ and $(7,2)$ make five, with fan counts $4\cdot2^{\lfloor\beta/\alpha\rfloor}$ and
$6\cdot2^{\lfloor\beta/\alpha\rfloor}$ and heads $27$ and $42$ --- the exponent is
$\lfloor\beta/\alpha\rfloor$ at every orbit and every $f$ probed.
The fan constant is thus shared across all three orbits probed; what varies per orbit is the
finite head. We state plainly that the forcing is an instance of the crossing question and is
open; the contribution of the law is that the question now has a measured, quantitative instance
with its arithmetic core proved, replicated on a second orbit.


\begin{remark}[Where the landing hypothesis can fail, and why it is not local]\label{rem:landingfail}
It is worth saying exactly what the remaining hypothesis costs. On the base the $f+1$ feet are
forced to be junctions, since no tiling vertex lies interior to a boundary edge, and the span
dichotomy then leaves only $f$ consecutive $a$s. At an \emph{interior} floor that premise is
unavailable: a vertex of an upper tile may sit interior to a lower tile's edge. Dropping it, the
length arithmetic still decides the cover of a length-$f^2$ stretch, but with two cases
(\texttt{A2BranchRow3.cover\_dichotomy}): $f$ $a$-edges, or a single $c$-edge. In the first the run
is reproduced one level down and the descent continues; in the second the $f-1$ interior feet become
$T$-vertices inside that $c$ and the descent is blocked.

So the feet reach the base \emph{unless} at some interior level a single $c$-edge lies directly
under the run. That is the whole residue, and it is one named configuration.

It is not locally refutable, and we record why so the ground is not walked again. At each interior
foot the wedge from below is $\pi$, so the tiles above sum to $\pi$ and the figure is
$\{\alpha,\beta,\gamma\}$ or $\{\alpha,\alpha,\alpha,\beta,\beta\}$ --- at most one
$\gamma$. Each upper tile lays an $a$, whose flanks are $\beta$ and $\gamma$, contributing exactly
one of each; by the $a$-run rigidity of \texttt{OrderForcing} the orientation word is monotone,
$M^iD^{f-i}$, and the $\gamma$s then fall on the feet $\{0,\dots,i-1\}\cup\{i+1,\dots,f\}$ ---
$f$ of them on $f$ distinct feet, one apiece, with foot $i$ free. Every constraint is met exactly.
The endpoints give nothing either: they are interior to the floor, so the tiles above them close to
$\pi$ by themselves and never meet the lower tile's $\alpha$ and $\beta$ in one equation.

Any proof must therefore be global, which is the boundary-versus-interior barrier already recorded
for the strip iteration. What has changed is that the barrier is now a single configuration rather
than a general claim about descents, and that its base case is settled outright.

The configuration is moreover not refutable by any argument blind to $m$ and $f$, and we record the
ground covered. At $e=1$ a run of $f$ $a$-edges has length $f\cdot ef=f^2=c$, so it matches one
$c$-edge exactly; for $e\ge2$ the match needs $e\mid f$ and cannot occur. The interface census
(\texttt{code/interface\_census.py}) reports the two-sided edge words of every maximal straight
interface in all eight certificates, and the equation $c=aa$ --- which at $f=2$ \emph{is} the
relation $c=f\cdot a$ --- occurs in every one of them: $3$ times in N44C, $2$ in N44B, $2$ in
\texttt{Tiling44}, $10$ in \texttt{Tiling99}, $1$ in \texttt{PgramTiling22}, $2$ in
\texttt{CevianTiling28}, $7$ in \texttt{CevianTiling63}, $2$ in \texttt{Tiling28} and $5$ in
\texttt{Tiling77}. So the configuration is not merely locally consistent, it is abundant in the
tilings we possess, all of which have $f=2$ and $m>1$. Any proof that it cannot occur must consume
$m=1$ and $f\ge3$ essentially, which is the same asymmetry Remark~\ref{rem:blockbreaks} identifies
as the only admissible shape of an argument here.

That survival is predicted, not accidental. \texttt{ScaleRigidity.no\_similarity\_invariant\_proof}
states that a criterion depending only on similarity-invariant data of the tile and the target cannot
exclude the $m=1$ case, since it would equally exclude $m=2$, where \texttt{Tiling44} exists. The
$c$-under-run configuration asserts a ratio between edge lengths and is therefore scale-free, so the
barrier applies to it directly. What the same theorem names as surviving is the scale-sensitive walk
input: at $m=1$ every admissible equal-side walk is free of $b$-edges, whereas at $m=2$ the walk
$(P,Q,R)=(3,2,1)$ carries two. Any proof that the descent lands must consume that, or another
condition separating the scales; a purely local or purely metric argument provably cannot.

Following that instruction reduces the residue further, from a configuration to two points. The
mechanism of $\texttt{side\_no\_b\_m1}$ is a $\gamma$-trap: on a side of length $f^3$ the walk
gives $f\mid Q$ modulo $f$, then $Q\ge f$ forces $R=0$ on size, and $R\ge1$ kills that; the $R\ge1$
holds because every $a$- and every $b$-edge puts a $\gamma$ at a junction of the side, no $\gamma$
sits at a base corner or the apex, and no other node carries two, so $\#a+\#b\le\#\text{edges}-1$.

The same count runs on the interior stretch. Each of the $f$ tiles above lays an $a$-edge, whose
flanks are $\beta$ and $\gamma$, so it contributes exactly one $\gamma$ among the $f+1$ junctions,
and a straight junction carries at most one. Hence \emph{the configuration dies if both endpoints of
the stretch are $\gamma$-free}, $f$ $\gamma$s being unable to fit in the $f-1$ interior junctions
(\texttt{A2BranchRow3.c\_under\_run\_dies\_if\_ends\_gamma\_free}, axiom-clean).

That criterion is true but \emph{vacuous}, and we state so rather than leave it looking like a
route. By the $a$-run rigidity the orientation word is monotone, $M^iD^{f-i}$, beta-far tiles first
and gamma-far after; so tile $1$ presents $\gamma$ at its left end, the stretch's left endpoint,
unless $i=0$, and tile $f$ presents $\gamma$ at its right end unless $i=f$. Both endpoints
$\gamma$-free would need $i=0$ and $i=f$ together, impossible for $f\ge1$
(\texttt{ends\_gamma\_free\_impossible}). Since \texttt{gamma\_count\_tight} shows the count is
exactly tight --- $f$ $\gamma$s do fit in $f+1$ junctions at one apiece --- the endpoint condition
was the only way to break it, and it cannot hold. The $\gamma$-count is a dead end.

What survives is the diagnosis. On a side the two ends are a base corner and the apex, both
$\gamma$-free for boundary reasons; on an interior stretch an endpoint's figure from above may be
$\{\alpha,\beta,\gamma\}$, which closes at $\pi$ since $\gamma+\alpha+\beta=3\alpha+2\beta$.
The $\gamma$-trap behind \texttt{side\_no\_b\_m1} is a boundary phenomenon and does not survive
the move inward. Any further attempt must couple the run to something other than its own junction
count.
\end{remark}

\begin{lemma}[Corner lines are column lines]\label{lem:columnlines}\lab{PROVED}
...
\begin{lemma}[The chain never needs the apex line]\label{lem:noapexline}\lab{PROVED}
...
\begin{lemma}[The vertex census]\label{lem:census}\lab{PROVED: the counting identity VERIFIED}
...
\begin{lemma}[The tile's three cosines]\label{lem:anglethreshold}\lab{VERIFIED}
For the tile $(a,b,c)=(f,f^2-1,f^2)$ with $f>1$ one has
\[
\cos\alpha=\frac{2f^2-1}{2f^2},\qquad
\cos\beta=\frac{3f^2-1}{2f^3},\qquad
\cos\gamma=-\frac{1}{2f},
\]
at the corners facing $a$, $b$ and $c$ respectively
(\texttt{AngleThreshold.cos\_alpha\_closed}, \texttt{.cos\_beta\_closed},
\texttt{.cos\_gamma\_closed}, via \texttt{.cos\_of\_sides}).
\end{lemma}

\begin{remark}[What the threshold is for]\label{rem:anglethreshold}\lab{PROVED}
Condition (P4) is the statement that the uncovered region never acquires a corner sharper than the
tile's own sharpest angle. The comparison is with $\cos\alpha$ above; the uncovered region is not an
object of the Lean development, which is what keeps this clause unformalized.
\end{remark}
...
\begin{lemma}[The thick-member monochotomy]\label{lem:monochotomy}\lab{VERIFIED}
...
$f^2>e^2+e$) are impossible (\texttt{c\_chord\_unique\_thick}).
\end{lemma}

\begin{remark}[What the monochotomy costs the chain]\label{rem:monochain}\lab{PROVED}
The $f$-$a$ branch of Lemma~\ref{lem:cchord} is therefore an $e=1$ phenomenon: at thick members
every $c$-chord fork of the corner chain is forced, and the chain machinery, where it applies, is
stiffer than at $e=1$.
\end{remark}

\begin{lemma}[Thick-side quantization]\label{lem:sidequant}\lab{VERIFIED}
...
\begin{lemma}[The thick base trichotomy]\label{lem:basetri}\lab{VERIFIED}
...
middle one, $f$ $a$-feet, $e$ $b$-edges, $e$ $c$-feet.
\end{lemma}

\begin{remark}[Close pairs]\label{rem:closepairbase}\lab{PROVED}
Close pairs ($f^2\le2ef+e^2$) admit in addition \emph{at most} finitely many decompositions with
$y>e$, computable per member, and possibly none. Finiteness is immediate from the walk equation:
all three edge lengths are positive, so the solutions lie in a box
(\texttt{base\_solutions\_finite}); every such decomposition has $y\equiv e\pmod f$
(\texttt{base\_column\_y\_form}). Existence, however, fails at some close pairs: at $(e,f)=(3,7)$
the base equation $21x+40y+49z=414$ has no solution with $y>3$ at all
(\texttt{close\_pair\_37\_no\_extra\_column}), before any of the filters of
Remark~\ref{rem:closepairs}; over the coprime close pairs with $e\le25$, $f\le59$, $82$ of the $282$
members admit none. The count itself is a computation and is not proved uniformly in $(e,f)$.
\end{remark}

\begin{lemma}[The shadow at a $c$-corner]\label{lem:shadow}\lab{PROVED}
...
\begin{lemma}[The thick base dichotomy]\label{lem:basedi}\lab{VERIFIED}
...
\begin{lemma}[The $c$-corner is rigid]\label{lem:ccorner}\lab{CONJECTURE}
...
\begin{theorem}[The column $(0,e,2e)$ admits no tiling]\label{thm:n1}\lab{CONJECTURE: proved at $e=1$; open at $e\ge2$, the induction being gapped (Remark~\ref{rem:n1gap})}
...
\begin{corollary}\label{cor:basewalls}\lab{CONJECTURE: rests on Theorem~\ref{thm:n1} at $e\ge2$, which is open}
...
\begin{corollary}[The base trichotomy and dichotomy without separation]\label{cor:basedi2e}\lab{VERIFIED}
...
\begin{proposition}\label{prop:n1fromwalls}\lab{CONJECTURE}
...
\begin{lemma}[Blocked-end quantization]\label{lem:anchorclear}\lab{PROVED}
...
\begin{corollary}[One-end-blocked chord dichotomy]\label{cor:onebloc}\lab{PROVED}
...
\begin{corollary}[The walls form at $e=1$, $f\ge3$]\label{cor:wallsf2e}\lab{PROVED}
...
n_c+pe=f ,
\]
and the $\gamma$-trap's $n_c\ge1$ gives $0\le p\le(f-1)/e$. Hypothesis~\ref{hyp:walls} is read throughout this
development as the assertion $p=0$; Remark~\ref{rem:wallsfix} records that this reading and the
block form of the hypothesis are not evidently equivalent. Lemma~\ref{lem:ccorner} already records this profile in the form ``$a$-count
...
\begin{corollary}[A uniform two-case side problem]\label{cor:ptwo}\lab{VERIFIED}
...
The count kills $p=2$ on $f=2e+1$ but not $p=1$. Full discussion in \cite{Obstructions}.
\end{remark}
...
\begin{lemma}[Straight-figure parity]\label{lem:parity}\lab{VERIFIED}
...
The parity constrains only the number of interior $T$-junctions, so it excludes no member. Full discussion in \cite{Obstructions}.
\end{remark}
...
\begin{lemma}[The apex identities]\label{lem:apexid}\lab{VERIFIED}
For every $(e,f)$,
\[
...
\begin{proof}
Neither identity is new, and it is worth saying which classical facts they are. Since
$3\alpha+2\beta=\pi$ and $\gamma=2\alpha+\beta$ one has $\gamma=\frac{\pi+\alpha}{2}$ and
$\beta=\frac{\pi}{2}-\frac{3\alpha}{2}$, whence
\[
\cos\tfrac{\alpha}{2}=\sin\gamma,\qquad
\cos\tfrac{3\alpha}{2}=\sin\beta,\qquad
\sin\tfrac{3\alpha}{2}=\cos\beta,\qquad
\sin\tfrac{\alpha}{2}=-\cos\gamma .
\]
The first identity therefore reads $b\sin\gamma=c\sin\beta$, which is the \emph{law of sines}
$b/\sin\beta=c/\sin\gamma$; the second reads $c\cos\beta+b\cos\gamma=a$, which is the
\emph{projection formula} for the tile. Both hold for every triangle whatever. Verified symbolically
and numerically at two precisions, and machine-checked in the closed forms used here
(\texttt{lean/ApexRigidity.lean}: \texttt{apex\_drop\_eq}, \texttt{apex\_edge\_eq}).
\end{proof}

\begin{remark}[What is actually new here]\label{rem:apexnovelty}
Since Lemma~\ref{lem:apexid} is classical, the content of Theorem~\ref{thm:apexconfig} lies entirely in
the \emph{configuration}, not in the algebra. The point is this. A base-$\beta$ target has base angle
$\beta$, so an equal side descends at angle $\beta$ to the horizontal. A tile presenting $\alpha$ at a
point of that side, with its $c$ along the side, therefore has its other edge at that vertex --- its
$b$ --- inclined at $\alpha+\beta=\pi-\gamma$ to the horizontal. The two edges drop by $c\sin\beta$ and
$b\sin\gamma$, and the law of sines makes those equal.

So the rigidity is not a coincidence of the family and not a property of the apex: it holds wherever a
tile presents $\alpha$ on an equal side with its $c$ along that side, which is exactly why it recurs at
every junction (Proposition~\ref{prop:selfsim}). What the family supplies is the hypothesis that makes
it bite --- that the target's base angle is the tile's own $\beta$, so that the tile's angle sum does
the work. The consequences drawn from it in this section, Theorems~\ref{thm:secondc},
\ref{thm:aforcesT} and \ref{thm:nobothmirror} and Corollary~\ref{cor:pbound}, are new; the identity
underneath them is not.
\end{remark}

\begin{theorem}[The first chord is covered exactly]\label{thm:apexconfig}\lab{PROVED}
...
\begin{corollary}[The tile below $T_1$'s $a$-edge has a vertex at $P$]\label{cor:noTP}\lab{PROVED}
...
\begin{theorem}[The second edge of an equal side is a $c$]\label{thm:secondc}\lab{PROVED}
Assume $e^{2}+ef<f^{2}$, equivalently $a<b$. Then on every equal side the tile edge immediately below
the first junction $J$ is a $c$. \emph{The hypothesis is removable}: Proposition~\ref{prop:nogolden}
reaches the same conclusion assuming only $\gcd(e,f)=1$, and it is that form the corollaries below use.
\end{theorem}
...
\begin{proposition}[The golden-ratio hypothesis is removable]\label{prop:nogolden}\lab{PROVED}
Theorem~\ref{thm:secondc} holds at every member with $\gcd(e,f)=1$, with no assumption relating $a$
and $b$.
\end{proposition}

\begin{proof}
Only one step used $a<b$: that $Y$'s edge from $P$ towards $J$, of length at most $|PJ|=a$, equals
$a$. A $c$-edge never fits, since $c=f^{2}>ef=a$. Suppose then $b<a$ and $Y$ lays $b$. Its far end is
interior to $T_1$'s $a$-edge, and the residual segment from $J$ to that point, of length $a-b$, must be
partitioned by whole tile edges: no edge may overrun $J$, which lies on the boundary, and none may
overrun $Y$'s vertex, which would put two tiles below the chord on the same ground. So
$a-b\in\langle a,b,c\rangle$.

It is not. Modulo $e$ the three lengths are $0$, $f^{2}$, $f^{2}$, and $a-b=e^{2}+ef-f^{2}\equiv-f^{2}$.
A representation $a-b=xa+yb+zc$ gives $(y+z+1)f^{2}\equiv0 \pmod e$, and $\gcd(e,f)=1$ forces
$e\mid y+z+1$, hence $y+z\ge e-1$ and $a-b\ge(e-1)b$. But $(e-1)b>a-b$, since that is $eb>a$, which is
$b>f$; and $b>f$ holds always, because $e\le f-1$ gives $e^{2}\le(f-1)^{2}<f^{2}-f$
(\texttt{ApexRigidity.b\_gt\_f}, \texttt{.eb\_gt\_a}). Finally $b<a$ forces $e\ge2$, so the modular step
is available (\texttt{.e\_ge\_two\_of\_b\_lt\_a}). Hence no representation exists, $Y$ cannot lay $b$,
and it lays $a$.

Checked on all $17$ coprime members with $b<a$ and $e\le9$, $f\le17$: in every case $a-b$ is a gap of
$\langle a,b,c\rangle$.
\end{proof}

\begin{remark}[What this does and does not extend]\label{rem:abnecessary}
The hypothesis $a<b$ was later removed entirely by Proposition~\ref{prop:nogolden}. Full discussion in \cite{Obstructions}.
\end{remark}

\begin{theorem}[The side parameter vanishes on the family $e=f-1$]\label{thm:efminus1}\lab{CONJECTURE}
Let $e=f-1$ with $f\ge2$. Then every equal side of the base-$\beta$ target at $m=1$ carries no
$a$-edge: $p=0$.

This establishes the \emph{west} half of Hypothesis~\ref{hyp:walls} at every such member, that half
being the assertion $p=0$ on the west side (Remark~\ref{rem:wallsfix}). We state the theorem in terms
of $p$, which is what the proof gives.
\end{theorem}

\begin{proof}
$\gcd(f-1,f)=1$, so Proposition~\ref{prop:nogolden} applies and Corollary~\ref{cor:pbound} gives
$n_c\ge2$. Here $n_c=f-pe=f-p(f-1)$. For $p=1$ this is $1$, contradicting $n_c\ge2$; for $p\ge2$ it is
at most $2-f\le0$, contradicting the $\gamma$-trap's $n_c\ge1$. Hence $p=0$.

Concretely at $p=1$ the side word would carry $f$ $a$-edges and a single $c$, the $c$ at the apex; and
Theorem~\ref{thm:secondc} requires a second $c$ immediately below it, which is not available. The side
does have at least three edges, so the edge below the first junction exists and the theorem applies.
\end{proof}

\begin{remark}[Scope, and the status of this theorem]\label{rem:efminus1}
The theorem is \emph{proved}, not machine-checked: steps~2--3 of its chain are planar geometry. Full discussion in \cite{Obstructions}.
\end{remark}


\begin{corollary}[A bound on the side parameter]\label{cor:pbound}\lab{PROVED}
Let $\gcd(e,f)=1$. Then $n_c\ge2$, hence
\[
pe+2\le f .
\]
...
\begin{proof}
The edge at the apex and the edge below $J$ are both $c$ by Theorem~\ref{thm:secondc}, in the form
freed of the hypothesis $a<b$ by Proposition~\ref{prop:nogolden}, which needs only $\gcd(e,f)=1$; so
$n_c\ge2$,
...
The bound reaches $p=1$ only at $(1,2)$; at every other member it stops at $p\ge2$. Full discussion in \cite{Obstructions}.
\end{remark}

\begin{remark}[The seven open primes of the tight subfamily]\label{rem:sevenprimes}
On the tight subfamily $f=2e+1$ the tile count is $N=11e^{2}+12e+3$, and the members whose $N$ is a
prime congruent to $11$ modulo $12$ and still open are these:
\[
\begin{array}{rrrll}
N & e & f & \text{tile }(a,b,c) & \text{target}\\\hline
227 & 4 & 9 & (36,65,81) & (729,729,908)\\
1223 & 10 & 21 & (210,341,441) & (9261,9261,12230)\\
3011 & 16 & 33 & (528,833,1089) & (35937,35937,48176)\\
4643 & 20 & 41 & (820,1281,1681) & (68921,68921,92860)\\
5591 & 22 & 45 & (990,1541,2025) & (91125,91125,123002)\\
8963 & 28 & 57 & (1596,2465,3249) & (185193,185193,250964)\\
13127 & 34 & 69 & (2346,3605,4761) & (328509,328509,446318)
\end{array}
\]
Every one satisfies the standing hypothesis: on $f=2e+1$ the inequality $e^{2}+ef<f^{2}$ reduces to
$e^{2}+3e+1>0$, true for all $e$. So the whole of \S\ref{sub:apex} applies at each, and
Corollary~\ref{cor:pbound} reads $pe\le 2e-1$, giving $p\le1$.

Each member is therefore in \emph{exactly} the state of $(3,7)$: the base walk is settled,
$p\in\{0,1\}$, and the single open question is $p=1$, which by
Corollary~\ref{cor:walls-from-T} reduces to whether a $T$-junction occurs at one of the points
$W(J)$. Whatever settles $(3,7)$ by theory settles all seven, since no step of \S\ref{sub:apex} uses
the member beyond $e^{2}+ef<f^{2}$ --- and conversely an exhaustive search at $(3,7)$ settles that
member alone, the next target having $N=227$ and a target triangle nine times larger in linear scale.
\end{remark}


\begin{proposition}[The descent is self-similar]\label{prop:selfsim}\lab{PROVED}
...
\begin{lemma}[A single $\gamma$ never excludes a $T$-junction]\label{lem:onegamma}\lab{VERIFIED}
...
\begin{theorem}[An $a$-edge forces a $T$-junction]\label{thm:aforcesT}\lab{PROVED}
...
\begin{corollary}[A sufficient condition for Hypothesis~\ref{hyp:walls}]\label{cor:walls-from-T}\lab{CONJECTURE}
...
\subsection{The inflation reformulation}\label{sub:inflation}

The base-$\beta$ tile is a rep-tile. Remark~\ref{rem:blockbreaks} records that the complete west block
is the tile scaled by $f$, containing $f^2$ tiles, and that at $m=1$ its far side is the whole equal
side. Read the other way, that says the equal side's edge word is the $c$-side word of an inflated
tile, and the side question becomes a question about the inflation alone.

\begin{proposition}[The inflated boundary]\label{prop:inflbdy}\lab{PROVED}
Let the tile be inflated by $f$, so its sides are $fa$, $fb$, $fc$. Then the $\alpha$- and $\beta$-corners of the inflated triangle are each met by a single
tile, which lays along the two sides the flanks of that angle. No assumption is made at the
$\gamma$-corner, and none is needed: the $c$-side joins the $\alpha$- and $\beta$-vertices, so both its
end edges are $c$; the $a$-side has its $\beta$-end forced to $a$ and the $b$-side its $\alpha$-end
forced to $b$, and in each case the residual composition is unique. At $(1,3)$ and $(3,7)$ the outcome
is:
\[
\begin{array}{llll}
(e,f) & a\text{-side} & b\text{-side} & c\text{-side}\\\hline
(1,3) & a^3 \text{ (rigid)} & b^3 \text{ (rigid)} & c^3 \ \text{ or }\ c\,a^3c\\
(3,7) & a^7 \text{ (rigid)} & b^7 \text{ (rigid)} & c^7 \ \text{ or }\ c\,a^7c^2c
\end{array}
\]
where the second $c$-side word at $(3,7)$ has $7$ $a$-edges and $4$ $c$-edges.
\end{proposition}

\begin{proof}
The residual lengths are $fa-2a$, $fb-2b$, $fc-2c$. At $(1,3)$, tile $(3,8,9)$: the residuals are
$3$, $8$, $9$, whose compositions over $\{3,8,9\}$ are $\{3\}$, $\{8\}$ and $\{9\},\{3,3,3\}$
respectively (\texttt{Inflation.middle\_three}, \texttt{.middle\_eight}, \texttt{.middle\_nine}). At
$(3,7)$, tile $(21,40,49)$: the $c$-side residual is $245$, with compositions $5c$ and $7a+2c$
(\texttt{.residual\_245}). For the $a$- and $b$-sides only one end is known, so the residuals are
$fa-a=126$ and $fb-b=240$; their compositions are again unique, $6a$ and $6b$
(\texttt{.residual\_126}, \texttt{.residual\_240}), and at $(1,3)$ the corresponding residuals $6$ and
$16$ give $2a$ and $2b$ (\texttt{.residual\_13\_sides}).

Only the $\alpha$- and $\beta$-corners were used. Two tiles at the $\alpha$-corner already give
$2\alpha>\alpha$; and a $\beta$-corner splits only as $\beta=k\alpha$, which with $3\alpha+2\beta=\pi$
gives $\alpha(3+2k)=\pi$, making $\alpha/\pi$ rational and contradicting its irrationality. So the
$\beta$-corner is single at every member, with no hypothesis
(\texttt{Inflation.beta\_not\_nat\_multiple}, \texttt{.beta\_corner\_single}).
\end{proof}

\begin{corollary}[The crux, on $f^2$ tiles]\label{cor:inflcrux}\lab{PROVED}
At $m=1$ the inflated $c$-side is the whole equal side, so the two $c$-side words of
Proposition~\ref{prop:inflbdy} are exactly $p=0$ and $p=1$. Deciding which occurs --- that is,
deciding whether the inflation is rigid --- decides the side parameter on that side.
\end{corollary}

\begin{remark}[What this buys, and what it does not]\label{rem:inflvalue}
Recorded in \cite{Obstructions}. Full discussion in \cite{Obstructions}.
\end{remark}


\begin{theorem}[The inflation is rigid at every member searched]\label{thm:inflrigid}\lab{CONJECTURE}
The inflated tile of Proposition~\ref{prop:inflbdy} admits no tiling by $f^2$ copies whose $c$-side
carries the $p=1$ word, at each of the ten members
\[
(1,3),\ (1,4),\ (1,5),\ (2,5),\ (3,5),\ (1,6),\ (1,7),\ (3,7),\ (5,7),\ (4,9).
\]
Its only boundary is therefore the standard one, $a^f$, $b^f$, $c^f$.
\end{theorem}

\begin{proof}
Exhaustive exact search over the two profiles left open by Proposition~\ref{prop:inflbdy}. At $(1,3)$,
target $(9,24,27)$ with $N=9$: the $c$-side profile $(3,0,2)$ gives \texttt{EXHAUSTED\_NO\_TILING} in
$9$ nodes, and $(0,0,3)$ gives \texttt{FOUND\_TILING} in $30$. At $(3,7)$, target $(147,280,343)$ with
$N=49$: the profile $(7,0,4)$ gives \texttt{EXHAUSTED\_NO\_TILING} in $57\,824$ nodes, and $(0,0,7)$
gives \texttt{FOUND\_TILING} in $443\,347$, with boundary words $a^7$, $b^7$, $c^7$ as predicted. At
$(2,5)$, target $(50,105,125)$ with $N=25$: the profile $(5,0,3)$ gives \texttt{EXHAUSTED\_NO\_TILING}
in $297$ nodes and $(0,0,5)$ gives \texttt{FOUND\_TILING} in $2\,212$. At $(4,9)$, $N=81$: the
profile $(9,0,5)$ gives \texttt{EXHAUSTED\_NO\_TILING} in $5\,161\,190$ nodes. The remaining six
members were generated by \texttt{code/engine/gen\_inflation.py}, which reproduces the five hand-built
instances byte for byte; their node counts are

\[
\begin{array}{lrrrr}
(e,f) & f^2 & k=f-e & \#\text{words} & p=1\ \text{nodes}\\\hline
(1,3) & 9 & 2 & 1 & 9\\
(1,4) & 16 & 3 & 5 & 25\\
(1,5) & 25 & 4 & 21 & 137\\
(2,5) & 25 & 3 & 6 & 297\\
...
(5,7) & 49 & 2 & 1 & 3\,219\\
(3,7) & 49 & 4 & 36 & 57\,824\\
(4,9) & 81 & 5 & 220 & 5\,161\,190
\end{array}
\]

with the word count of Remark~\ref{rem:wordcensus} beside each. Every member carries a
\texttt{FOUND\_TILING} control on its $p=0$ instance.

The arithmetic behind the two-word dichotomy is machine-checked at the first three members
(\texttt{Inflation.middle\_nine}, \texttt{.residual\_75}, \texttt{.residual\_245}), as is the
uniqueness of the $a$- and $b$-side compositions.

The \texttt{FOUND} runs are negative controls: a search exhausting both profiles would indicate a
malformed instance rather than a theorem. Their node counts are reported for orientation only and are
\emph{not} reproducible constants: a \texttt{FOUND} search halts at the first tiling it meets, so the
count depends on how work was distributed across threads. At $(2,5)$ the same instance returns
$2\,212$, $2\,199$, $2\,223$ and $2\,414$ nodes on different thread counts. The
\texttt{EXHAUSTED} counts carry no such dependence --- an exhaustive search visits the same tree
however it is scheduled, and this was checked directly at $(3,7)$, where $1$, $2$ and $4$ threads all
return $57\,824$ nodes with all four prune counters equal. At $(4,9)$ that control is still running at the time of
writing, so the fourth member is stated on the strength of its \texttt{EXHAUSTED} verdict alone and
should be read as one degree weaker than the first three until the control returns. Instances, certificates and node counts are in
\texttt{code/engine/inflation/}. Search verdicts are declared computer assistance and are not
formalized.
\end{proof}

\begin{corollary}[Remark~\ref{rem:sidenoa}'s step, proved]\label{cor:sidenoa-proved}\lab{CONJECTURE}
At each of the ten members of Theorem~\ref{thm:inflrigid}: if the west corner block is complete, then the equal side on that side
carries no $a$-edge.
\end{corollary}

\begin{proof}
A complete west block is the tile inflated by $f$ (Remark~\ref{rem:blockbreaks}), and at $m=1$ its far
side is the whole equal side. By Theorem~\ref{thm:inflrigid} that side is $c^f$, so $n_a=0$.
\end{proof}

\begin{remark}[What this settles and what it does not]\label{rem:inflrigidscope}
This does not decide $p$ at $(3,7)$: the inflated tile is the west corner block only if that block is complete, which is the west half of Hypothesis~\ref{hyp:walls} itself. Full discussion in \cite{Obstructions}.
\end{remark}

Theorem~\ref{thm:inflrigid} is proved one member at a time by search, which is unsatisfying: the
statement is uniform in $(e,f)$ and ought to have a uniform proof. One is available for half the
family, at the cost of an extra hypothesis.

\begin{proposition}[Edge parity kills the $p=1$ boundary when $e$ and $f$ differ in parity]\label{prop:inflparity}\lab{PROVED}
Assume the dissection of the inflated tile is \emph{edge-to-edge}. If $e\not\equiv f \pmod 2$, then the
boundary word with $p=1$ is impossible.
\end{proposition}

\begin{proof}
The $f^2$ tiles contribute $3f^2$ edge-slots. Edge-to-edge means every edge of the subdivision is an
entire edge of each tile meeting it, so each interior edge consumes exactly two slots and each boundary
edge exactly one; hence $3f^2 - B \equiv 0 \pmod 2$, where $B$ is the number of boundary edges. The
standard boundary has $B=3f$, and $3f^2-3f=3f(f-1)$ is always even. The $p=1$ boundary has $f$ edges on
the $a$-side, $f$ on the $b$-side, and $f + (f-e)$ on the $c$-side, so $B=4f-e$ and
\[
  3f^2-(4f-e) \;\equiv\; f^2+e \;\equiv\; f+e \pmod 2 .
\]
This is odd exactly when $e\not\equiv f$, and an odd value contradicts the count.
\end{proof}

\begin{corollary}[The $p=1$ word needs two $c$-edges, so $e=f-1$ is rigid]\label{cor:twoc}\lab{VERIFIED}
A $p=1$ word exists only if $e+2\le f$. Consequently the inflation is rigid at every member with
$e=f-1$, uniformly in $f$ and with no search.
\end{corollary}

\begin{proof}
Proposition~\ref{prop:inflbdy} forces both end letters of the $c$-side to be $c$-edges: the $c$-side
joins the $\alpha$- and $\beta$-vertices, each met by a single tile laying the flanks of its angle, and
neither $\alpha$ (flanks $\{b,c\}$) nor $\beta$ (flanks $\{a,c\}$) can present an $a$ there once the
$a$-side is $a^f$ and the $b$-side $b^f$. The two ends are distinct positions, because the $p=1$ word has
$f+(f-e)\ge2$ letters. Hence the word carries at least two $c$-edges, and its $c$-count is $f-e$, giving
$f-e\ge2$. At $e=f-1$ the $c$-count is exactly $1$, so no $p=1$ word exists and the standard boundary is
the only one. \textup{(}Machine-checked, \texttt{p1\_c\_count\_ge\_two}, \texttt{no\_p1\_word\_at\_e\_pred}.\textup{)}
\end{proof}

\begin{remark}[The word census, and why the search cost is what it is]\label{rem:wordcensus}
Corollary~\ref{cor:twoc} also explains the observed cost of the searches. Writing $k=f-e$, a $p=1$ word
is fixed by placing the remaining $k-2$ $c$-edges among the $f$ $a$-edges between the two forced end
$c$'s, so the number of admissible words is
\[
  \binom{f+k-2}{\,k-2\,}.
\]
This is $1$ when $k=2$ --- the word is forced to be $c\,a^{f}c$ --- and grows quickly with $k$. The
search's top-level task count is exactly this number: the $(4,9)$ run, with $k=5$, dispatched
$\binom{12}{3}=220$ tasks. The table below shows the count beside the measured node counts, and the
$k=2$ members are cheap for a structural reason rather than an accidental one.
\end{remark}

\begin{proposition}[Monotonicity on a side of the target]\label{prop:orientmonobdy}\lab{VERIFIED}
Let a maximal run of consecutive $a$-edges lie on a straight side of the target, let
$T_1,\dots,T_L$ be the tiles laying them, and suppose each $T_i$ presents $\beta$ or $\gamma$ at
each end of its $a$-edge. Then no $\mathrm{BG}$ is followed by a $\mathrm{GB}$, the sequence is
$\mathrm{GB}^{j}\mathrm{BG}^{L-j}$ for a single $j$, and at most one junction of the run is the
transition.

The local step is the corner figure: at a junction each tile presents $\beta$ or $\gamma$, and two
$\gamma$'s are impossible there, since the straight system $x+2z=3$, $y+z=2$ forces $x=-1$ at $z=2$
(\texttt{MarchRun.no\_two\_gammas}, \texttt{.junction\_cases}). The global form is then immediate:
writing $\mathrm{BG}$ as $\mathsf{true}$, ``never $\mathrm{BG}$ then $\mathrm{GB}$'' says the word
is monotone, hence a block of $\mathrm{GB}$ followed by a block of $\mathrm{BG}$, hence carries at
most one transition (\texttt{MarchMonotone.mono\_of\_no\_step}, \texttt{.block\_form},
\texttt{.at\_most\_one\_transition}). No passage from a real boundary to an orientation word is
used: the argument runs at the junctions themselves.
\end{proposition}

\begin{proposition}[The orientation of a boundary $a$-run is monotone]\label{prop:orientmono}\lab{PROVED}
The same conclusion for a run on a side of the \emph{inflated tile}, together with the clause that
each $T_i$ presents $\beta$ at one end of its $a$-edge and $\gamma$ at the other.

Neither is covered by Proposition~\ref{prop:orientmonobdy}. The first needs the inflated tile's side
to carry the straight-vertex figure, which is the scale map; the second is the tile-corner fact that
the two corners flanking the $a$-side are the $\beta$- and $\gamma$-corners, $\alpha$ being opposite
$a$ --- available in \texttt{TilePlacement.incident\_sides} but not composed with
\texttt{cornerAngle} to a statement about what a placed tile presents.
\end{proposition}

\begin{proof}
Side $a$ is opposite $\alpha$, so the angles at its two ends are $\beta$ and $\gamma$; a tile laying
$a$ on the boundary therefore presents one of each, never $\beta$ twice. A junction interior to the run
receives the right end of one tile and the left end of the next, so the four orientation pairs give
contributions $\{\gamma,\beta\}$, $\{\gamma,\gamma\}$, $\{\beta,\beta\}$, $\{\beta,\gamma\}$
respectively. By Proposition~\ref{M-prop:vertexfigures} the only straight figures are
$\{\alpha,\beta,\gamma\}$ and $\{3\alpha,2\beta\}$, carrying one $\gamma$ and none. So
$\{\gamma,\gamma\}$ --- the pair $\mathrm{BG},\mathrm{GB}$ --- admits no figure at all, and
$\{\beta,\beta\}$ --- the pair $\mathrm{GB},\mathrm{BG}$ --- forces $\{3\alpha,2\beta\}$. The first
statement forbids the descent $\mathrm{BG}\to\mathrm{GB}$, which is the monotonicity; the second
identifies the $\{3\alpha,2\beta\}$ junctions with the $\mathrm{GB},\mathrm{BG}$ transitions, of which
a monotone sequence has at most one. \textup{(}Machine-checked: \texttt{BG\_GB\_forbidden},
\texttt{orient\_monotone}, \texttt{AAB\_iff\_transition}.\textup{)}
\end{proof}

\begin{remark}[What this buys]\label{rem:orientmono}
The branching at the $k=2$ word falls from $3^{f-1}$ to $f+1$, and the proposition applies verbatim to an $a$-run on an equal side. Full discussion in \cite{Obstructions}.
\end{remark}

\begin{remark}[Scope, and why this is not a proof of Theorem~\ref{thm:inflrigid}]\label{rem:inflparity}
The obstruction assumes the dissection is edge-to-edge, which the main paper does not, and it is silent when $e$ and $f$ are both odd --- which includes $(1,3)$ and $(3,7)$. Full discussion in \cite{Obstructions}.
\end{remark}

\subsection{The forced row and the slot dichotomy: the induction \texorpdfstring{$W(k)$}{W(k)}}\label{sub:forcedrow}

Write $W(k)$ for the statement: \emph{every scale-$k$ inflation occurring inside a tiling of the
$m=1$ target is subdivided standardly} ($1\le k\le f$; at $k=f$ this is the inflation of
Proposition~\ref{prop:inflbdy} again, now as an occurrence rather than in isolation). The
induction $W(k-1)\Rightarrow W(k)$ proceeds by forcing the first row of the occurrence tile by
tile; the forcing stalls at exactly one per-slot dichotomy, whose surviving branch is the
\emph{rogue} configuration that the slot-kill theorems
(\texttt{RogueContainment}, \texttt{RogueChord}, \texttt{RogueFan}, \texttt{RogueMirror}) and the
engine certificates are stated against. Those modules cite the row forcing as ``the forced row
(paper)''; this subsection is that reference. The arithmetic skeleton --- the position calculus,
the junction ledger, the figure forcing and the dichotomy --- is machine-checked in
\textup{\texttt{lean/ForcedRow.lean}}, cited step by step below.

\emph{Setup.} Let $\Delta_k$ be a scale-$k$ occurrence: a sub-region of the hypothetical tiling,
bounded by tile edges, congruent to the tile scaled by $k$, with the tiling respecting its
boundary. Place its $\alpha$-corner at $A$, its $\gamma$-corner at $C=A+kb\,w$ and its
$\beta$-corner at $B=A+kc\,u$, where $w$, $u$, $\hat v$ are the unit vectors along the base
$AC$, the $c$-side $AB$, and the $a$-side direction $C\to B$ respectively; the chart identity
$c\,u-a\,\hat v=b\,w$ (\texttt{RogueMirror.generator\_x/\_y}) relates them. Write
\[
Y_i \;=\; A+(i+1)\,b\,w \qquad (0\le i\le k-2)
\]
for the base junctions and $X_i=A+i\,b\,w+c\,u$ for the row apexes; the jump identity
$Y_i+a\,\hat v=X_i$ is \texttt{ForcedRow.jump\_x}.

\begin{lemma}[Boundary words at scale $k$]\label{lem:rowwords}\lab{PROVED}
For $k<f$ the three side words of $\Delta_k$ are forced outright: the base reads $b^k$
\textup{(\texttt{Inflation.b\_side\_rigid})}, the $a$-side reads $a^k$
\textup{(\texttt{.a\_side\_rigid})}, and the $c$-side carries no $b$-edge
\textup{(\texttt{.c\_side\_no\_b})}. At the wall scale $k=f$ the same three readings hold:
the $c$-side by the $\gamma$-trap \textup{(\texttt{SideNoB.side\_no\_b\_uncond})}, the base by
the corner coupling \textup{(\texttt{RogueMirror.wall\_base\_reading})}, and the $a$-side is
$a^f$ or $c^e$ \textup{(\texttt{Inflation.a\_side\_words}; the $c^e$ branch is the transverse
configuration, treated separately)}.
\end{lemma}

\begin{lemma}[The corner tile]\label{lem:rowp0}\lab{PROVED}
The tile $P_0$ of the occurrence at $A$ is unique and standard: it presents $\alpha$ at $A$ with
its $b$-flank on the base and its $c$-flank on the $c$-side.
\end{lemma}

\begin{proof}
The region's corner angle at $A$ is $\alpha$, and a fill $x\alpha+y\beta+z\gamma=\alpha$ has
$(x,y,z)=(1,0,0)$ \textup{(\texttt{CChord.fill\_alpha}, via the multiplicity bridge
\texttt{Geometry.vertex\_multiplicities})}: a single tile presenting $\alpha$, whose two edges at
$A$ are the flanks $\{b,c\}$ of $\alpha$. The $c$-side carries no $b$-edge
(Lemma~\ref{lem:rowwords}), so the $b$-flank lies on the base.
\end{proof}

\begin{lemma}[The first partner, and the parallelogram]\label{lem:rowq0}\lab{PROVED}
$P_0$'s $a$-edge $[\,b\,w,\;c\,u\,]$ is matched by a single tile $Q_0$, the half-turn image of
$P_0$ about the edge midpoint; $P_0\cup Q_0$ is the parallelogram $(A,\;b\,w,\;b\,w+c\,u,\;c\,u)$.
\end{lemma}

\begin{proof}
Both endpoints of the $a$-edge lie on the region boundary, and its extensions leave the region
(below the base line at $b\,w$; across the $c$-side at $c\,u$, since $\hat v$ points outward
there). The far side of the edge is therefore covered by whole tile edges partitioning it exactly
\textup{(\texttt{Geometry.Dissection.wall\_two\_sided}, with tile edges walls by
\texttt{edge\_point\_not\_interior} and the covering tiles contained in the region --- the
containment obligation)}, with lengths summing to $a$; \texttt{Inflation.a\_unsplittable} leaves a
single $a$-edge, hence a single partner tile carrying $\{\beta,\gamma\}$ at the two ends. The
reflected assignment puts its $\gamma$ at $b\,w$, where $P_0$ already presents $\gamma$: a
junction interior to the base carries at most one $\gamma$
\textup{(\texttt{BaseBetaCorners.pi\_vertex\_gamma\_le\_one}; directly, $2\gamma=\pi+\alpha>\pi$)}.
So the direct partner is forced, and the half-turn closes the parallelogram
\textup{(\texttt{ForcedRow.pgram\_x})}.
\end{proof}

\begin{lemma}[The row advance at $Y_0$]\label{lem:rowp1}\lab{PROVED}
The tile completing the junction $Y_0=b\,w$ is $P_1=P_0+b\,w$.
\end{lemma}

\begin{proof}
At $Y_0$ the figure sums to $\pi$ on the region side; $P_0$'s $\gamma$ and $Q_0$'s $\beta$ leave
the residual $\alpha$ \textup{(\texttt{ForcedRow.junction\_ledger})}, filled by a single
$\alpha$-tile \textup{(\texttt{CChord.fill\_alpha})} with flanks $\{b,c\}$ on the rays $w$ (the
base, forward) and $u$ ($Q_0$'s $c$-edge ray). Laying $c$ on the base would contradict the base
word $b^k$ ($b\neq c$, \texttt{ForcedRow.base\_letter\_distinct}), so the $b$-flank lies on the
base and the tile is the translate $P_1$.
\end{proof}

\begin{proposition}[The slot dichotomy]\label{prop:slotdichotomy}\lab{PROVED}
Let $1\le i\le k-2$ and suppose the row is standard up to $P_i$. At the junction $Y_i$ the figure
is $\{\alpha,\beta,\gamma\}$: the $\gamma$ is $P_i$'s, the $\alpha$ is the next row tile
$P_{i+1}=P_0+(i+1)b\,w$, forced with its $b$-flank on the base, and the $\beta$-tile spans the
residual between the rays $u$ and $\hat v$ at $Y_i$ with flanks $\{a,c\}$, laid in one of exactly
two ways:
\begin{itemize}
\item[\textup{(std)}] $a$ along $\hat v$ --- its $a$-edge is exactly $P_i$'s, the tile is the
direct partner $Q_i=Q_0+i\,b\,w$, and the parallelogram closes as before;
\item[\textup{(rogue)}] $c$ along $\hat v$ --- the $c$-edge overruns the row apex $X_i$ by
$\Delta=c-a=f(f-e)$ \textup{(\texttt{ForcedRow.overrun\_x};
\texttt{RogueChord.delta\_identities})}, an overhang shorter than every tile edge, making $X_i$ a
T-vertex.
\end{itemize}
At $i=0$ the rogue branch does not exist, since both ends of the chord are blocked
(Lemma~\ref{lem:rowq0}); the slot range is exactly $1\le i\le k-2$, i.e.\ $M:=i+1\in[2,k-1]$.
\end{proposition}

\begin{proof}
When the forcing reaches $Y_i$ only $P_i$'s $\gamma$ is present there. The junction figure solves
$x\alpha+y\beta+z\gamma=\pi$, so $(x,y,z)$ is $(3,2,0)$ or $(1,1,1)$
\textup{(\texttt{BaseBetaCorners.pi\_vertex\_figures})}; the $\gamma$ kills the first, leaving
exactly one $\alpha$ and one $\beta$ besides it \textup{(\texttt{ForcedRow.slot\_figure})}. The
$\beta$-tile cannot be base-adjacent: its flanks are $\{a,c\}$
\textup{(\texttt{Interface.beta\_has\_no\_b\_flank})} and the base letter is $b$, with
$a\neq b\neq c$ \textup{(\texttt{ForcedRow.base\_letter\_distinct})}. So the base-adjacent tile
is the $\alpha$, laying $b$ on the base --- the translate $P_{i+1}$, as in
Lemma~\ref{lem:rowp1} --- and the $\beta$-tile occupies the residual
$\pi-\gamma-\alpha=\beta$ \textup{(\texttt{ForcedRow.slot\_residual\_beta})} between $u$ and
$\hat v$, laying $\{a,c\}$ on those rays in one of the two orders
\textup{(\texttt{ForcedRow.flank\_product}, \texttt{.std\_iff\_chord\_a})}. In the std case the
$a$-edge coincides with $P_i$'s $a$-edge $[Y_i,X_i]$ (\texttt{ForcedRow.jump\_x}) and the tile is
the direct partner; in the rogue case $c>a$ produces the stated overrun
\textup{(\texttt{ForcedRow.rogue\_overrun\_pos})}. Unlike at $i=0$, the chord through $Y_i$ and
$X_i$ is blocked only at $Y_i$ --- beyond $X_i$ it runs into untiled interior --- so the far-side
covering may overrun at $X_i$ and \texttt{a\_unsplittable} does not exclude the rogue branch.
\end{proof}

\begin{corollary}[The induction step]\label{cor:rowinduction}\lab{PROVED}
If every slot of $\Delta_k$ takes the std branch, the first row is the standard one ---
$P_0,\dots,P_{k-1}$ and $Q_0,\dots,Q_{k-2}$, $2k-1$ tiles --- and the closure of
$\Delta_k$ minus the row is the scale-$(k-1)$ occurrence anchored at $A+c\,u$
\textup{(\texttt{ForcedRow.remainder\_x/\_y}; the count $(2k-1)+(k-1)^2=k^2$ is
\texttt{ForcedRow.row\_count})}. Hence $W(k-1)\Rightarrow W(k)$ holds whenever every slot's rogue
branch is refuted, and the whole tower $\{W(k):k\le f\}$ closes when this happens at every scale.
\end{corollary}

The rogue branches are refuted per slot by the kill theorems: containment for $Me<f$
(\texttt{RogueContainment}, wired as \texttt{ForcedRow.slot\_dies\_by\_containment}), the first
slot, boundary exit and top slot (\texttt{RogueChord} K1--K3), the low slots by the fan
(\texttt{RogueFan}), the top-2/top-3 slots by the corridor (\texttt{RogueMirror}), and the
residual $(M,k)$ cells by engine certificate. The wall-scale input --- the base of $\Delta_f$
reads $b^f$ --- is \texttt{RogueMirror.wall\_base\_reading}, so every scale $k\le f$ consumes
Lemma~\ref{lem:rowwords} uniformly.

\begin{remark}[Standing hypotheses, and labels]\label{rem:rowhyps}
The chain above is proved on paper over exactly the following named inputs, and nothing else.
(i) \emph{Angle sums}: the tile angles at a point of the region sum to $2\pi$ (interior), $\pi$
(interior to a side), or the corner angle --- the \texttt{Dissection.HasAngleSums} class, now
proved for the target (\texttt{Geometry.Dissection.hasAngleSums} in
\texttt{lean/AngleSumDissection.lean}, with the vertex-sector crux
\texttt{Tri.volume\_inter\_ball\_vertex} in \texttt{lean/VertexSector.lean}); the passage from
these real equations to the integer multiplicities consumed by the fills is proved
(\texttt{Geometry.vertex\_multiplicities}, with $\alpha/\pi\notin\Q$ proved at every member).
The region-local instances (the figure at a point of a sub-region such as $\Delta_k$ summing to
the region's angle) are the same computation read inside the region and remain the named form
in which the row forcing consumes them. (ii) \emph{Coverings}: each side of a wall segment
is partitioned exactly once by whole tile edges --- proved
(\texttt{Geometry.Dissection.side\_partition}, \texttt{wall\_two\_sided}); what remains of that
obligation is the ordered-run extraction (\texttt{ChordInterface.FarSide}'s order), which is
bookkeeping on the proved partition. (iii) \emph{Containment}: a tile with interior points in a
tile-bounded region lies in it. (iv) \emph{The boundary words} of Lemma~\ref{lem:rowwords},
themselves standing on (i)--(iii). The tower statement is about occurrences: it does not by
itself decide Hypothesis~\ref{hyp:walls}, which asks whether the west corner block occurs at all
(Remark~\ref{rem:inflrigidscope}).
\end{remark}
...
\begin{lemma}[The $\gamma$-injection budget at $p=2$]\label{lem:tight}\lab{PROVED}
...
\begin{proposition}[The $p=2$ side is forced]\label{prop:tightside}\lab{PROVED}
...
\begin{lemma}[The chord at the last junction]\label{lem:chord}\lab{PROVED}
...
\begin{theorem}[$p=2$ is excluded on the tight subfamily]\label{thm:ptwodead}\lab{PROVED}
...
\begin{theorem}[The last-junction dichotomy]\label{thm:lastjunction}\lab{PROVED}
...
and \cite{Obstructions} rules out the two most natural candidates, so that mechanism must be metric.
\end{remark}

\begin{theorem}[The side carrying $T_2$'s $c$-edge is not mirrored]\label{thm:nobothmirror}\lab{PROVED}
...
\begin{corollary}[The figure at $P$ on that side is fully determined]\label{cor:figureP}\lab{PROVED}
...
\begin{proposition}[The two placements of $U$]\label{prop:Uplacements}\lab{PROVED}
...
flush along the chord through the second junction, laid by the $c$-tile below $J$ and by $U$. That the
two vertices land at the same height is the law of sines for the tile, $c\sin\beta=b\sin\gamma$, not a
coincidence of the member.

The two placements are cleanly distinguished by that chord: in (i) $U$ lies \emph{flush} with it,
its third side lying in the chord; in (ii) $U$ \emph{straddles} it, with vertices at heights
$+180\sqrt{187}/343$ and $-9\sqrt{187}/14$ relative to it at $(3,7)$. Placement (ii) puts a $\gamma$
against a straight angle at a point interior to $Y$'s $c$-edge, which by Lemma~\ref{lem:onegamma} no
angle count can exclude; and we have not excluded it geometrically either. Neither placement is
contradictory as far as we can determine, so $U$ does not close the case.

The two placements are separated by a further fact, obtained by the semigroup argument of
Proposition~\ref{prop:nogolden}. In placement (ii) the vertex $X_1$ sits at distance $b$ along $Y$'s
$c$-edge $[P,W]$, leaving a residual $[X_1,W]$ of length $c-b=e^{2}$. At $(3,7)$ that is $9$, below
every tile side, hence a gap of $\langle a,b,c\rangle$; and the covering on $U$'s side of that edge
cannot overrun $X_1$, which would place two tiles under $U$. It must therefore overrun $W$, so a
$T$-junction at $W$ is \emph{forced}, and the figure there is determined:
\[
\underbrace{\gamma}_{Z}+\underbrace{\alpha}_{Y}+\pi+\beta=2\pi .
\]
Placement (i), by contrast, shares $W$ with $Y$ edge-to-edge and forces no such $T$-junction. So the
two placements are distinguished by exactly the phenomenon Corollary~\ref{cor:walls-from-T} is about.
This is consistent, not contradictory: $p=1$ requires a $T$-junction somewhere, and placement (ii)
supplies one. **Correction, 2026-09-07**: the argument for $e^{2}$ being a gap does not need
$e^{2}<\min(a,b)$ at all --- \texttt{gap\_e\_squared} (\texttt{Frontier.lean}, zero-`sorry`,
axiom-clean) proves $e^2\notin\langle ef,f^2{-}e^2,f^2\rangle$ unconditionally, for every coprime
$1\le e<f$, by a divisibility argument ($f\mid(1{+}y)\Rightarrow y\ge f{-}1\ge e\Rightarrow
y\cdot b>e^2$) that never uses the size bound $e^2<\min(a,b)$. This paragraph's earlier text
undersold the corpus's own result, contradicting \S\ref{rem:overrunshape}, which already states
the gap holds for no member.'' The fact IS family-wide; what remains open, unaffected by this
correction, is that placement (ii)'s T-junction is forced but not yet shown contradictory.

One structural fact about the boundary of the forced region is worth recording, since it says where an
argument has to engage. Every tile forced so far uses one of only \emph{two} edge directions: the side
direction, at $\beta$ below the horizontal, and the direction at $\alpha+\beta$ below it. Consequently
every forced vertex sits at a \emph{rational} multiple of the chord spacing $\delta=H/f$; at $(3,7)$
the heights are $0$, $1$, $c/b$, $2$, $(b+c)/c$, $(b+c)/b$, with denominators dividing
$\operatorname{lcm}(b,c)$. This cannot persist, since the vertical projection of the $\mathbb Z$-module
generated by all admissible edge vectors is dense (\cite{Obstructions}). Of the four ways to
continue past $X_1$ in placement (ii), exactly two stay inside the two-direction system and two
introduce a third direction. The rationality breaks precisely at those two, and that is the boundary
of the region the present arguments control. What is gained is that the
...
\begin{proposition}[The figure at $P'$ on the mirrored side]\label{prop:figurePprime}\lab{PROVED}
...
The sign condition governing the third cross product is not a second, independent appearance of
$e^{2}+ef<f^{2}$, and it would be misleading to present it as one. The quadratic factor is a
difference of squares,
\[
(e^{2}-ef-f^{2})(e^{2}+ef-f^{2})=(f^{2}-e^{2})^{2}-(ef)^{2}=b^{2}-a^{2},
\]
so the third cross product is $e\sqrt{4f^{2}-e^{2}}\,(b^{2}-a^{2})/(2f^{2})$ and is positive precisely
when $b>a$. The containment test \emph{is} the edge-length comparison, in disguise. For the same
reason the first two cross products stand in the ratio $b:c$.
\end{remark}
...
\subsection{Close pairs: the extra base columns}\label{sub:closepairs}

The close-pair regime $f^2\le 2ef+e^2$ admits base decompositions the separated members do not.
This subsection puts them in closed form and gives the sharp criterion, which is Hall's condition
rather than the counting bound.

\begin{remark}[Close pairs: what the unconditional filters leave]\label{rem:closepairs}\lab{PROVED}
At a close pair ($f^2\le 2ef+e^2$) Lemma~\ref{lem:basetri} no longer applies and the base may admit
extra columns --- not every close pair does; see Remark~\ref{rem:closepairbase} --- and each of
them has $y=e+kf$ for some $k\ge1$ (\texttt{base\_column\_y\_form}, unconditional). Three unconditional filters apply: the
...
\begin{remark}[The extra base columns, in closed form]\label{rem:columnreduction}\lab{PROVED}
...
\begin{proposition}[The extra base columns at a close pair]\label{prop:closepaircolumns}\lab{VERIFIED}
...
\begin{proposition}[Sharp criterion for an extra base column]\label{prop:sharpcolumn}\lab{VERIFIED}
...
\begin{lemma}[Unit parallelogram]\label{lem:pgram}\lab{VERIFIED}
...
\begin{lemma}[Collar decomposition]\label{lem:collar}\lab{PROVED: the cell and tile counts VERIFIED}
...
\begin{theorem}[Realizability, member $(1,2)$]\label{thm:realize12}\lab{CONJECTURE}
...
\begin{theorem}[The family is not uniform]\label{thm:notuniform}\lab{PROVED}
...
The failure of the seeds to travel is not an accident of the search; it is visible in the
certified tilings themselves.

\begin{proposition}[The relation lattice]\label{prop:rellattice}\lab{VERIFIED}
For the tile $(a,b,c)=(ef,f^2-e^2,f^2)$ with $\gcd(e,f)=1$, the lattice
$\Lambda(e,f)=\{n\in\Z^3:\ n_aa+n_bb+n_cc=0\}$ is free of rank two, generated by
\[
v_1=(f,0,-e)\quad(f\ \text{$a$'s}=e\ \text{$c$'s}),\qquad
v_2=(e,f,-f)\quad(e\ \text{$a$'s}+f\ \text{$b$'s}=f\ \text{$c$'s}).
\]
Every $n\in\Lambda$ has $f\mid n_b$ (\texttt{rel\_b\_mult}), and every $n$ with $n_b=0$ lies in
$\Z v_1$ (\texttt{rel\_param}); subtracting the right multiple of $v_2$ reduces the general case to
that one (\texttt{rel\_span}), and $v_1,v_2$ are independent (\texttt{rel\_indep}).
\end{proposition}

\begin{proposition}[The relation floor]\label{prop:relfloor}\lab{VERIFIED}
For the tile $(a,b,c)=(ef,f^2-e^2,f^2)$ with $0<e<f$ and $\gcd(e,f)=1$, every nonzero relation has
one-sided length at least $f\min(a,b)$ (\texttt{InterfaceFloor.interface\_floor}). The bound is
sharp: $v_1=(f,0,-e)$ attains it, its positive side being $f$ $a$-edges of total length $f a$
(\texttt{.floor\_attained}, \texttt{.v1\_relation}).
\end{proposition}

\begin{proposition}[The interface floor]\label{prop:interfacefloor}\lab{PROVED}
Hence every maximal straight interface of any tiling that is shorter than $f\min(a,b)$ carries the
same edge multiset on both sides. Maximal straight interfaces of a dissection are not objects of the
Lean development, which is what blocks this clause.
\end{proposition}

\begin{proof}
A maximal straight segment of the tiling is covered on each side by whole tile edges, and the two
words have equal total length, so their multiset difference lies in $\Lambda$. Reducing the
defining equation mod $f$ gives $f\mid n_be^2$, hence $f\mid n_b$; writing $n_b=fy$ and cancelling
$f$, then reducing mod $e$, gives $n_c=-fy+et$ and back-substitution $n_a=ey-ft$, i.e.
$n=y\,v_2-t\,v_1$. For the floor: if $n_b\neq0$ one side carries at least $f$ $b$-edges (length
$\ge fb$); if $n_b=0$ and $n=sv_1\neq0$ one side carries at least $f$ $a$-edges (length $\ge fa$).
\end{proof}

\begin{theorem}[The certified seeds are $(1,2)$-degenerate]\label{thm:seeddegenerate}\lab{CONJECTURE}
Sweep, exactly over $\Q(\sqrt{15})$, all eight kernel-certified tilings of the $(1,2)$ family ---
\texttt{Tiling44} $=$ \texttt{N44B}, \texttt{N44C}, \texttt{Tiling99}, \texttt{PgramTiling22},
\texttt{CevianTiling28}, \texttt{CevianTiling63}, \texttt{Tiling28}, \texttt{Tiling77}
(\texttt{code/interface\_census.py}). The two-sided words of every maximal straight interface
either agree as multisets or differ by one of the relations
\[
c=2a,\qquad 2b=3a,\qquad 2b=a+c,\qquad 3c=4b,\qquad a+2b=2c ,
\]
and each of these, evaluated at $(a,b,c)=(ef,f^2-e^2,f^2)$, factors as a unit times $(f-2e)$
times a quantity positive on $0<e<f$. Every nontrivial interface equation used by every certified
seed therefore vanishes exactly on $f=2e$: \emph{the seed complexes close on the member $(1,2)$
and on no other member}, so no relabelling of any certified seed transfers along the family ---
in structural agreement with Theorem~\ref{thm:notuniform}. What a construction at a general
member must use instead is the member's own lattice $\Lambda(e,f)$, whose shortest nonzero
relations have length $ef^2$ and $f^3$; the two-lattice architecture of
Theorem~\ref{thm:pgram-e1} is the one known construction that manages with the universal
relations alone, and it is exactly the one that generalises.
\end{theorem}

\begin{proposition}[Cevian reduction: two tiles and an atom]\label{prop:cevianatom}\lab{PROVED}
In $\Delta_m$ let $P_e$ be the base point at distance $ecm$ from the east corner. The cevian from
the apex to $P_e$ has length $fbm$ and cuts $\Delta_m$ into the triangle
$(ef^2m,\,fbm,\,f^3m)=fm\cdot(a,b,c)$ --- the tile at scale $fm$, tileable by $(fm)^2$ copies for
every $m$ --- and the west piece $W_m$ with sides $(e(2f^2-e^2)m,\,f^3m,\,fbm)$ and angles
$(\beta,\,2\alpha,\,\alpha+\beta)$. The wall from $(fam,0)$ to the apex cuts $W_m$ in turn into
the tile at scale $fm$ and the \emph{atom} $A_m$: the isosceles triangle with legs $fbm$, base
$ebm$, apex angle $\alpha$, carrying $bm^2$ tiles. Hence
\[
\Delta_m \;=\; fm\cdot T \;\sqcup\; fm\cdot T \;\sqcup\; A_m,
\qquad m^2N_1 \;=\; 2(fm)^2 + bm^2
\]
(\texttt{two\_tiles\_plus\_atom} in \textup{\texttt{lean/Primitives.lean}}), and realizability at
multiplier $m$ follows from a tiling of $A_m$ alone. The certified instances of $W_m$ at $(1,2)$
are \texttt{CevianTiling28} ($W_2$) and \texttt{CevianTiling63} ($W_3$), and the $28$-tile
triangle $(12,14,16)$ of \texttt{Tiling28} is the same shape as $W_2$. No certified tiling of any
$A_m$ is known: both cevian certificates cross the internal wall ($3$ resp.\ $7$ straddlers,
exact sweep), the unit atom $A_1$ admits no tiling at any $e=1$ member --- its base word is forced
to the single letter $b^e$ by the residue census (\texttt{atom\_base\_forced}), and at $e=1$ the
single $b$-edge's $\gamma$-end overflows the base corner opening $\alpha+\beta$
(\texttt{corner\_fill\_unique}) --- and $A_1$ admits none at $(2,3)$ by the forced overlap of the
retraction commit. The atom spectrum $\{m:A_m\ \text{tiles}\}$ is open at every member, including
$(1,2)$; a member-uniform tiling of some $A_{m_0}$ would make every family's realizable set
nonempty with primitives bounded by $m_0$, and is the sharpest open construction question of the
branch.
\end{proposition}

The cevian piece, unlike the atom, has seeds --- and at $e=1$ its own addition law, which closes
its spectrum at $(1,2)$ completely.

\begin{lemma}[The W-parallelogram, $e=1$]\label{lem:wpgram}\lab{PROVED}
Let $e=1$. The parallelogram spanned by $fb$ (cevian direction) and $2f^2-1$ (base direction),
included angle $\alpha+\beta$, admits a tiling by $2(2f^2-1)$ tiles. Explicitly: the $c$-glued
two-tile brick is the parallelogram with sides $a,b$ and angle $\alpha+\beta$; the base side is
$Y=2f^2-1=f\cdot a+b$ (recall $a=f$, $b=f^2-1$), so $f$ slabs of height $a$ (each $fb/b=f$
bricks laid $b$-along-the-cevian-direction) and one slab of height $b$ ($fb/a=b$ bricks laid
$a$-along) fill it with $f^2+b=2f^2-1$ bricks
(\texttt{w\_slab\_counts} in \textup{\texttt{lean/Primitives.lean}}). Every interface is a full
straight segment.
\end{lemma}

\begin{theorem}[W-addition, and the second complete pair]\label{thm:wfamily}\lab{CONJECTURE}
Let $e=1$ and write $S_W=\{m:\ W_m\ \text{admits a tiling}\}$. Then $S_W$ is closed under
addition: $W_{m_1+m_2}$ decomposes as $W_{m_1}$ anchored at the $\beta$-corner, the parallelogram
of Lemma~\ref{lem:wpgram} at scale $m_1\times m_2$, and an upright translate of $W_{m_2}$, all
interfaces full straight segments. At $(e,f)=(1,2)$ the kernel-certified seeds
\textup{\texttt{CevianTiling28}} ($W_2$) and \textup{\texttt{CevianTiling63}} ($W_3$) therefore
give $W_m$ for every $m\ge2$; and $W_1$ admits no tiling unconditionally, since a $7$-tiling of
$W_1$ together with the east $f$-scaled tile would tile $\Delta_1=(8,8,11)$, refuted by the
certified $N=11$ exhaustion. Hence the instance spectrum of the pair
(scalene $(2\alpha,\beta,\alpha{+}\beta)$ target, tile $(2,3,4)$) is completely determined:
\[
W_m\ \text{is tileable}\iff m\neq1,\qquad\text{realizing}\quad N=7m^2\in\{28,\,63,\,112,\,175,\,252,\dots\}\ \text{for every }m\ge2 .
\]
This is the second (target, tile) pair with no undecided instance, over the same tile as
Theorem~\ref{thm:realize12}, and it contributes the count $175=7\cdot5^2$, reachable neither from
$28$ nor $63$ by the subdivision ladder.
\end{theorem}

\begin{proof}
The decomposition is the barycentric cut of the band below the apex-anchored copy, exactly as in
Theorem~\ref{thm:addlaw}, transported to the scalene shape: in the affine frame $u=$ (west side),
$v=$ (base) at the $\beta$-corner, $W_s=\{xu+yv:\ x,y\ge0,\ x+y\le s\}$, and the cut $x=m_1$
splits the band $m_1\le x+y\le s$ into the parallelogram spanned by $m_1(u-v)$ and $m_2v$ ---
sides $m_1\,fb$ and $m_2(2f^2-1)$, angle $\alpha+\beta$, tiled by Lemma~\ref{lem:wpgram} --- and
the upright translate $\{x\ge m_1\}$ of $W_{m_2}$. Counts:
$(m_1+m_2)^2(2f^2-1)=m_1^2(2f^2-1)+2m_1m_2(2f^2-1)+m_2^2(2f^2-1)$ (\texttt{add\_count}); the
anchors are \texttt{w\_counts}. Sums of $2$s and $3$s reach every $m\ge2$
(\texttt{two\_step}, \textup{\texttt{lean/Collar.lean}}).
\end{proof}
...
\begin{theorem}[The unit parallelogram, $e=1$]\label{thm:pgram-e1}\lab{PROVED}
...
\begin{theorem}[The dichotomy]\label{thm:dichotomy}\lab{PROVED}
...
\begin{proposition}[Wide parallelograms, all $(e,f)$]\label{prop:widecol}\lab{PROVED}
A parallelogram with sides $p\,Y_1$ (base direction) and $q\,X_1$ (leg direction), angle $\beta$,
is a pure $a\times c$ lattice --- a $(pN_1/f)\times(qf)$ grid of two-tile cells, hence tileable by
$2pqN_1$ tiles --- if and only if $f\mid p$ (the side divisibilities and count are
\texttt{pgram\_cols}, \texttt{pgram\_cell\_count} in \textup{\texttt{lean/Primitives.lean}}).
\end{proposition}

\begin{theorem}[Addition law]\label{thm:addlaw}\lab{PROVED}
Fix a member $(e,f)$ and $m_1,m_2\ge1$ with $\Delta_{m_1}$ and $\Delta_{m_2}$ tileable. If
$f\mid m_1$, or if $e=1$, then $\Delta_{m_1+m_2}$ is tileable.
\end{theorem}

\begin{proof}
Anchor $\Delta_{m_1}$ at the apex of $\Delta_{m_1+m_2}$. The band below it is the trapezoid with
parallel horizontal sides $m_1Y_1$ (top) and $(m_1+m_2)Y_1$ (base) and legs $m_2X_1$. Cut it from
the top-west vertex parallel to the east leg: by the isosceles altitude identity
$X_1\cos\beta=Y_1/2$ (as in Lemma~\ref{lem:collar}) the cut has length $m_2X_1$ and lands at
$(m_2Y_1,0)$, splitting the band into the upright scale-$m_2$ triangle on $[0,m_2Y_1]$ and the
parallelogram $P$ spanned by $m_1Y_1$ (base direction) and $m_2X_1$ (east-leg direction). Tile the
triangle by hypothesis. For $P$: if $f\mid m_1$, Proposition~\ref{prop:widecol} applies with
$(p,q)=(m_1,m_2)$; if $e=1$, $P$ is an $m_1\times m_2$ grid of translates of the unit
parallelogram $P_1$ of Theorem~\ref{thm:pgram-e1}. All three interfaces are full straight
segments. The count bookkeeping is \texttt{add\_count} in \textup{\texttt{lean/Primitives.lean}}.
\end{proof}

\begin{remark}[Correction, and the closure structure]\label{rem:widecolfix}
An earlier version of Proposition~\ref{prop:widecol} asserted the single-seed step ``$\Delta_M$ is
tileable whenever $\Delta_t$ is tileable and $f\mid M-t$''. That step is withdrawn: slicing the
band below $\Delta_t^{\mathrm{apex}}$ by lines parallel to a leg always leaves an upright corner
triangle of scale $M-t$, which the hypothesis does not tile; the two-seed form of
Theorem~\ref{thm:addlaw} is what the geometry gives. The closure structure of
$S(e,f)=\{m:\Delta_m\ \text{tileable}\}$ is therefore: closed under $m\mapsto km$ (subdivision),
closed under addition of an $f$-divisible element, and closed under addition outright at $e=1$.
With seeds $\{2,3\}$ at $(1,2)$ this recovers Theorem~\ref{thm:realize12}; at $(1,3)$, where
$m=1$ is exhausted and $m=2$ excluded (Theorem~\ref{thm:notuniform}), the set generated by a
$\Delta_3$ seed under all three operations is exactly $3\Z_{\ge1}$
(\texttt{reachable\_13} in \textup{\texttt{lean/Primitives.lean}}).
\end{remark}
