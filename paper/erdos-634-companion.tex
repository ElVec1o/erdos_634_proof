...
\begin{theorem}[The inflation is rigid at $(1,3)$ and $(3,7)$]\label{thm:inflrigid}
The inflated tile of Proposition~\ref{prop:inflbdy} admits no tiling by $f^2$ copies whose $c$-side
carries the $p=1$ word, at $(1,3)$ and at $(3,7)$. Its only boundary is therefore the standard one,
$a^f$, $b^f$, $c^f$.
\end{theorem}
...
gives \texttt{FOUND\_TILING} in $443\,347$, with boundary words $a^7$, $b^7$, $c^7$ as predicted.
...
At $(1,3)$ and $(3,7)$: if the west corner block is complete, then the equal side on that side carries
no $a$-edge.
\end{corollary}
...
step rather than proving it. Corollary~\ref{cor:sidenoa-proved} proves it at the two members where the
