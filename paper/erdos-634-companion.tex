...
A nonzero relation has one-sided length at least $f\min(a,b)$, and \emph{every maximal straight
interface of any tiling that is shorter than $f\min(a,b)$ carries the same edge multiset on both
sides}. (\texttt{rel\_v1}, \texttt{rel\_v2}, \texttt{rel\_b\_mult}, \texttt{rel\_param} in
\textup{\texttt{lean/Primitives.lean}}.)
\end{proposition}
