...
One hypothesis remains, and it is the only one: that $V$ is a \emph{vertex} of the serving tile
rather than a point interior to one of its edges. The figure at $V$ supplies it in principle --- the
single straight angle there belongs to the tile below (Proposition~\ref{prop:flankline}'s companion
\texttt{alpha\_wall\_figure\_real}, with $s=1$), so no upper tile has $V$ interior to an edge --- but
that step is not composed with the pigeonhole in the formal development: the tile the pigeonhole
returns is not yet known to be one of the tiles the figure counts.

Everything else in route 1 is discharged. The approach points exist and are covered; the serving
tile is an upper tile; its edge directions are weakly upward; the flank at $V$ lies along the line;
a $b$- or $c$-edge from $V$ contains $E$ strictly and dies by the very blocking whose failure
defined the escape; the surviving $a$-advance is a descent that terminates after finitely many wall
edges. The residue is one composition step, not a mechanism.
\end{remark}
