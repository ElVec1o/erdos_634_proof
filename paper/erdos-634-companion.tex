...
\texttt{sides\_strict} supplying $f<f^2-1<f^2$). The one input still not proved is the
side--angle correspondence itself, that the shortest side faces the smallest angle, so that the
$a$-edge is the one opposite $\alpha$. It is standard, and it is the last geometric fact the
bridge consumes. No further geometric obstacle stands in the way, but until those two are supplied the
