...
Read through the identification of \S\ref{sub:sidep}, this is Hypothesis~\ref{hyp:walls} at every such
member. We state the theorem in terms of $p$, which is what the proof establishes, because that
identification is asserted there rather than proved.
\end{theorem}
...
One dependency deserves flagging rather than silent use. What the argument establishes directly is
$p=0$ on every equal side; the identification of that with Hypothesis~\ref{hyp:walls} is the statement
in \S\ref{sub:sidep} that the hypothesis ``is exactly the assertion $p=0$'', which is asserted there
and not proved. Remark~\ref{rem:blockbreaks} does not close the gap either: it identifies the complete
$f$-block as the tile scaled by $f$, whose far side at $m=1$ is the whole equal side, but completeness
of that block does not by itself fix how its far side is partitioned into tile edges, and it is that
partition which $p$ measures. Deriving either implication remains open, and we record it as such
rather than leave the equivalence doing unexamined work. Theorem~\ref{thm:efminus1} is therefore unconditional as a statement about $p$, and
