...
$p=0$ on every equal side at \emph{every member with $e=f-1$}: no equal side carries an $a$-edge. Read
through the identification recorded in the companion, that is Hypothesis (walls) there; we state it in
terms of $p$, which is what the argument establishes, since that identification is asserted rather
than proved. The tile counts are $N=2f^{2}+2f-1$, and the base-$\beta$ candidates among them --- primes
