...
only if $m\ne1$, apparently the first incommensurable pair with no undecided instance. The
phenomenon does not persist along the family. For the next member the scale-$2$ target admits no
...
Theorem~\ref{thm:realize12} does not extend along the base-$\beta$ family. For $(e,f)=(1,3)$ --- tile
...
correct argument in the literature. For the subfamily $e=1$ the companion note \cite{Companion}
now proves the exclusion outright (its Theorem on the $e=1$ family, for all even $f$): this
settles by argument the candidates $11, 47, 107, 191, 431,\dots$ --- every prime
$p=3f^2-1$ with no second base-$\beta$ representation --- and leaves $e\ge2$ as the open residue. This assessment includes the recent preprint \cite{BeesonNoPrime}
...
parallelogram needs, so Proposition~\ref{prop:cornerpara} holds with \emph{no} proviso. Second, $e=1$ --- which
...
$(8,7,13)$ tile (Remark~\ref{rem:tilinggroup}) to the base-$\beta$ tile itself
...
admit one, by the kernel-checked certificates of Remark~\ref{rem:thm14false}. Hence, if $\mathcal C$
...
Proposition~\ref{prop:product} is a statement about the $2\pi/3$ branch only. It says nothing about
the base-$\beta$ branch of $3\alpha+2\beta=\pi$, which is a different branch with a different tile
and is where the one genuine gap of Theorem~\ref{thm:main} lives (Remark~\ref{rem:isobeta}). What it
does buy is a narrowing of the rationality dependence: Corollary~\ref{cor:similar} and
Proposition~\ref{prop:F1free} discharge three of the eleven shapes --- tile-similar, $F_1$, $F_1'$
...
\begin{remark}\label{rem:ratfreescope}
...
$44,77,80$ and the standard commensurable and prime\,$\equiv1\!\pmod4$ values, and all others are
excluded. The last undetermined value below $80$ was $N=70$, settled by exhaustive search of its
single surviving instance (Remark~\ref{rem:seventy}).
\end{theorem}
\begin{proof}
...
The sweeps of Theorems~\ref{thm:frontier2}--\ref{thm:frontier4} test the isosceles and $F_1$ targets
...
Section~\ref{sec:full}, the minima over primitive $120^\circ$-triples are
\[
...
are not new, the sweep of Section~\ref{sec:full} having already searched the same tile $(3,5,7)$ on
...
that no scalene $2\pi/3$ shape can affect any $N<88$, so Theorem~\ref{thm:frontier4} and everything
...
theorem's divisibility condition $g\mid M$, whose proof is unsound (Remark~\ref{rem:b3prime}):
...
\]
the first two being the instances of Theorem~\ref{thm:frontier2} and Remark~\ref{rem:seventy},
...
Theorem~\ref{thm:fullprime}, which is conditional on the companion's complete-corner-wall
hypothesis (Remark~\ref{rem:conditional}). \emph{So $83$ is not excluded unconditionally by this
paper}, and it is the first value for which that is true: everything below $81$ is unconditional,
either by an $N$-form, by Theorem~\ref{thm:main} for primes $\equiv7\pmod{12}$, or by an exhaustion.
...
$3\alpha+2\beta=\pi$ at $m=1$, and the companion paper records the following unconditional
progress on that hypothesis.
...
$1\le p\le(f-1)/e$ where $fp$ is the number of $a$-edges on a side, together with the members
...
Proposition~\ref{prop:product}, Corollary~\ref{cor:similar} and Proposition~\ref{prop:F1free}
...
Lemma~\ref{lem:cancel} is the one place where the remaining geometry sits, and it is now isolated to
a single statement rather than distributed over the proof. \texttt{InvariantCore.cancellation\_core}
derives the lemma from one hypothesis: that the total directed length of interior tile edges is
invariant under reversal of direction. \texttt{interiorBalanced\_of\_segments} reduces
that global identity to a local one --- each maximal interior segment is covered once from each side
--- and the local statement is assembled from four machine-checked steps: a tile coincides with a
half-plane near a point in the relative interior of one of its edges
(\texttt{Tri.inter\_ball\_eq\_halfplane}); a half-plane through the centre halves a ball
(\texttt{volume\_halfspace\_inter\_ball}, which Mathlib does not have); a ball inside the target is
partitioned by the tiles (\texttt{Dissection.volume\_ball\_eq\_sum}); and the resulting count forces
exactly two tiles (\texttt{Dissection.local\_balance}). The classification of a tile's local
contribution is exhaustive away from the tiles' vertices (\texttt{Tri.classify}), a finite set.
What is \emph{not} yet written is the mechanical assembly of these pieces into
\texttt{InteriorBalanced}; no further geometric content is required for it.

What remains on written proofs, checked numerically but not formalized, is the direction grid of
Section~\ref{sec:prelim} and Lemma~\ref{lem:value}. Search verdicts (\texttt{EXHAUSTED}) are
...
included (Section~\ref{sec:verification}). What is \emph{not} settled is the base-$\beta$ branch:
Theorem~\ref{thm:fullprime} is conditional on the complete-corner-wall hypothesis of the companion
note, and Remark~\ref{rem:conditional} states precisely what survives without it. The whole is
