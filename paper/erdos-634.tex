...
every tiling of every $e=1$ member rather than in one branch of an alternative. We note in passing that the
prime candidates in the subfamily all have $f$ even, since $3f^2-1$ is even for odd $f$
(\texttt{lean/Erdos634/ThinHole.lean}, \texttt{thin\_prime\_f\_even}); the odd-$f$ members
$N=26,74,146,\dots$ are composite, so the parity hypothesis whose removal is recorded in the
companion was never binding on the prime problem. For $e\ge2$ the collapse does not occur: the
