...
We can now say something sharper about the hypothesis's strength, and it is not favourable. At
$e=1$, $m=1$, $f\ge3$ the hypothesis is not weaker than its conclusion --- it is \emph{equivalent}
to it. The block form places $f$ $a$-feet on the base at the west corner, feet being consecutive and
starting at the corner, so it asserts that the base word begins with $a^f$. But the companion's
reduction theorem for the $e=1$ subfamily proves unconditionally that the base word is a permutation
of $(a^f,b,c)$ beginning \emph{and} ending with an $a$-edge, and $f+1$ letters $a$ are not
available. So no tiling of an $e=1$ member has a complete west corner, and to assert the hypothesis
of every tiling at those members is precisely to assert that there are none. Theorem~\ref{thm:fullprime}
accordingly carries no $e=1$ content beyond the vacuous.

It matters that the clash is at the \emph{west} corner. The companion's Remark on the asymmetry of
the hypothesis locates an incompatibility at the east corner and retreats to a west-only reading,
``the hypothesis is exactly $p=0$''; that retreat is unavailable, because the west block is itself
the sole premise of the derivation of $p=0$, and a structure that provably never occurs cannot
support a derivation. The argument is formalised in \texttt{lean/Erdos634/WallsCircular.lean}
(\texttt{west\_block\_never\_complete}, \texttt{walls\_iff\_no\_tiling}), axiom-clean.

No member is lost to this. The $e=1$ primes are excluded by two routes that never mention the
hypothesis: the companion's unconditional theorem on the $e=1$ family for all even $f$, and the
reduction of every $f$ to the single geometric fact that the multiples of $f$ in $[0,f^2)$ are
tiling vertices on the base (\texttt{lean/Erdos634/E1Assembly.lean}). What is lost is only the
appearance that the hypothesis was doing work at $e=1$. For $e\ge2$ the collapse does not occur: the
base word has $f+2e$ letters with $e$ $b$'s and $e$ $c$'s, an $a^f$ prefix costs no more than the
$f$ available $a$'s, and the east block's $c$-feet can still occupy the final positions.
\end{remark}
...
settles by argument the candidates $11, 47, 107, 191, 431,\dots$ --- and, by
