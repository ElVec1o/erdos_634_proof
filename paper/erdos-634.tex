...
the arithmetic core is machine-checked in Lean for \emph{all five} target shapes, with our own
proofs replacing Beeson's Theorems~18 and~20, whose printed arguments are unsound); an isosceles tile with
...
correct, by the base-length obstruction of Proposition~\ref{prop:b3prime}. For the isosceles
base-$\beta$ target the prime exclusion is likewise machine-checked
(\texttt{isobeta\_square\_not\_prime}): with $s=e/f$ in lowest terms the tile is
$(ef,\,f^2-e^2,\,f^2)$, and the Theorem-14 equation $N(e+f)^2=M^2(3f^2-e^2)$ forces
$N\mid 3f^2-e^2$ (an exponent-parity descent), while integrality of the equal side
($X^2=Nbc$, so $N(f^2-e^2)$ is a square) forces $N\mid f^2-e^2$; together $N\mid 2f^2$,
impossible for an odd prime with $\gcd(e,f)=1$.
