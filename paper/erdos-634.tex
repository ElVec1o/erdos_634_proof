...
lists die arithmetically, and that claim has now produced one false exclusion. Until the sweep's
scalene $3\alpha+2\beta$ step (\texttt{verify\_frontier.py}) is re-audited against the cevian
family $N=m^2(2f^2-e^2)$ over all members $(e,f)$, the composite exclusions of
Theorems~\ref{thm:frontier}--\ref{thm:frontier5} carry that caveat; the prime exclusions are
untouched, resting on Theorem~\ref{thm:main} and the congruence class alone. The re-audit is in
progress.
\end{remark}
