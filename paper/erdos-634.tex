...
the spectra (with a parity refinement of the invariant counts) with the published branch theorems
and an exhaustive exact search of the surviving finite instances, we settle every previously
undetermined value up to $32$: no triangle can be cut into $14$, $15$, $21$, $22$, or $30$
congruent triangles; and $N=46$, the sharpest test of Zhang's completeness conjecture, is likewise
excluded, in agreement with that conjecture. The arithmetic and combinatorial layer is
...
spectrum above bounds the realized set from outside, and on tiles such as $(7,8,13)$ and
$(5,16,19)$ the two now coincide as families.
\end{remark}
...
No triangle can be cut into $21$, into $22$, into $30$, or into $46$ congruent triangles.
\end{theorem}

\begin{proof}
The sweep above; for $21$, additionally the two exhaustive searches. Dependencies as in
...
and $14,15,21,22,30,46$ by Theorems \ref{thm:frontier} and~\ref{thm:frontier2}, the membership
of every $N\le 32$ is now determined; the smallest undetermined value, to our knowledge, is
$N=33$, whose sweep leaves two isosceles-$(\alpha{+}\beta)$ instances (tiles $(272,33,289)$ and
$(28,33,49)$) for the same treatment.
...
(\texttt{pi3\_equilateral\_fails\_14\_15} through \texttt{iso\_ab\_congruence\_kills\_14}), and of
the $N=21,22,46$ sweep (\texttt{F1\_invariant\_kills\_21}, \texttt{parity\_kills\_46},
\texttt{iso\_ab\_congruence\_kills\_22}).
\end{itemize}
