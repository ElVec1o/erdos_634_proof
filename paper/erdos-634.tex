...
one of: $3\alpha+2\beta=\pi$, for which $N$ is not prime \cite{BeesonIII} (for the $(2\alpha,\beta,\alpha{+}\beta)$
target the arithmetic core --- Beeson's first tiling equation $N+M^2=2K^2$ with $K\mid N$ admits no
odd prime --- is re-proved and machine-checked in Lean, \texttt{lean/Beeson3NotPrime.lean}); an isosceles tile with
