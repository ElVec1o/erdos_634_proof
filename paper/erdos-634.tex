...
\end{enumerate}
Feeding in the $\gamma$-trap ($R\ge1$ on every side, Remark~\ref{rem:e2}) kills the third base and the
first side: \emph{for $f>2e$ at $m=1$ the base is $\{b^{e},c^{2e}\}$ or $\{a^{f},b^{e},c^{e}\}$, and
neither equal side carries a $b$-edge.}
\end{theorem}
...
The final sentence is immediate: $\{a^{2f},b^{e}\}$ and $\{a^{e},b^{f}\}$ are exactly the walks with
$R=0$.
\end{proof}
...
$e=4$, $f\ge9$; \dots --- covering the primes $N=47,71,107,191,227,239,359,\dots$, and it delivers the
$e\ge2$ analogue of the $e=1$ structure theorem of Remark~\ref{rem:lem6}: the base is one of two
walks and the equal sides are $b$-free. The step that fails for $m\ge2$ is the very first one: there
...
statement in this branch that is uniform in $e$. It does not yet close the branch: what it shows is
that the difficulty is concentrated in the \emph{thick} regime $f\le2e$, where the level $2e$ admits
up to $\lfloor 2ef/(f^2-e^2)\rfloor+1$ values of $j$ and the trichotomy dissolves. That is precisely
where the smallest open members sit: $N=59$ is $(e,f)=(4,5)$ with $f\le2e$, as are
