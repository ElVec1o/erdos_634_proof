...
$v$, which does occur here (Remark~\ref{rem:parityfail}). Mathlib has no theory of planar dissections,
so this angle statement is not formalized; it is the geometric input flagged in
Section~\ref{sec:verification}. What \emph{is} formalized is the arithmetic consequence drawn from it
in Proposition~\ref{prop:vertexfigures} (\texttt{vertex\_multiplicities} and its three corollaries)
...
In any tiling of a triangle by a tile with $3\alpha+2\beta=\pi$ and $\alpha\notin\Q\pi$, no junction
interior to a side of $ABC$ carries $\gamma$ on both sides.
\end{proposition}

\begin{proof}
Such a junction is a $\pi$-vertex, so by Proposition~\ref{prop:vertexfigures} its multiplicity vector
is $(3,2,0)$ or $(1,1,1)$; in both cases $\gamma$ occurs at most once.
\end{proof}
...
(Proposition~\ref{prop:gammatrap}) is proved above from the vertex figures. Two further structural
facts are used only to prune the exact search, and we state them as such, having verified them
against the $44$-tiling rather than proved them in general: (a) every side of $ABC$ carries at least
one $c$-edge, and at $m=1$ neither equal side carries a $b$-edge, so all boundary $b$-edges lie on
the base; (b) \emph{corner parallelogram} --- the $\beta$-corner tile $T_B$ has its $b$-edge a chord
matched by a single tile $T_3$ with $\alpha,\gamma$ interchanged, so $T_B\cup T_3$ is a parallelogram
with sides $a,c$, whence the first two and last two base edges and the first two edges of each equal
side lie in $\{a,c\}$. Both are pruning heuristics: the searches they accelerate are also run
without them, and the verdicts agree.
...
quoted in the literature without scope and cannot hold in the stated generality (Remark~\ref{rem:e2}).
Put
...
\begin{remark}\label{rem:rationality}
...
functional of Section~\ref{sec:invariant} is defined and integral, rationality is automatic. It does
