...
reduction of the scalene shapes, this excludes every prime $\equiv 3\pmod 4$ exceeding $3$
\emph{except} the base-$\beta$ candidates $N=3f^2-e^2$, conditional on the cited classification of
the remaining branches. The base-$\beta$ branch is \emph{not} settled here. The companion note's
forcing chain for it rests on a complete-corner-wall hypothesis which we show is, at $e=1$,
equivalent to its own conclusion, so the conditional theorem carries no content there; at $e\ge2$
the hypothesis is a genuine assumption and remains unproved. Those candidates are exactly the primes $\equiv11\pmod{12}$
...
those branches; the equilateral criteria likewise reduce to elementary divisor conditions. For the equilateral
target we also close $N=105$ outright: no equilateral triangle can be cut into $105$ congruent
triangles (Theorem~\ref{thm:eq105}), the three surviving candidate tiles being excluded by
certified exhaustive search. Combining the spectra with the published
...
\begin{theorem}\label{thm:main}\lab{PROVED}
...
\begin{theorem}[Full prime exclusion, conditional]\label{thm:fullprime}\lab{CONJECTURE: effective, its hypothesis being open at $e\ge2$ and equivalent to the conclusion at $e=1$}
...
$N=11,23,26,47,59,66,71,107$, $191$, $431$, $587$, $971$ and $1451$, each by a certified exhaustive search of
every surviving instance, which depend on no branch theorem whatever. The class $11\pmod{12}$ beyond those values,
$N=83$ first among them, is \emph{not} settled unconditionally.

The value $191$ is new here and deserves its mechanism. At $e=1$, $f=8$ the base walk is one of
$42$ words; the pincer window at the proved reach~$3$ kills all but $14$, which fall into $7$
mirror-orbits, and each of those is now excluded by exhaustive search. What made this feasible is a
prune we introduce below: a convex corner of the uncovered region smaller than the tile's
second-smallest angle can be met only by tiles presenting the smallest angle --- an edge through it
would contribute $\pi$ --- so the corner is completable only if it is an exact integer multiple of
that angle, and when it is not the whole subtree is empty and is cut at its root rather than
explored through its $2^{k}$ chirality branches. The orbit that had previously resisted, $(3,2)$ at
$f=8$, ran over three million nodes without terminating and now exhausts in under thirty thousand.
Since $191$ is prime it forces $m=1$, and its representation $191=3\cdot8^{2}-1^{2}$ is unique, so
no other instance arises.

The values $431$ and $587$ follow the same route at $f=12$ and $f=14$, with $33$ and $52$ orbits
respectively, and were reached by a second prune described in \cite{Obstructions}: a corner of the
uncovered region is fillable precisely when its angle is $X\alpha+Y\beta$ for integers
$X,Y\ge0$, and the search had been testing that criterion only at \emph{acute convex} corners.
Extending it to obtuse and to reflex corners --- where the interior angle is $2\pi-\theta$ and the
cosine is unchanged --- collapses the two words that had previously resisted every configuration we
tried, from searches that never terminated to $506$ and $807$ nodes.

The value $971$ is settled the same way at $f=18$.  Its $174$ surviving words fall into $87$
mirror-orbits; a transversal of $102$ instances --- the $87$ orbits, together with $15$ words the
pincer window already kills, run again as a control --- exhausts with no tiling found, the largest
subtree being under $10^{5}$ nodes.  Since $971$ is prime and $971=3\cdot18^{2}-1^{2}$ is its only
representation, no other instance arises.
...
\begin{theorem}[The exceptional set is a congruence class]\label{thm:mod12}\lab{VERIFIED}
...
\begin{theorem}[The first complete member]\label{thm:realize12}\lab{CONJECTURE}
...
\begin{corollary}[Theorem~\ref{thm:main}, congruence form]\label{cor:mod12}\lab{PROVED}
...
\begin{proposition}[Base-$\alpha$ prime exclusion, replacing {\cite[Thm.~20]{BeesonIII}}]\label{prop:isoalphaprime}\lab{VERIFIED}
...
$(125,125,198)$) and $N=70$ (base-$\alpha$, Remark~\ref{rem:seventy}) --- have since returned the
same verdict, at $1\,838\,175$, $7\,232\,464$ and $134\,631\,158$ nodes respectively; all three
values are excluded. For the base-$\beta$ target the
...
\begin{proposition}[Unsplittability, and the rigidity of the thick regime]\label{prop:unsplit}\lab{VERIFIED}
...
\begin{proposition}[Dissection basics]\label{prop:dissection}\lab{VERIFIED}
...
\begin{proposition}[Vertex figures]\label{prop:vertexfigures}\lab{VERIFIED}
...
\begin{proposition}[Corner figures]\label{prop:cornerfig}\lab{VERIFIED}
...
\begin{proposition}[$\gamma$-trap]\label{prop:gammatrap}\lab{VERIFIED}
...
\begin{proposition}[$b$ is unsplittable]\label{prop:bunsplit}\lab{VERIFIED}
...
\begin{proposition}[Corner parallelogram]\label{prop:cornerpara}\lab{PROVED}
...
\begin{proposition}[Base-$(\alpha{+}\beta)$ prime exclusion, correcting {\cite[Thm.~18]{BeesonIII}}]\label{prop:b3prime}\lab{CONJECTURE: reduced 2026-09-06 to R2 degree balance; the K|N input has no sound source (Beeson Thm 6 is refuted here) -- see lean/PAPER_MAP.md}
...
\begin{proposition}[Rationality from integrality of the invariant]\label{prop:rationality}\lab{VERIFIED}
...
\begin{proposition}\label{prop:reduction}\lab{PROVED}
...
\begin{lemma}[Cancellation]\label{lem:cancel}\lab{PROVED}
...
\begin{lemma}[Tile value]\label{lem:value}\lab{PROVED}
...
\begin{corollary}[Integrality and parity]\label{cor:int}\lab{PROVED}
...
\begin{proposition}[The invariant product]\label{prop:product}\lab{VERIFIED}
...
\begin{corollary}[Tile-similar target, citation-free]\label{cor:similar}\lab{VERIFIED}
...
\begin{proposition}[$F_1$ excluded for prime $N$, without a rationality hypothesis]\label{prop:F1free}\lab{PROVED}
...
\begin{lemma}[Non-integrality]\label{lem:nonint}\lab{VERIFIED}
...
\begin{theorem}\label{thm:iso}\lab{PROVED}
...
\begin{theorem}[Fibonacci families are the extremal ones]\label{thm:fib}\lab{VERIFIED}
...
\begin{theorem}[Scaling ladder]\label{thm:ladder}\lab{VERIFIED}
...
\begin{corollary}[The realizable set of a base-$\beta$ family is a union of multiples]\label{cor:ladder}\lab{PROVED}
...
\begin{corollary}[An infinite family of tilings in an incommensurable branch]\label{cor:elevenm}\lab{PROVED}
...
\begin{theorem}[The prime case]\label{thm:primefull}\lab{PROVED}
...
\begin{theorem}[Isosceles admissible spectrum]\label{thm:admissible}\lab{PROVED}
...
\begin{proposition}[Solvability of $e\mid(a+b-c)$]\label{prop:solv}\lab{VERIFIED}
...
\begin{proposition}[The scale parameter is a lattice]\label{prop:latticeparam}\lab{VERIFIED}
Normalise $e=g e_1$, $r=g r_1$ with $\gcd(e_1,r_1)=1$ (as $g=\gcd(e,r)$ provides), and set
$T=r_1+d e_1^2(a+2b)$. For $w=e_1u$ the two admissibility conditions of
Theorem~\ref{thm:admissible} --- $e\mid w r$ and $wr/e\equiv N\pmod 2$ with $N=dw^2(a+2b)$ ---
hold if and only if $uT\equiv0\pmod2$ (\texttt{SpectrumLattice.admissible\_iff}); the divisibility
alone is equivalent to $e_1\mid w$ (\texttt{.dvd\_iff\_e1\_dvd}). Since $u T\equiv0$ iff $T$ is even
or $u$ is even (\texttt{.case\_split}), every $u$ is admissible when $T$ is even and only even $u$
when $T$ is odd.
\end{proposition}

\begin{theorem}[Spectrum lattice]\label{thm:lattice}\lab{PROVED}
...
\begin{theorem}[Spectrum theorem]\label{thm:spectrum}\lab{PROVED}
...
\begin{proposition}[The rationality input, partly internalised]\label{prop:ratfree}\lab{PROVED}
...
\begin{proposition}[The other branches]\label{prop:otherspectra}\lab{PROVED}
...
\begin{proposition}[The equilateral identities]\label{prop:eqspec}\lab{VERIFIED}
Let $(a,b,c)$ be a $2\pi/3$ tile, $S>0$, and put $X=c+a-b$, $Y=c+b-a$, $s=3S/X$, $t=3S/Y$ and
$N=S^2/(ab)$. Then
\[
XY=3ab,\qquad st=3N,\qquad (t-s)^2+16N=q^2\ \text{ with } q=\frac{2S(a+b)}{ab},
\]
and conversely a triple proportional to $\bigl(q+(t-s),\,q-(t-s),\,2(s+t)\bigr)$ satisfies the
$2\pi/3$ relation.
\end{proposition}

\begin{proposition}[Equilateral admissibility]\label{prop:eqspecint}\lab{PROVED}
If moreover an equilateral triangle of side $S$ is $N$-tiled by that tile, then $s$ and $t$ are
positive integers, $s\equiv t\equiv N\pmod 2$, and $q$ is an integer; primitivity then pins the
scale, hence $S$.
\end{proposition}
...
\begin{proposition}[The conic identities]\label{prop:conic}\lab{VERIFIED}
Over $\Z$: if $ts=3N$ and $(t-s)^2+16N=q^2$ then $(qs)^2=(s^2+N)(s^2+9N)$, hence
$(s^2+5N)^2-(qs)^2=16N^2$, so
\[
(s^2+5N-qs)(s^2+5N+qs)=16N^2 ,
\]
an explicit factorization of $16N^2$. For the $\pi/3$ companion,
$(5N-M^2)^2-16N^2=(9N-M^2)(N-M^2)$.
\end{proposition}

\begin{proposition}[Conic form: the criteria are equivalent]\label{prop:conicequiv}\lab{VERIFIED}
Fix $N$ and let $s\neq0$ with $ts=3N$. Then
\[
(t-s)^2+16N=q^2 \iff (s^2+5N-qs)(s^2+5N+qs)=16N^2 ,
\]
so the conic condition and the factorization of $16N^2$ with $\tfrac{u+v}2-5N=s^2$ are the same
condition (\texttt{EquilateralConic.factor\_2pi3\_iff}; the return direction is
\texttt{.factor\_2pi3\_conv}, an identity after multiplying by $s^2$ and using $ts=3N$).
For the $\pi/3$ companion, $(9N-M^2)(N-M^2)=k^2$ iff $(5N-M^2-k)(5N-M^2+k)=16N^2$
(\texttt{.conic\_pi3\_iff}).
\end{proposition}

\begin{proposition}[Conic form]\label{prop:conicform}\lab{PROVED}
Hence a $2\pi/3$ tile requires a factorization $uv=16N^2$ with $u+v\equiv0\pmod2$ and
$\tfrac{u+v}2-5N=s^2$ for a divisor $s$ of $3N$ with $s\equiv 3N/s\equiv N\pmod2$; a $\pi/3$ tile
requires $uv=16N^2$ with $\tfrac{u+v}2 = 5N-M^2$ for some $0<M<\sqrt N$, $M\equiv N\pmod 2$. The
word \emph{requires} is what is not formalized: that a tiling yields integers $s,t,q$ of those
parities is Proposition~\ref{prop:eqspecint}, which quantifies over tilings.
\end{proposition}
...
\begin{theorem}\label{thm:frontier}\lab{PROVED}
...
\begin{theorem}\label{thm:frontier2}\lab{PROVED}
...
\begin{theorem}\label{thm:44}\lab{VERIFIED}
...
\begin{theorem}\label{thm:frontier3}\lab{PROVED}
...
\begin{theorem}[$63$ is realizable]\label{thm:63}\lab{VERIFIED}
...
\begin{corollary}[The cevian spectrum at $(1,2)$ is complete]\label{cor:wfamily}\lab{CONJECTURE}
...
divisibility, none now remains pending: $59$ and $66$ have likewise been exhausted by exact search,
at $1\,838\,175$ and $7\,232\,464$ nodes respectively.
\end{remark}
...
\begin{theorem}[No equilateral $105$-tiling]\label{thm:eq105}\lab{PROVED}
No equilateral triangle can be cut into $105$ congruent triangles.
\end{theorem}

\begin{proof}
By the classification of tiles admitting an equilateral tiling, a candidate tile has one angle
$\pi/3$ or $2\pi/3$, or all angles rational multiples of $\pi$; the last requires
$N=d\,k^{2}$ for some $d\mid 6$, and $105$ is none of $k^{2},2k^{2},3k^{2},6k^{2}$. The two
$\pi/3$ rows of Beeson's Table~2 at $N=105$, tiles $(7,15,13)$ and $(5,21,19)$, are excluded by
certified exhaustive search ($1\,922\,194$ and $3\,497\,208$ nodes). For a $2\pi/3$ tile
$(a,b,c)$ the area equation gives $s^{2}=105\,ab$ for the side $s$, so $105ab$ is a square; the
condition is invariant under scaling, and enumerating primitive triples $c^{2}=a^{2}+ab+b^{2}$ with
$a\le b<4000$ leaves exactly one, $(21,320,331)$ with $s=840$. That instance is excluded by
certified exhaustive search ($25\,742\,338$ nodes, maximum depth $57$, $132$ hours on ten
threads). No candidate remains.
\end{proof}

\begin{remark}[Scope]\label{rem:eq105scope}
The exhaustions are computer assistance and are labelled as such; they are not formalized. The
enumeration of $2\pi/3$ candidates is verified for $a\le b<4000$, which covers every primitive
triple the area condition admits in the range searched. This closes one line of Beeson's
Table~2 \cite{BeesonEq}; the table has $55$ lines and the others are untouched by this argument.
\end{remark}
...
\begin{theorem}\label{thm:frontier4}\lab{PROVED}
...
\begin{theorem}\label{thm:frontier5}\lab{CONJECTURE}
No triangle can be cut into $86$ or $87$ congruent triangles. In the range $81\le N\le 90$ the
realizable values include $81,82,85,89,90$; $86$ and $87$ are excluded unconditionally; $83$ is
...
\begin{theorem}[Decidability]\label{thm:decidable}\lab{PROVED}
...
\emph{every $N\le80$} is now determined, with no exception (Theorem~\ref{thm:frontier4}).
...
\begin{proposition}[Uniqueness of the base-$\beta$ representation]\label{prop:repunique}\lab{VERIFIED}
...
For $e\ge2$ the base walk is narrowed to two decompositions \emph{on the separated members}, and it
is worth being precise about how much of the residue that is. Separation, $f^2>2ef+e^2$, is the condition
$f/e>1+\sqrt2$. Of the $4438$ primes below $200\,000$ whose representation has $e\ge2$, only
$1609$ --- $36.3\%$ --- are separated; the remaining $2829$ are not reached by the result at all,
and they include every one of the smallest cases: $23, 59, 83, 131, 167, 179, 227, 251, 263, 311$.
In particular $N=83$, the first prime not settled unconditionally, has $(e,f)=(5,6)$ with
$f/e=1.2$ and is \emph{not} separated. The narrowed-walk result should therefore not be read as
covering the $e\ge2$ branch; it covers a minority of it that excludes the cases of interest
(\texttt{code/analysis/separated\_census.py}). With that said: the walk of a separated member,
meaning one with $f^2>2ef+e^2$, admits exactly three edge decompositions; the $\gamma$-trap deletes
the one with no $c$-edge, narrowing the base walk to $(0,e,2e)$ or the walls form $(f,e,e)$
unconditionally. The further exclusion of $(0,e,2e)$ --- previously asserted here outright --- is
proved only at $e=1$: the corner-chain induction behind it applies the straight-junction identity
$\pi-\gamma-\beta=\alpha$ at the points $V_k$, which for $k\ge1$ are interior, where the residue
is $\pi+\alpha$; the induction therefore stops after three tiles, and at $e\ge2$ the column
$(0,e,2e)$ survives every filter we have (full discussion in \cite{Obstructions}). So at a separated
thick member the base walk is \emph{one of two} forms, and the walls form is forced only if the
column's exclusion is supplied.
