...
$3$, conditional on the cited classification of the remaining branches. The arithmetic core is
...
triangle $(\alpha,\beta,2\pi/3)$. These coincide with the six sporadic non-equilateral $2\pi/3$
...
Hence no such tiling exists. Finally $19>3$ is prime and $\equiv3\pmod4$, so no triangle is cut into
...
$120^\circ$-triples up to $c\approx 9\times10^6$).

The entire \emph{arithmetic} layer of the proof is formalized and machine-checked in Lean~4 with
Mathlib (\texttt{lean/Erdos634.lean}); every theorem below depends only on the standard axioms
\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}, with no \texttt{sorry}:
\begin{itemize}
\item[] \texttt{k\_not\_dvd\_sum\_sub}, \texttt{M\_not\_int}: Lemma~\ref{lem:nonint}
($(c-a-b)/\sqrt b\notin\Z$), the isosceles obstruction;
\item[] \texttt{iso\_reduction\_identity}: the algebraic identity
$(c-a-b)(c+a-b)=b\,(a+2b-2c)$ of Theorem~\ref{thm:iso};
\item[] \texttt{add\_not\_prime}: the $F_1$ step that $a+b$ is never prime for a primitive
$120^\circ$-triple (Section~\ref{sec:reduction});
\item[] \texttt{prime\_three\_mod\_four\_excluded}: that a prime $p\equiv3\pmod4$ with $p>3$ is
...
\end{itemize}
The geometric lemmas (Lemmas~\ref{lem:cancel}, \ref{lem:value}) are proved in the text and checked
numerically; they are not formalized, as Mathlib has no theory of planar dissections.
