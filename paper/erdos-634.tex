...
So the residue has \emph{no} known route: linear invariants, the census, Euler and counting are all
provably vacuous, and the one arithmetic statement that would settle it ($f\mid m$) is now known to
be false. We record it as genuinely open and settle its members one at a time by exhaustive search.
\end{remark}
