...
\begin{theorem}[The base-$\beta$ family at $e=1$ does not tile]\label{thm:basebeta-e1}
...
$(X,X,Y)=(f^3,\,f^3,\,3f^2-1)$ admits \emph{no} tiling by $N=3f^2-1$ copies of the tile. In
particular the base-$\beta$ branch is closed for
\[
N=3f^2-1=11,\ 26,\ 47,\ 74,\ 107,\ 146,\ 191,\ 242,\dots
\]
--- an infinite family containing infinitely many of the at-risk primes; and for each such $N$,
$(e,f,m)=(1,f,1)$ is the \emph{only} solution of $N=m^2(3f^2-e^2)$ with $1\le e<f$, $\gcd(e,f)=1$,
so the entire base-$\beta$ sub-branch closes for those $N$.
\end{theorem}
...
the contradiction is absent. One citation remains load-bearing at a single point
(\cite[Lem.~6]{BeesonIII}, ``each side carries at least two $c$-edges'', used only to eliminate the
one-$c$-edge base composition); for $f=2,3,4$ ($N=11,26,47$) the conclusion is independently
