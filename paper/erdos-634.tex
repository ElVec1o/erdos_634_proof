...
splits into $2p-3q+r=\sigma$ and $q+r=\tau$ for integers determined by $t$. A base corner has
...
\begin{remark}\label{rem:gammascope}
The hypothesis that the corner angles are $\beta,\beta,3\alpha$ is essential, and this is why
\cite[Lem.~6]{BeesonIII} is false as quoted (Remark~\ref{rem:lem6false}): on the tile-similar target
...
Proposition~\ref{prop:gammatrap}.
...
that side presents $\alpha$ (Proposition~\ref{prop:cornerfig}), whose incident edges are $b$ and
...
Proposition~\ref{prop:gammatrap}. So $n_b=0$, and then $n_a+n_cf=f^2$, so $f\mid n_a$; write
$n_a=fk$, $n_c=f-k$.

By Proposition~\ref{prop:cornerfig} the apex tile on that side presents $\alpha$, whose incident
...
Proposition~\ref{prop:cornerpara} the first two edges lie in $\{a,c\}$ and so do the last two; with
...
Proposition~\ref{prop:cornerpara} places the $b$ away from the first two and last two positions.
...
side; by Proposition~\ref{prop:cornerpara} its partner $T_3$ presents $\gamma$ at $J_1$. So $J_1$
already carries a $\gamma$, and since a $\pi$-vertex carries at most one
(Proposition~\ref{prop:vertexfigures}), $E_2$ must place its $\gamma$ at $J_2$; inductively $E_i$
places at $J_i$, and $E_n$ places at $J_n$, a corner --- contradicting
Proposition~\ref{prop:cornerfig}. Hence $E_1$ is an $a$-edge, and the same argument at $B$ gives
...
(Proposition~\ref{prop:vertexfigures}). These types are not optional --- the dissections here are
not edge-to-edge (Remark~\ref{rem:parityfail}) --- and an earlier version of this census omitted
...
(Proposition~\ref{prop:gammatrap}) and the corner parallelogram
(Proposition~\ref{prop:cornerpara}), both proved above from the corner figures and the
...
Proposition~\ref{prop:cornerpara} holds on all six sides of the $44$- and $99$-tilings, and at each
...
$63$; and the $7\,232\,464$-node exhaustion at $66$ (Remark~\ref{rem:isobeta}).
\end{proof}
