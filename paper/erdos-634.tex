...
$3\alpha+2\beta=\pi$ at $m=1$: that both base-corner tiles lay an $a$-edge on the base and a $c$-edge
on the equal side, equivalently that neither equal side carries an $a$-edge. The companion records the
following unconditional progress on that hypothesis, and also the correction by which the hypothesis
is now stated in this form, an earlier corner-block phrasing having been inconsistent with the
$e=1$ walk theorem.
...
$p=0$ on every equal side at \emph{every member with $e=f-1$}: no equal side carries an $a$-edge, which
is Hypothesis (walls) at those members. The tile counts are $N=2f^{2}+2f-1$, and the base-$\beta$ candidates among them --- primes
...
Lemma~\ref{lem:cancel} was the one place where geometry remained, and it is now discharged.
