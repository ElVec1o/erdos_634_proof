...
gap remaining in the prime dichotomy --- though Theorem~\ref{thm:basebeta-e1} below now removes an
infinite sub-family of it, and Remark~\ref{rem:nogo} explains why the method \cite{BeesonIII} used
cannot repair the rest.
\end{remark}

\begin{theorem}[The base-$\beta$ family at $e=1$]\label{thm:basebeta-e1}
Let $f\ge2$ and let the tile be $(a,b,c)=(f,\,f^2-1,\,f^2)$ (the primitive $3\alpha+2\beta=\pi$ tile
at $e=1$). Then the isosceles triangle with base angles $\beta$ and sides
$(X,X,Y)=(f^3,\,f^3,\,3f^2-1)$ admits \emph{no} tiling by $N=3f^2-1$ copies of the tile, in each of
the following cases:
\begin{enumerate}
\item[(i)] $f=2$ (i.e.\ $N=11$) --- \emph{unconditionally};
\item[(ii)] $f\ge3$ --- \emph{conditionally on} \cite[Lem.~6]{BeesonIII} (``every side of $ABC$
carries at least two $c$-edges''), which we have not verified and which we flag as a real risk in
Remark~\ref{rem:lem6}.
\end{enumerate}
In case (ii) the family is $N=3f^2-1=26,\,47,\,74,\,107,\,146,\,191,\,242,\dots$, and for each such
$N$ the triple $(e,f,m)=(1,f,1)$ is the \emph{only} solution of $N=m^2(3f^2-e^2)$ with $1\le e<f$,
$\gcd(e,f)=1$, so the whole base-$\beta$ sub-branch closes for those $N$ --- conditionally.
Independently, $N=11,26,47$ are confirmed by exhaustive search.
\end{theorem}
...
\texttt{vertex\_apex}). Each side of $ABC$ is partitioned into whole tile edges (a supporting line
meets a convex tile in a face). At $e=1$, $m=1$ the base $Y=3f^2-1$ then admits, among coverings with
at least two $c$-edges, the unique multiset $\{b,c,c\}$ (\texttt{base\_composition\_e1}); since the
corner tile's $\beta$-vertex is flanked by $a$ and $c$ and $a\notin\{b,c\}$, the base reads $c,b,c$
and each corner tile puts its $a$-edge on the equal side. Let $P_1,P_3$ be the interior base division
points and $T_2$ the tile on the middle $b$-edge; $T_2$ carries $\alpha$ at exactly one of $P_1,P_3$
and $\gamma$ at the other. But the corner tile's own $b$-edge has both endpoints on $\partial ABC$
and is transversal there, so by the same edge arithmetic mod $f$ it is matched by a \emph{single}
tile $T^*$ whose $b$-edge is exactly that segment; the type at $P_1$ cannot be
$\{\alpha,\beta,\gamma\}$ (none of the three tiles meeting there can carry $\beta$), hence is
$\{3\alpha,2\beta\}$, which contains no $\gamma$ --- forcing $T_2$ to carry $\alpha$ at $P_1$. The
mirror argument at $P_3$ forces $\alpha$ there too, a contradiction.

The four arithmetic/analytic pillars (irrationality via Niven, the three vertex enumerations, and the
base composition) are machine-checked in Lean (\texttt{lean/BaseBetaE1.lean}, axiom-clean). The
argument is sharp: it does \emph{not} touch the genuine $44$-tiling, which is this same family at
$m=2$ --- there the base walk is $a,a,a,a,c,c,c,a$ and carries \emph{no} $b$-edge, so the driver of
the contradiction is absent.
\end{proof}

\begin{remark}[Exactly where the $e=1$ argument is conditional]\label{rem:lem6}
The step ``the base is $\{b,c,c\}$'' is where \cite[Lem.~6]{BeesonIII} enters, and it is
load-bearing. At $e=1$, $m=1$ the congruence $Q\equiv1\pmod f$ and the length $Y=3f^2-1$ leave
\emph{three} compositions, not one:
\[
\{b,c,c\},\qquad \{a^{f},b,c\},\qquad \{a^{2f},b\}.
\]
A $\gamma$-trap argument (no boundary junction carries $\gamma$ on both sides, so only a $c$-edge can
leave the $\gamma$-state) shows citation-free that every side carries \emph{at least one} $c$-edge,
killing $\{a^{2f},b\}$; and for $f=2$ the corner-chord forcing kills the rest, giving case (i)
unconditionally. But $\{a^{f},b,c\}$ --- e.g.\ the word $a\,c\,b\,a\,a$ at $f=3$, of length
$3+9+8+3+3=26=Y$ --- survives every citation-free constraint we have; only Lemma~6's ``at least
\emph{two} $c$-edges'' removes it. We flag this prominently: \cite{BeesonIII} is the paper whose
Lemma~8 we show to be \emph{false} (Remark~\ref{rem:b3prime}) and whose Theorems~14, 18, 19, 20 we
show to be unsound. We therefore do not regard case (ii) as settled, and record $N=107,191,\dots$
(the $f\ge5$ primes of this family) as resting on an unverified citation rather than on a proof of
ours.
\end{remark}
...
The $e\ge2$ residue ($N=23,59,71,83,131,167,\dots$) resists all three natural methods, and in two
cases \emph{provably}:
\begin{itemize}
...
is false. The $e\ge2$ residue therefore has \emph{no} known route: linear invariants, the census and
Euler are provably vacuous, and the one arithmetic statement that would settle it is now known to be
false. We record it as genuinely open and settle its members one at a time by exhaustive search.
\end{remark}
