...
$N=dw^2(a+2b)$ with $e\mid w(c-a-b)$ --- bounding the realizable set from outside; and, combining
the spectra with the published branch theorems and an exhaustive exact search of the four surviving
finite instances, we settle the two smallest previously undetermined values: no triangle can be cut
into $14$, or into $15$, congruent triangles. The arithmetic and combinatorial layer is
...
\[
st=3N,\qquad (t-s)^2+16N=q^2\ \text{ for an integer } q;
\]
...
$Y\mid 3S$, so $s,t\in\Z_{\ge1}$, and $st=9S^2/(XY)=9\,Nab/(3ab)=3N$. From $X-Y=2(a-b)$ we get
...
curves~\cite{LaczkovichEll}. With $7$ and $11$ excluded by \cite{BeesonSeven}, $19$ and $23$ by
Theorem~\ref{thm:main}, and $14$ and $15$ by Theorem~\ref{thm:frontier}, the smallest values
whose membership remains undetermined, to our knowledge, are $N=21$ and $N=22$.
...
in Lean~4 with Mathlib (\texttt{lean/Erdos634.lean}, twenty theorems); every theorem depends only
...
$e\mid w(c-a-b)$;
\item[] the finite arithmetic of the $N=14,15$ sweep of Section~\ref{sub:frontier}, in five
theorems: the $\pi/3$-equilateral square criterion fails at $14$ and $15$; the first tiling
equation has no admissible solution; the two equilateral factor-pair uniqueness statements; and the
boundary congruence that kills the $N=14$ isosceles candidate
(\texttt{pi3\_equilateral\_fails\_14\_15} through \texttt{iso\_ab\_congruence\_kills\_14}).
\end{itemize}
