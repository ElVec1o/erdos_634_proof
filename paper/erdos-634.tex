...
and \emph{a triangle can be cut into $44$ congruent triangles} --- the isosceles $(16,16,22)$
into $44$ copies of the $(2,3,4)$ tile, found by the search and verified exactly, to our
...
\begin{remark}[The scope of Theorem~\ref{thm:main}, stated exactly --- a correction]\label{rem:mainscope}
The exception is not cosmetic, and earlier versions of this paper did not carry it; we correct that
here. The base-$\beta$ candidates are exactly the $N=3f^2-e^2$ with $\gcd(e,f)=1$, $1\le e<f$; they are
automatically $\equiv3\pmod4$, and the smallest are
...
(Remark~\ref{rem:e2}). Asserting Theorem~\ref{thm:main} without the exception --- as we previously
did, on the strength of that citation --- is exactly the error this paper documents in
\cite{BeesonIII}, and it would be no better for being ours. The candidates are therefore settled one
at a time by exhaustive search (Table~\ref{tab:basebeta}): at present $11,23,47,71,107$ are excluded
and $59,83$ are under search. So Theorem~\ref{thm:main} is unconditional for every prime
$N\equiv3\pmod4$, $N>3$, outside the sequence above, and for $11,23,47,71,107$ besides. The
headline consequence is unaffected: $19\ne3f^2-e^2$, and more generally the two frontier claims of
...
so the prime theorem's rationality input is refereed; the general-tile rationality of
...
the base-$\beta$ isosceles target, is the exception and is discussed honestly in
...
\begin{remark}[The base-$\beta$ target: an honest gap]\label{rem:isobeta}
...
their values are treated as pending. We flag this plainly: for the base-$\beta$ target our
$3\alpha+2\beta$ prime argument rests on either \cite[Thm.~14]{BeesonIII} (whose printed proof is
unsound) or a per-value search, \emph{not} on a machine-checked general theorem --- the one genuine
gap remaining in the prime dichotomy --- Theorem~\ref{thm:basebeta-e1} removes $N=11$
...
then the \emph{unique} possible base. Two consequences, which we state plainly against our own earlier
claim. First, $B_1$ carries exactly \emph{one} $c$-edge; so \cite[Lem.~6]{BeesonIII} (``every side
...
junction is a $(1,1,1)$-vertex. This is the object to attack.
\end{remark}
...
\begin{remark}[Validation, and what the pierced corner is for]\label{rem:pierce}
The theorem is falsifiable on the genuine $44$-tiling ($m=2$; its equal sides do end with $c$), and
holds there exactly: the apex rays pair one clean $b\,|\,b$ and one mismatch $c\,|\,b$; the middle
...
\texttt{pierced\_corner\_types}, \texttt{BaseBetaWalks.lean}). This is the first \emph{interior}
forcing in the branch, uniform in $(e,f)$ and unconditional at $m=1$: every hypothetical thick-regime
tiling --- $N=59$ included --- must contain this configuration. The two continuation types are where a
full forcing attack on the thick regime now stands.
\end{remark}
...
\begin{remark}[The golden ratio, and a correction to our own reflex]\label{rem:golden}
Two consequences worth stating, both against expectation. First, Proposition~\ref{prop:unsplit} is
exactly the hypothesis the corner parallelogram needs, so Remark~\ref{rem:e2}(b) now holds with
\emph{no} proviso. Second, the case that looks easy is the loose one: $e=1$ --- which is what both
genuine tilings realize --- is the only case where an edge splits at all, while the open $e\ge2$
regime is perfectly rigid. Relatedly, one must never compare $a$ with $b$ without care:
...
$N=83$ $(5,6)$ and $N=167$ $(5,8)$ have $b<a$ --- so any argument that silently assumes $b>a$
(which the $e=1$ intuition invites) is false exactly where it is needed. Our first attempt at
Proposition~\ref{prop:unsplit} did assume it; the proof above does not.
\end{remark}
...
\begin{remark}[Validation, and the search consequence]\label{rem:rung2}
On the genuine $44$-tiling ($f^2=4>2e^2=2$, so $t=0$): the piercer spans exactly $[3,6]$ --- it
starts at the T-junction --- and its corner there is $\alpha$ to numerical precision, as forced.
The arithmetic core is machine-checked (\texttt{pre\_pierce\_dichotomy}). Beyond its intrinsic
content, the rung has an operational consequence: the apex neighbourhood is \emph{near-rigid}
(three forced $\alpha$-tiles, a forced mismatch ray, a forced T-junction, a forced piercer start),
while the base admits several walks. The exhaustive search anchors at the lowest vertex, so in the
standard orientation it explores the fat part first and meets the rigid apex last --- backwards.
Re-anchoring by \emph{flipping the target apex-down} (a reflection, so verdicts transfer verbatim)
lets the forced chain prune at the root: on the settled thick member $N=23$ the flipped search
exhausts in $3{,}954$ nodes against $19{,}677$ --- a five-fold collapse --- while on the thin member
$N=47$ the flip \emph{loses} (there the base, with its single $b$-edge, is the rigid side, exactly
as the classification predicts). The record-critical thick searches ($N=59,66$) now run apex-down.
\end{remark}
...
$V=36\in(33,44)$. This is the first result that pins an extended \emph{interior} structure of a
hypothetical thick tiling, uniformly in $(e,f)$; it is the natural root prune for the apex-down search
and the base for a rung-three attack (the forced $b^f$ wall must land on the base at coordinate
$e\cdot f^2$, whose $b$-count is bounded by base\_b\_bound). It does not, on its own, close any member.
\end{remark}
...
only on $g\mid M$ ($21,39,66,70$) were right to be demoted (Remarks~\ref{rem:seventy},
\ref{rem:isobeta}); $21$ and $39$ are now excluded by exhaustive search instead, and had we trusted
Theorem~14 the record would have been \emph{wrong}. Finally $N=99$ is itself a new realization: $99$
is not a square, a sum of two squares, nor $2n^2,3n^2,6n^2$, so it is not commensurably realizable.
\end{remark}

\begin{remark}[What the $e\ge2$ residue costs, and where it now sits]\label{rem:e2}
...
$59$, $66$, $83$ are under search. (An earlier version of this table omitted $N=83$ and claimed all
eleven were settled; both errors are corrected here. $N=83$, $(e,f)=(5,6)$, is the smallest
base-$\beta$ prime never yet searched.)
...
$59$  & $(4,5)$ & $(20,9,25)$  & thick & searching\\
$66$  & $(3,5)$ & $(15,16,25)$ & thick & searching\\
...
base to carry at least two $c$-edges (\cite[Lem.~6]{BeesonIII}) and a number of $b$-edges
$\equiv-M\pmod g$, hence at least $g-M>0$ (\cite[Lem.~45(iii)]{BeesonIII}); so $Y\ge(g-M)b+2c$. But
...
A prime $p$ that is not a base-$\beta$ candidate ($p\ne3f^2-e^2$ for all coprime $1\le e<f$) is a
number of congruent triangles into which some triangle can be cut if and only if $p=2$, $p=3$, or
$p\equiv 1\pmod 4$. The same holds for the base-$\beta$ candidates that have been settled by search,
currently $11,23,47,71,107$ (Table~\ref{tab:basebeta}); for the remaining candidates
$59,83,131,167,179,191,\dots$ the ``only if'' direction is open
(Remark~\ref{rem:mainscope}).
\end{theorem}
...
\begin{remark}[The value $70$ --- retracted to pending]\label{rem:seventy}
An earlier version of this theorem included $N=70$. Its single surviving instance is
...
regard $70$ as \emph{undetermined} pending the direct exhaustive search of the instance, which is
in progress. The value $21$, whose analogous isosceles-$\alpha$ candidate (tile $(2,3,4)$, $M=5$,
...
prime\,$\equiv1\!\pmod4$ values, and all others are excluded; in particular the contiguous
record is every $N\le 69$, extending through $80$ once the pending $70$-instance search completes.
\end{theorem}
\begin{proof}
