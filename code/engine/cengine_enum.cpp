...
struct Search {
    ...
    // ENUM_ALL (env): do not stop at the first tiling -- count every completed tiling,
    // dump each to its own file, and run the branching to exhaustion.  The complete-branching
    // property of dfs() is untouched: this only changes what happens at a completed leaf.
    bool enum_all = getenv("ENUM_ALL") != nullptr;
    long long enum_found = 0;
    void dump_enum(const std::vector<Poly>& placed) {
        std::string safe = name;
        for (size_t i = 0; i < safe.size(); i++)
            if (safe[i] == '/' || safe[i] == ':') safe[i] = '_';
        char fn[512];
        snprintf(fn, sizeof fn, "tiling_%s_enum%lld.txt", safe.c_str(), enum_found);
        FILE* f = fopen(fn, "w");
        if (!f) return;
        gmp_fprintf(f, "%s %ld %Zd\n", name.c_str(), N, QD_D.get_mpz_t());
    ...
    void dfs(std::vector<Poly>& polys, long left, std::vector<Poly>& placed) {
        ...
        if (polys.empty()) {
            if (left == 0) {
                if (enum_all) { enum_found++; dump_enum(placed); return; }
    ...
    const char* run() {
        ...
        if (enum_all) {
            static char buf[64];
            snprintf(buf, sizeof buf, "EXHAUSTED_ENUM found=%lld", enum_found);
            return buf;
        }
