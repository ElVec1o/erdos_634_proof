...
// P8: per-ENDPOINT forced edge type.  CORNER_TYPE forces both ends of a side to one type,
// which cannot express "b at one end, a at the other" -- the shape every corner figure of a
// 60-triple (and of a 120-triple) actually forces.  CT_A[s]/CT_B[s] are the types at the two
// endpoints of side s (-1 = free).  Sound: it only removes placements the corner figure forbids.
static int CT_A[3] = {-1, -1, -1};
static int CT_B[3] = {-1, -1, -1};
static bool CORNER2_PRUNE = false;
        ...
        for (int i = 0; i < 3; i++) {
            ...
            for (int s = 0; s < 3; s++) {
                ...
                if (on_segment(p, A, B) && on_segment(q, A, B)) {
                    ...
                    // P8: per-endpoint types
                    if (CORNER2_PRUNE) {
                        if ((p == A || q == A) && CT_A[s] >= 0 && idx != CT_A[s]) {
                            out.push_back(std::make_pair(-1, -1)); return;
                        }
                        if ((p == B || q == B) && CT_B[s] >= 0 && idx != CT_B[s]) {
                            out.push_back(std::make_pair(-1, -1)); return;
                        }
                    }
...
static bool make_instance_file(const std::string& path, Tile& tile, Poly& target, long& N) {
    ...
    if (fscanf(fp, "%4095s", tok) == 1 && std::string(tok) == "WALKS") {
        ...
        if (fscanf(fp, "%4095s", tok) == 1 && std::string(tok) == "CORNERS2") {
            for (int i = 0; i < 3; i++) { CT_A[i] = (int)rd_long(); CT_B[i] = (int)rd_long(); }
            CORNER2_PRUNE = true;
            fprintf(stderr, "P8 per-endpoint corner prune ON: "
                    "(A,B) = (%d,%d) (%d,%d) (%d,%d)\n",
                    CT_A[0], CT_B[0], CT_A[1], CT_B[1], CT_A[2], CT_B[2]);
        } else if (std::string(tok) == "CORNERS") {
