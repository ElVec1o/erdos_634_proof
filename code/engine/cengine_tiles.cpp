...
static FILE* g_tilef = nullptr;

#define TDUMP(r) do { if (g_dumpf) { fprintf(g_dumpf, "%c %ld", (r), d); for (int ix_ : cur_idx) fprintf(g_dumpf, " %d", ix_); fputc('\n', g_dumpf); } if (g_tilef) { fprintf(g_tilef, "K %c %ld %zu\n", (r), d, placed.size()); for (const Poly& t_ : placed) { for (const Pt& p_ : t_) { dump_qd(g_tilef, p_.x); fputc(32, g_tilef); dump_qd(g_tilef, p_.y); fputc(32, g_tilef); } fputc(10, g_tilef); } } } while(0)
#include <algorithm>
#include <cstdio>
#include <cstdlib>
#include <cstring>
...
int main(int argc, char** argv) {
    ...
    if (const char* e = getenv("CENGINE_TILEDUMP")) g_tilef = fopen(e, "w");
