...
//
// cengine_eq -- variant of cengine_iso that accepts tiles with IRRATIONAL side ratios.
//   The three side fields of a FILE: instance are read as the SQUARED side lengths (integers),
//   and each actual length is recovered as an element of Q(sqrt D) via sqrt_qd.  This admits e.g.
//   the 30-60-90 tile (squares 3, 9, 12 over D=3) and the 30-30-120 tile (squares 1, 1, 3), which
//   the integer-sided engine cannot represent at all.
//   Differences from cengine_iso, all sound (they only remove pruning, never tilings):
//     * runs_ok / run_count_ok (P7 and the semigroup run prune) are disabled -- they accumulate run
//       lengths as RATIONALS, which is false here.  Node counts differ; verdicts do not.
//   Instance files for this binary are NOT interchangeable with cengine_iso's.
...
// --- exact square root inside Q(sqrt D) -------------------------------------------------------
// Decompose a QD into integer (P, Q, Den) with value (P + Q sqrt D)/Den.
static void qd_parts(const QD& s, mpz_class& P, mpz_class& Q, mpz_class& Den) {
    if (!s.B) { P = i128_to_mpz(s.p); Q = i128_to_mpz(s.q); Den = i128_to_mpz(s.d); }
    else { P = s.B->pn; Q = s.B->qn; Den = s.B->den; }
}

// Solve r^2 = s for r = (u + v sqrt D)/(2 Den) in Q(sqrt D), r >= 0.
// Writing s = (P + Q sqrt D)/Den and A = 4 P Den, B = 4 Q Den, we need
//   u^2 + D v^2 = A,   2 u v = B,
// whose solvability is exactly: A^2 - D B^2 is a perfect square E^2, and then
//   u^2 = (A + E)/2,   D v^2 = (A - E)/2   (or the branch with E negated).
// This strictly generalises sqrt_rational, which is the special case v = 0.
static bool sqrt_qd(const QD& s, QD& out) {
    mpz_class P, Q, Den;
    qd_parts(s, P, Q, Den);
    if (sgn(Den) < 0) { P = -P; Q = -Q; Den = -Den; }
    if (sgn(Den) == 0) return false;
    mpz_class A = 4 * P * Den, B = 4 * Q * Den;
    mpz_class disc = A * A - QD_D * B * B;
    if (sgn(disc) < 0) return false;
    mpz_class E = sqrt(disc);
    if (E * E != disc) return false;
    for (int branch = 0; branch < 2; branch++) {
        mpz_class Ee = branch ? -E : E;
        mpz_class n1 = A + Ee, n2 = A - Ee;              // 2u^2 and 2 D v^2
        if (sgn(n1) < 0 || sgn(n2) < 0) continue;
        if (mpz_odd_p(n1.get_mpz_t()) || mpz_odd_p(n2.get_mpz_t())) continue;
        mpz_class u2 = n1 / 2, dv2 = n2 / 2;
        if (!mpz_divisible_p(dv2.get_mpz_t(), QD_D.get_mpz_t())) continue;
        mpz_class v2 = dv2 / QD_D;
        mpz_class u = sqrt(u2), v = sqrt(v2);
        if (u * u != u2 || v * v != v2) continue;
        if (sgn(B) < 0) v = -v;                          // 2uv = B fixes the relative sign
        if (2 * u * v != B) continue;
        if (u * u + QD_D * v * v != A) continue;
        out = qd_raw(u, v, 2 * Den);
        return true;
    }
    return false;
}
...
// ------------------------------------------------------------------ tile & instances --------
struct Corner { QD cs, sn; QD L1, M1, L2, M2; };  // two side pairs (L along, M other)
struct Tile {
    long a, b, c;
    QD area2;
    Corner corners[3];
...
struct Search {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            for (int s = 0; s < 3; s++) {
                ...
                if (on_segment(p, A, B) && on_segment(q, A, B)) {
                    ...
                    QD L2 = dot(d, d);
                    int idx = -1;
                    if (L2 == qd_frac(tile.a, 1)) idx = 0;
                    else if (L2 == qd_frac(tile.b, 1)) idx = 1;
                    else if (L2 == qd_frac(tile.c, 1)) idx = 2;
    ...
    bool run_count_ok(long L, long left_) const {
        (void)L; (void)left_; return true;   // cengine_eq: disabled with runs_ok
        if (tile.c <= 0) return true;
        long need = (L + tile.c - 1) / tile.c;
        return need <= left_;
    }
    bool runs_ok(const Poly& poly, long left_ = -1) {
        (void)poly; (void)left_;
        return true;   // cengine_eq: run lengths are not rational; prune disabled
#if 0
        size_t n = poly.size();
    ...
    void placements(const Poly& poly, int vi, std::vector<Poly>& out) {
        ...
        auto unit = [](const Pt& p, const Pt& q, Pt& u) {
            ...
            QD L;
            if (!sqrt_qd(L2, L)) {
                fprintf(stderr, "FATAL: boundary edge length not in Q(sqrt D)\n");
                exit(3);
            }
            u = Pt{d.x / L, d.y / L};
        };
        ...
        for (int ci = 0; ci < 3; ci++) {
            ...
            QD Ls[2] = {C.L1, C.L2}, Ms[2] = {C.M1, C.M2};
            for (int k = 0; k < 2; k++) {
                QD L = Ls[k], M = Ms[k];
    ...
    const char* run() {
        ...
        // s[] hold SQUARED side lengths, so cos of the smallest angle is
        // (s1 + s2 - s0) / (2 sqrt(s1 s2)) and its square is (s1+s2-s0)^2 / (4 s1 s2).
        mpz_class num = (mpz_class)s[1] + (mpz_class)s[2] - (mpz_class)s[0];
        mpz_class den2 = 4 * (mpz_class)s[1] * (mpz_class)s[2];
        cosmin2 = qd_raw(num * num, 0, den2);
...
static bool make_instance_file(const std::string& path, Tile& tile, Poly& target, long& N) {
    ...
    rd_mpz(QD_D);
    qd_sync_D();
    // cengine_eq: these three are the SQUARED side lengths.
    tile.a = rd_long(); tile.b = rd_long(); tile.c = rd_long();
    QD sd[3];
    { long sq[3] = {tile.a, tile.b, tile.c};
      for (int i = 0; i < 3; i++) {
          if (!sqrt_qd(qd_frac(sq[i], 1), sd[i])) {
              fprintf(stderr, "side length sqrt(%ld) is not in Q(sqrt %s)\n",
                      sq[i], QD_D.get_str().c_str());
              exit(2);
          }
      } }
    QD adj[3][2] = {{sd[1], sd[2]}, {sd[0], sd[2]}, {sd[0], sd[1]}};
    for (int i = 0; i < 3; i++) {
