...
struct Search {
    ...
    long N;
    std::string name;
    long long nodes = 0, node_cap = 0, prune_area = 0, prune_run = 0, prune_dir = 0;
    long maxdepth = 0;
    Semigroup semi;
    QD cosmin2;
    ...
    void dfs(std::vector<Poly>& polys, long left, std::vector<Poly>& placed) {
        ...
        if (now - last_log >= 60) {
            last_log = now;
            printf("  [%s] nodes=%lld depth=%ld max=%ld pruneA=%lld pruneR=%lld pruneP4=%lld t=%lds\n",
                   name.c_str(), nodes, d, maxdepth, prune_area, prune_run, prune_dir,
                   (long)(now - t0));
            fflush(stdout);
        }
        if (polys.empty()) {
...
int main(int argc, char** argv) {
    ...
    fflush(stdout);
    const char* r = S.run();
    printf("RESULT %s nodes=%lld maxdepth=%ld pruneA=%lld pruneR=%lld pruneP4=%lld\n", r, S.nodes,
           S.maxdepth, S.prune_area, S.prune_run, S.prune_dir);
    if (S.has_found) {
