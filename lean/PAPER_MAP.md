...
| M `prop:cornerfig` | base corner is a single β-tile, apex exactly three α-tiles, with the edge pattern | `VertexFigureReal.corner_angle_sum`, `.base_corner_figure`, `.apex_figure_real`, `TilePlacement.corner_multiplicities`, `.base_corner_counts`, `.apex_counts`, `TileAt.congruentDissection_base_corner_counts`, `.congruentDissection_apex_counts`, `CornerBaseEdgesReal.congruentDissection_base_corner_edges`, `ApexEdgesReal.congruentDissection_apex_edges` | VERIFIED — **edge-clause gap CLOSED 2026-09-09**: `ApexEdgesReal.congruentDissection_apex_edges` now supplies the apex half (each of the three real α-presenting apex tiles has corner-incident edges `{b,c}`, unordered pair, via the same `isVertex_of_localAngle_ne` technique generalized to not need uniqueness), completing what `CornerBaseEdgesReal` started for the base corner. **Both halves of the edge clause now genuinely hold of real geometry, not just angle multiplicities** — the VERIFIED label is now actually justified end to end, closing the citation gap flagged below rather than leaving it open. **Citation gap found and closed 2026-09-02**: this row's note previously claimed 'all three clauses ... are now theorems about a real dissection' — false as stated. `base_corner_counts`/`apex_counts` take `hvals : ∀ i, localAngle ∈ {α,β,γ,π,0}` as a *hypothesis*, and before tonight nothing in the corpus ever supplied it (grepped: no caller anywhere). The VERIFIED label was resting on the paper's own `\lab{VERIFIED}` tag, not on an independent check that the cited declarations' hypotheses were actually discharged — the same failure class as the ten wrong VERIFIED labels found in the 2026-08-30 audit. `TileAt.congruentDissection_base_corner_counts`/`.congruentDissection_apex_counts` now derive `hvals` for a real `CongruentDissection` (via `tile_angle_dichotomy_at_vertex` + the pre-existing but previously-unused `CongruentAngles.congruent_corner_angles`), so the label is now actually justified rather than inherited on trust. The edge-pattern clause's citations (`angle_lt_of_side_lt` etc.) are unchanged and were not audited this pass **UPDATE 2026-09-09 (tile-placement layer)**: audited now. The paper's own proof text cites `corner_beta_unique`/`corner_apex_unique` (`BaseBetaCorners.lean`) for the edge assertion — but those are PURE ARITHMETIC on the multiplicities `(p,q,r)` (angle counts), not edge-length facts; nothing in the corpus, before this pass, cited or built a theorem about which real edges the corner tile actually presents. **This is a genuine, previously-unflagged citation gap for the edge clause specifically** — the angle clause (single β-tile, three α-tiles, no γ at a corner) is soundly backed, but "the two edges of the base-corner tile at that corner are `a` and `c`" was asserted with no matching Lean content. `CornerBaseEdgesReal.congruentDissection_base_corner_edges` (new) now supplies exactly this, for a real `CongruentDissection`'s actual corner tile: its two corner-incident edges have lengths `{a,c}` as an unordered pair (built from `TileAt.congruentDissection_base_corner_tile_vertex` + a new companion to `congruent_corner_angles`, `congruent_opposite_side`, + `TilePlacement.corner_tile_edges`). **This closes the base-corner half only.** The apex half ("the edges of the apex tiles at the apex are `b` and `c`", for all three apex tiles) is NOT yet built by the same technique — a real, separate remaining gap. **Label not changed here**: flagging for review rather than silently patching a VERIFIED row; the angle clause is sound, only the edge clause's citation was never real, and even now only half of it is |
| M `prop:vertexfigures` | vertex-figure classification at a point of a tiling | `VertexFigureReal.boundary_multiplicities_cards`, `.interior_multiplicities_cards`, `.localAngle_mem`, `.boundary_figure_cases`, `.interior_figure_cases`, `AngleArithmetic.apex_forced` | VERIFIED — both the **boundary** and the **interior** cases are VERIFIED end to end, from a real figure to the classification; what is unformalized is only the paper's surrounding discussion. **Citation corrected 2026-09-10 (Rule-6 vacuity sweep).** This row previously cited `VertexFigureReal.vertex_multiplicities_real` *first*, as the real-dissection half of the claim. That theorem is **vacuous as stated**: its conclusion is `∃ p q r s : ℕ, p·α + q·β + r·γ + s·π = π`, and `(0,0,0,1)` satisfies it for any reals `α β γ` with no dissection, no point, and none of the hypotheses — `Erdos634.VacuityProbe.vertex_multiplicities_real_vacuous`, machine-checked. Its sibling `.interior_multiplicities_real` is vacuous the same way via `(0,0,0,0,1)` (`.interior_multiplicities_real_vacuous`). Neither is used anywhere else in the corpus (grepped), so nothing downstream breaks. **The label does not change**, because the row's real content was always carried by the `_cards` variants, which name the multiplicities as the actual `Finset` cardinalities of the tiles presenting each angle at `v` and are genuinely dissection-level; those are now cited explicitly, together with the two `_figure_cases` arithmetic classifiers they feed. WARNING docstrings added to both vacuous theorems; statements unchanged. Same bug class as commit `08d8da2` (`lem:basedi`, `cor:basedi2e`, `thm:walkstruct` clause ii). |
| C `lem:anglecalc`(1) | `π/2` has no representation `xα+yβ`: it would force `2x = 3` | `AngleArithmetic.no_right_angle`, `.no_perpendicular_cut` | VERIFIED — **Statement column corrected 2026-09-10**: it previously read "no piece of a dissection has a right angle", which is *not* clause (1) but the statement of `rem:norightangle` (a separate row, VERIFIED on `NoRightAngle.no_piece_has_right_angle`). Clause (1) as the paper writes it is the pure representability arithmetic, and `AngleArithmetic.no_right_angle : ∀ x y : ℕ, ¬(2*x = 3 ∧ 2*y = 2)` (one `omega`) is exactly that. The old wording paired a geometric statement with a purely arithmetic declaration under a VERIFIED label — the `lem:collar` pattern. No label change: clause (1) really is verified, and the geometric consequence really is verified elsewhere |
...
| O `rem:nilptower` | the nilpotent tower has no layer past class 2 | none | PROVED — no Lean file treats the lower central series **UPDATE 2026-09-08**: the general algebraic half is now IN Lean — `NilpotentTower.lcs_eventually_const` (if a group's lower central series is constant for one step, it stays constant from there on — pure group theory, general, no hypothesis on which step) and `.no_layer_past_class_two` (the exact `γ₂=γ₃ ⟹ γ_k=γ₂ ∀k≥2` instantiation). Zero axioms beyond the standard three; non-vacuous witness (`CommGroup ⟹ γ₁=⊥`, hence `γ₂=γ₃=⊥`) included and checked. **Label stays PROVED**: this formalizes the IMPLICATION only. The premise — that the tiling group's own `γ₂=γ₃` actually holds — is `Remark OM-tilinggroup`'s separate content, about a specific group, and remains unformalized. |
...
| M `prop:b3prime` | Beeson III Thm 8 core: no odd prime | `Beeson3NotPrime.triquadratic_not_prime` | **CONDITIONAL — BLOCKER 2026-09-05, REDUCED 2026-09-06** (was VERIFIED). **The hole is not a second defect**: `N` prime forces `gcd(M,K)=1`, and there Beeson's Lemma 21 is a one-liner (`PrimeBranchRepair.lemma21_at_coprime`), restoring Lemmas 23/26/27/28/31/32. The only step of Thm 6 still broken is its degree-balance appeal — **which is R2's defect**. Repairing R2 repairs this branch too. Separately, **every prime `N ≡ ±3 (mod 8)` is excluded outright with no Beeson input** (`2` must be a QR mod `N`), and no arithmetic repair can reach the rest. The Lean theorem is a correct conditional and is unchanged. Its hypothesis `K ∣ N` has **no sound source**: `paper/erdos-634.tex` (`rem:b3prime`) takes it from Beeson Thm 8, whose proof opens "By Theorem 7 … `K ∣ M²`, or equivalently `K ∣ N`" (`b1206.txt:2936`), and Thm 7 rests on **Thm 6, which this repo refutes** with the zero-axiom `CevianTiling63` (63 copies of `(2,3,4)` tiling `(21,18,24)`, a genuine `(2α,β,α+β)` target, `(M,K)=(3,6)`: `6∤9`, `6∤63`). Without `K ∣ N` the equation `N=2K²−M²` excludes no prime (7, 17, 23, 47, …). Since excluding this scalene target is a step behind "prime ⟹ base-β", **that reduction carries an undischarged dependence on a refuted theorem**. See `B3PrimeBlocker.lean`. NOT affected: `fourcomp_not_prime` (the `(2α,α,2β)` target), which needs no divisibility input. VERIFIED. **STRENGTHENED 2026-09-06** (`FourCompCongruence.lean`): the existing `fourcomp_not_prime` is a *triviality given its hypothesis* — `N = (2f²−e²)(3f²−e²)k²` hands you the factorization — and it is reached only through Beeson III **Theorem 11, which is misprinted** (statement says `ℓ = lcm(a,c)`, proof uses `lcm(a,b,c)`; only `lcm(a,c)` reproduces Beeson's own Figs 12/13/14, checked at `N = 77, 442, 1288`). The pre-Theorem-11 equation `N·R² = M²·A·B` (`A = 2f²−e²`, `B = 3f²−e²`, `R = (f−e)(2f+e)`) carries **no divisibility hypothesis on `M`**. Standard coprime factorization splits it into `(A,B) = (Nu²,v²)` or `(u²,Nv²)`; the two congruence obstructions that kill both branches are now formalized zero-axiom: `B_never_square` (mod 3: `3f²−e² = v²` forces `3∣e` and `3∣f`) and `A_square_forces` (mod 4: `2f²−e² = u²` ⇒ `3f²−e² ≡ 2`), combining to `fourcomp_no_odd_prime`. Theorem 11 is nowhere in that chain. **Scope, honestly**: steps (1)–(3) — `gcd(A,B)=1` plus `v_N`-parity coprime factorization — are NOT formalized; the branch is supplied as a hypothesis, and a witness (`e=f=1`, `N=2`) is exhibited so the conditional is not vacuous. **COMPLETED 2026-09-06 (room `fc`)**: steps (1)-(3) are formalized too — `FCStep1.gcd_A_B` (two-line Bézout certificate: `gcd(A,B)` divides `B−A=f²` and `3A−2B=−e²`), `FCStep2.mul_eq_prime_mul_sq_of_raw` (step 2, by a `gcd(M,R)` extraction — **this repairs a genuine non sequitur in Beeson's own proof** at `b1206.txt:3563`, which argues "`p ∤ N` since `N` is prime"), `FCStep2.split_of_mul_eq_prime_mul_sq` (step 3). Audited non-vacuous with a negative control (`FCAudit.A_square_forces_needs_coprimality`: false at `e=f=2` without coprimality). **TWO GAPS REMAIN; THIS IS NOT THEOREM 12.** (a) Thm 12 concludes "`N` is not prime"; this chain concludes `N = 2`, **and 2 is prime** — excluding `N=2` was stated NOWHERE in the corpus. **GAP (a) CLOSED 2026-09-09 (`FCTwoTiles.lean`), by an argument disjoint from the Thm 11/12 route and from all `e,f,M,K` arithmetic.** A 2-piece triangle dissection is a cevian cut; at the interior cut point the two pieces contribute `θ` and `π−θ`, both tile angles; if `θ ≠ π/2` they are distinct tile angles summing to `π`, forcing the third to be `0`; so **the tile has a right angle** (`angles_at_interior_point_force_right`). But a `3α+2β=π` tile has angles `(α, β, 2α+β)` and **none is `π/2`** (`three_alpha_two_beta_no_right_angle`: `α=π/2 ⇒ 3α>π`; `β=π/2 ⇒ α=0`; `2α+β=π/2` with `3α+2β=π` ⇒ `α=0`). Hence `no_two_tile_dissection`, zero axioms beyond the standard three, no `sorry`. **Scope**: the plane-topology step "2 pieces ⇒ cevian cut" is not formalized; it is the structure `TwoPieceCut`, the interface a real `CongruentDissection` with `N=2` must supply — and it is **exhibited non-vacuously** by `rightIsoscelesCut` (the isosceles right triangle split by its altitude), so this is not a `False → False`. The exclusion does not use the target shape, so it covers **all five** `3α+2β` target shapes, not only `(2α,α,2β)`. Gap (b) is untouched, so this row does not move. (b) That `N·R² = M²·A·B` holds for a real dissection is unformalized (blockers 1 and 2: tile-placement layer, scale map). Further: with `R` pinned to `(f−e)(2f+e)` the hypothesis set is **empty for every coprime `f < 40000`** (`w ∤ R` at the unique hit `(647,2993)`: `R=15561018`, `w=15213571`, remainder `347447` — verified by hand), so `chain_unconditional` is non-vacuous only because `R` is free; state that whenever it is cited. **No promotion.** Also corrected: the earlier claim that Beeson Thm 11 makes the old route unsound is WITHDRAWN — Thm 10, Thm 11's statement and Thm 12 all use `lcm(a,c)` consistently; the misprint is confined to one recap sentence (which carries two errors, the `lcm` and a sign flip). **Note 2026-09-03**: `Beeson3NotPrime.lean` also has `.fourcomp_not_prime`, Beeson III Thm 12's arithmetic core for the *other* scalene `3α+2β` target shape, `(2α,α,2β)` — read `erdos-634.tex:404-416` (`rem:b3prime`) closely: it cites both `triquadratic_not_prime` (Thm 8, this row's target `(2α,β,α+β)`) and `fourcomp_not_prime` (Thm 12) side by side for the branch's two scalene targets. `rem:b3prime` itself carries no `\lab{}` tag (a remark, not a tracked statement) and neither scalene target has its own named proposition — `prop:b3prime` (this row) and `prop:isoalphaprime` cover the *other* two target shapes of the same `3α+2β` branch (isosceles `α+β` and isosceles `α`), so `fourcomp_not_prime` has no `\lab`-tracked row of its own to attach to. **Corrected a real citation error**: `fourcomp_not_prime` was previously mis-cited (below, now removed) against `prop:reduction`, an unrelated `γ=2π/3` case-analysis statement it has nothing to do with — see `prop:reduction`'s correctly-cited row further down (`InvariantProduct.*`, line ~442). Duplicate/wrong row deleted from this table; no label changed by this correction, only a stale citation fixed. |
...
| C scale break | `side_no_b` fails at m = 2, every member | `ScaleBreak.side_no_b_fails_m2` (`.side_walk_m2` is its length identity) | **VERIFIED — citation repaired 2026-09-10.** The row previously cited only `side_walk_m2`, which is the ℕ identity `e(ef) + f(f²−e²) + f·f² = 2f³` and says nothing about `side_no_b`: it names no `b`-count, no γ-trap, and is not in that lemma's hypothesis shape. The other half was `side_walk_m2_has_b`, the identity function `(h : 1 ≤ f) ↦ h : 0 < f`, which asserts nothing at all (now annotated CONTENT-FREE in the file). `side_no_b_fails_m2` (new, zero axioms beyond the standard three, no `sorry`) states the failure in the exact hypothesis shape of `BaseBetaWalks.side_no_b` — `B + e² = f²`, walk equation `P(ef) + QB + Rf² = 2f³`, γ-trap `1 ≤ R` — and exhibits `(P,Q,R) = (e,f,f)` with `Q ≠ 0`, for every `1 ≤ e < f`. |
...
| C `thm:basebeta-full` | no base-β instance at m = 1 | `BAdjacency`, `Rigidity`, `W2Core`, `MidTriangle`, `SurplusLattice`, `GeneralPillars`, `MasterLemmas`, `Pentagon`, `PentagonLemma`, `AngleArithmetic` | **CONDITIONAL** on the complete-corner-wall hypothesis (companion `hyp:walls`) | **The blocker is NOT formalization.** It is an unwritten mathematical step: that no base corner is starved or broken. The companion's own `rem:cleanfail` calls it "the single highest-value open step in this branch". Note also: of the files listed, only `Pentagon`, `PentagonLemma` and (since 2026-09-10) the filler identity of `GeneralPillars` — now `FillerGeneral.filler_b_general_ef` — are general in (e,f); the rest are per-member instantiations, and `MasterLemmas` contains no e = 1 member at all. **VACUITY on the family `e = f−1` (2026-09-12, `EpredColumnKill.walls_refuted_epred`)**: `hyp:walls`'s block form is unsatisfiable at every `e = f−1` member given the papers' own PROVED `p = 0` on both sides (`thm:efminus1`): the east foot must be a `c` but the east corner tile lays `a` (`p_east = 0`), and the west `a^f` prefix plus the forced final `a` needs `f+1` letters `a` out of `x ≤ f`. Same phenomenon as `WallsCircular` at `e = 1` (there from `thm:e1reduce`), by a different mechanism. So this theorem's conditional exclusion of `N = 83, 179, 263, 311, …` (every prime of the family) has no content beyond `WallsCircular.conditional_is_vacuous`, exactly as at `e = 1`. Rests on `thm:secondc` (PROVED, not VERIFIED). |
...
|---|---|---|---|
| M `rem:memberdep`, C `thm:notuniform` | Δ₂ does not tile at (1,3) | 43,541,916 nodes | PROVED (computational) — **paper tag corrected 2026-09-10**: `thm:notuniform` carried `\lab{CONJECTURE}` while this row, its structural twins in this same table (`thm:dichotomy`, `rem:e2strip`, `rem:zhangsharp`, all `\lab{PROVED}` / PROVED (computational)), `data/SETTLED.tsv` row 16 (`basebeta 1 3 2 104 "3 8 9" NO_TILING`), the obstructions duplicate `OM-memberdep`, and the main paper's own **abstract** (`erdos-634.tex:87`, "the scale-2 target admits no tiling at all (N=104 on the tile (3,8,9), by exhaustive search)") all assert it flatly. Nothing in the statement is conjectural: the (1,3) half is the exhaustion, and the (1,2) half — Δ₂ exists — is `Tiling44Bridge.dissection`, VERIFIED. Flipped to `\lab{PROVED}` to match `thm:dichotomy`. Its computer-assistance caveat is the standing certified-search-format blocker, shared with every other row of this table |
...
| O `prop:chorddecomp` | flush total ∈ `{0,21,40,42,49}`; a straddler exists; two if none is flush | `ChordDecomp.flush_classification`, `.straddle_ne_nil`, `.two_straddlers_of_no_flush` | the `ChordTrace` fields: traces cover the chord with disjoint interiors, flush traces are whole edges |
...
| C `cor:wallsf2e` | The walls form at $e=1$, $f\ge3$ | `SideNoB.side_no_b_uncond`, `SideNoB.side_quantized`, `SideNoB.c_corner_forces_side_a` | *(`c_corner_forces_side_a` is `SideNoB.lean`'s own side-parameter fact — `0 < f·p → 1 ≤ p`, i.e. a `c`-corner fails `hyp:walls`'s `p=0` side condition — genuinely relevant here, not a mis-citation. **Note 2026-09-04**: the stale cross-reference to `lem:shadow`'s row was removed; `lem:shadow`'s own citation was corrected to `OrderForcing.strips_tall`/`.shadow_footage_e1` and no longer mentions this theorem at all, so the old pointer was dangling.) **Correction 2026-09-10**: the `lem:ccornerside` row's 2026-09-09 note that this row "still inherits `cor:basedi2e`'s separate blocker" is stale — `cor:basedi2e` went VERIFIED 2026-09-03. Checked directly: `cor:basedi2e` leaves two base-decomposition branches, `(0,1,2)` and `(f,1,1)`; excluding `(0,1,2)` needs `BaseCountsE1.lean:69-76`'s three-letter argument, which itself needs `prop:cornerpara` ("the first two and last two edges lie in `{a,c}`") — the deeply blocked corner-`b`-edge-straddle row (line 411 above), not `cor:basedi2e`. So this row's real residual is `prop:cornerpara`, not `cor:basedi2e`.*
| C `lem:anglethreshold` | The angle threshold | `cos_alpha_closed` |
| O `lem:avgen` | The $\alpha$-vertex gap, general | `alpha_vertex_gap_gen` |
| C `lem:basedi` | The thick base dichotomy | `base_dichotomy_thick` |
| C `lem:basetri` | the base's three edge decompositions at a separated member | `CChord.base_trichotomy` | VERIFIED — the declaration is the statement: any `(x,y,z)` solving the base walk at a separated member is one of the three. **Note 2026-09-10**: `CChord.base_trichotomy` is a *general* theorem in `(e,f)`, not an enumeration; the old "enumerated for every separated coprime pair with `f < 24`" phrasing understated it (that enumeration was a cross-check, not the Lean content) |
| C `rem:closepairbase` | close pairs admit further decompositions with `y > e` | `Frontier.base_solutions_finite`, `.close_pair_37_no_extra_column`, `.base_column_y_form`, `.base_column_reduction`, `.base_column_k_one`, `.base_column_k_bound`, `.k_bound_sharp`, `.column_x_congruence`, `.one_column_per_k` | **STATEMENT FALSE AS WRITTEN — corrected 2026-09-10; HEURISTIC → PROVED after the correction.** Two separate findings. (i) *Citation*: the row said `none` and the `none`-scan listed it among the "16 genuinely uncited", but `Frontier.lean:318–395` is precisely this cluster (its own docstring cites `rem:closepairs`); the companion's `rem:closepairs`/`rem:columnreduction` carry the same content and were **untracked** (no `\lab{}`, no row) — rows added below. (ii) *Falsity*: "close pairs admit in addition finitely many decompositions with `y > e`" reads as existence at every close pair, and that is false. At `(e,f) = (3,7)` — a close pair, `49 ≤ 42 + 9` — the base equation `21x + 40y + 49z = 414` has **no** solution at all with `y > 3`, before any of `rem:closepairs`'s three unconditional filters (`Frontier.close_pair_37_no_extra_column`, by `interval_cases`+`omega`, axiom-clean). Brute enumeration over every coprime close pair with `e ≤ 25`, `f ≤ 59`: **82 of 282 members admit none**, `(3,7)` smallest — consistent with `rem:columnreduction`'s own "198 members carrying survivors", which is the accurate phrasing. What IS true and now proved: finiteness (`base_solutions_finite` — all three edge lengths positive, so the solution set sits in a box; this is the remark's "immediate from the walk equation") and the form `y ≡ e (mod f)` (`base_column_y_form`, unconditional given `gcd(f,e²)=1`). Paper text reworded accordingly; the residual disclaimer — the count per member is a computation, not uniform in `(e,f)` — keeps the label at PROVED, not VERIFIED. |
| C `rem:closepairs` | at a close pair the base may admit extra columns, all with `y = e+kf`; what the three unconditional filters leave | `Frontier.base_column_y_form`, `.base_column_k_bound`, `cornerpara_filter` | **NEWLY TRACKED 2026-09-10** — the remark carried no `\lab{}` tag and no row, so it was invisible to `code/census.sh` while holding the content `Frontier.lean:318-395` was written against. Now `\lab{PROVED}`. Clause 1 (`y = e+kf`, i.e. `f ∣ y-e`) is VERIFIED and unconditional (`base_column_y_form`); the size bound `k ≤ 2ef/(f²-e²)` and its vanishing at separation is `base_column_k_bound`'s regime; the filter counts ("54 of the 62 extra columns", `e ≤ 9`, `f ≤ 15`) are measurements, and the `thm:n1` filter is itself CONJECTURE at `e ≥ 2` — hence PROVED, not VERIFIED. **Text corrected the same day**: "the base admits extra columns" was a universal existence claim and is false (82 of 282 close pairs with `e ≤ 25`, `f ≤ 59` admit none; `(3,7)` smallest, `Frontier.close_pair_37_no_extra_column`); reworded to "may admit". |
| C `rem:columnreduction` | the extra columns in closed form: the reduction identity, the `k=1` pinning, both `k`-bounds, one column per `k` | `Frontier.base_column_reduction`, `.base_column_k_one`, `.base_column_k_bound`, `.k_bound_sharp`, `.one_column_per_k`, `.column_x_congruence` | **NEWLY TRACKED 2026-09-10** — also untagged and unrowed, and it is the most heavily formalized remark in the close-pair cluster: all six named declarations exist and are axiom-clean, and the remark cites each by name. `\lab{PROVED}` rather than VERIFIED for two reasons: `k_bound_sharp` consumes `x+z ≥ 4` from the corner parallelogram (`prop:cornerpara`, PROVED, tile-placement blocker), and the closing clause "there is exactly one surviving column for each `k` in range" is stated for the chain `x = x₀+tf`, `z = z₀-te` whose Lean content is the criterion `one_column_per_k`, not the uniqueness itself — plus the enumeration figures (655 columns, 198 members, `e ≤ 25`, `f ≤ 59`) are measurements. |
...
| O `lem:charge` | The mirrored piece is charged | `mirrored_left_junction`, `escape_charge` |
...
| C `lem:parity` | Straight-figure parity | `census_parity`, `CornerAnglePermTarget.congruentDissection_straight_parity`/`.congruentDissection_straight_parity_isosceles` | **VERIFIED 2026-09-10.** `census_parity`'s hypothesis (the census α-identity) is now discharged for a real `CongruentDissection` by `CornerAnglePermCensus.census_alpha_sum` + `apex_fibre_card`/`base_fibre_card` (pre-existing, never wired to this row). `congruentDissection_straight_parity_isosceles` concludes `(S+N) % 2 = 1` with `S` split as the `(1,1,1)`/`(3,2,0)` fibre counts, matching the paper statement word for word. `lake build Erdos634.All` clean, no `sorry`, axioms `[propext, Classical.choice, Quot.sound]`. Conditional on `hirr` (`α/π` irrational) as a hypothesis, same standing conditionality as `GammaTrap.congruentDissection_gammatrap` (already VERIFIED) — not a new gap this row introduces. |
...
| O `lem:solitary` | The crossing kill, and solitude of branches | `crossing_tangency` |
...
| ~~C `prop:a2branch`~~ | ~~The $A_2$ branch dies~~ | — | **Superseded — see the corrected row below** (citation list here omits `anti_brick_side`, which the paper's own proof, `companion.tex:1902-1918`, cites as a fifth ingredient; the other row lists all five). |
...
| O `prop:straddlegen` | The general form | `ChordDecomp.area_never_integral`, `ChordDecomp.coprime_tight` |
| C `prop:tightside` | The $p=2$ side is forced | `p_two_single_c`, `pi_vertex_with_gamma` |
| O `prop:unify` | The chain machinery is member-independent | `fill_beta`, `fill_alpha`, `fill_alpha_beta`, `partition_jb_gen` |
...
| O `lem:ccornerside` | A $c$-corner carries a side $a$-edge |
| O `lem:climberdetect` | Climbers detect deviation |
| C `lem:columnlines` | Corner lines are column lines |
| O `lem:cornerstep` | The corner step, unconditional at every member |
| O `lem:endpoints` | Endpoint angles |
...
| C `prop:n1fromwalls` | proposition | `EpredColumnKill.column_zero_side_word`, `.column_zero_dies_of_two_c` | **Checked 2026-09-08**: its entire two-line proof depends on `lem:ccorner` ("the column `(0,e,2e)` forces `p≥1` on each equal side") — blocked transitively, same reason. **2026-09-12, family `e = f−1` only**: the assumed `p = 0` is PROVED there (main paper after `cor:pbound`; companion `thm:efminus1`), so the proposition is unconditional on that family. Lean now has the arithmetic: a side with `P > 0` at `e = f−1` reads exactly `a^f c` (`column_zero_side_word`), killed by `n_c ≥ 2` (`column_zero_dies_of_two_c`). Two geometric links remain and are named as hypotheses: `hPpos` (corner lays `c` on the base ⟹ the side chain's `P > 0`, the `lem:ccornerside` placement step) and `hR : 2 ≤ R` (`thm:secondc`'s `a`-exclusion). Note the killed configuration — one `c` at the apex, an `a` below it — *is* `thm:secondc`'s open case, so the column's death and `thm:secondc` are one open fact in Lean. Label stays CONJECTURE (general `e`). |
| C `prop:rung2` | Rung two: the pre-piercer chain |
| C `prop:widecol` | Wide columns, all $(e,f)$ |
| C `thm:chain` | The $b$-run orientation lemma |
| C `thm:e1family` | The $e=1$ base-$\beta$ family: no tiling of the $(1,f)$ target by $(f,f^2-1,f^2)$ | `E1BaseWord.e1_family_f_ge_3`, `.e1_family_f_ge_3_congruent`, `.e1_family_f_ge_3_of_sides`, `E1NormalForm.normal_form`, `MarchCompose.base_word_dies` | Paper tag CONJECTURE ("effective, the strip-and-column exposition being deferred"). **Lean, 2026-09-12 (sequential step 6): the statement is an UNCONDITIONAL theorem for `f = n ≥ 3`.** `E1BaseWord.e1_family_f_ge_3_congruent`: for every `N` and every `CongruentDissection N` whose target is congruent to `baseBetaTarget 1 n` and whose model is congruent to `modelTri n` (the tile `(n, n²−1, n²)`), `False`; the only other hypothesis is `3 ≤ n`. `e1_family_f_ge_3` is the normal-position form (`htgt`, `ModelData`, `AngleData`), `e1_family_f_ge_3_of_sides` takes the model by its three side lengths (SSS). Hypotheses individually witnessed (`MarchCompose.bundle_witness`, `E1NormalForm.bundle_f4`, `.target_self_congruent`, `.model_self_congruent`, `E1BaseWord.hyps_f4`); `#print axioms` standard three on every theorem; no `sorry`; `lake build Erdos634.All` clean. What step 6 added over step 5's conditional: (1) `thm:e1reduce`(ii) is no longer assumed — the base chain is built from `WallEndpoints.chain_endpoints` on the base line (`base_chain`: `[0, L]` exactly partitioned into whole tile edges of `D` with lengths in `{f, f²−1, f²}`), its counts are read on the *actual* chain (`base_counts_of_chain`), its end letters are not `b` (`first_edge_not_b`, `last_edge_not_b`), and the four words this leaves outside `base_word_dies` are killed: `a^p b a^q` (`word_aba_dies`), `c a^q b a^r` (`word_c_p0_dies`), `c b c` (`word_cbc_dies`), `a^p b a^q c` (`word_bfirst_r0_dies`, by `reflCD`); the one new geometric fact is at the corner `c`-slot, where there is no filler: the cap's far vertex `(1/2, h_b)` is outside the target (`corner_bCapM_dies`) and the overshoot's run `[K, A]` is clamped at `A` by the target boundary — `run_partition`'s clamp (c), `prop:cornerpara`'s own "extension leaves ABC" step in coordinates (`corner_ext_outside`, `cslot_run_kill_outside`, `corner_bOverM_dies`); (2) normal position and the model labelling are derived (`E1NormalForm.normal_form`, `normalFormC`, `congruent_modelTri_of_sides`). **Label NOT moved, by the rule (VERIFIED = the paper statement as written is the Lean theorem)**: the paper says `f ≥ 2`; `f = 2` (`N = 11`) is outside the Lean theorem — `BaseCountsE1.base_counts` needs `f ≥ 3` (at `f = 2` the walk equation also admits `n_b = 3`) — and rests on the 135-node search (`tab:basebeta`), not formalized; the "in particular" prime clause composes with `thm:mod12`/`prop:repunique`/`thm:main` outside the theorem. The paper's own CONJECTURE tag is now contradicted for `f ≥ 3` by a machine-checked proof along a route (the march: `MarchInduction` → `MarchSlots` → `RunPartition` → `MarchCompose` → `E1BaseWord`) other than the deferred strip-and-column exposition; re-tagging the paper is the author's decision. |
...
| M `prop:conicform` | the conic form as a criterion, with divisor and parity conditions, equivalent to `prop:eqspecint` | `EquilateralConic.*` | PROVED — the identities are VERIFIED; the divisor and parity conditions come from the integrality of `s`, `t`, `q`, which is `prop:eqspecint`, and the equivalence needs the converse direction **CORRECTED 2026-09-08**: this row's stated blocker was wrong. `prop:conicequiv`'s `factor_2pi3_iff`/`conic_pi3_iff` (`EquilateralConic.lean`, VERIFIED) already prove the converse direction (`.factor_2pi3_conv`) — checked directly against the Lean source. The paper's own proof text for `prop:conicform` (`erdos-634.tex:1772-1782`) names the actual remaining gap explicitly: "the word *requires* is what is not formalized: that a tiling yields integers `s,t,q` of those parities is Proposition `eqspecint`, which quantifies over tilings" — a geometric quantifier, not the converse. Label stays PROVED; only the description of the blocker is corrected. |
| M `prop:cornerpara` | Corner parallelogram none yet wired | **PROVED, unchanged.** 2026-09-07: the room's one named open link — "the corner tile's two edges at the base corner lie one along each of the two target sides" — has its GENERAL geometric lemma now VERIFIED: `Geometry.wedge_forces_ray_eq` (`CornerParaLink.lean`). Two wedges at a point with the same unsigned angle β∈(0,π), the sub-wedge's boundary directions ORIENTED-contained in the big wedge's oriented-angle range (unsigned equality alone is insufficient — a mirror-image sub-wedge shares the unsigned angle but never coincides; the oriented containment fixes this), force the two ray-pairs to coincide, in one of the two matching orders (mirrors `c_corner_side_a`/`a_corner_side_c`'s case split). Zero axioms beyond the standard three; witnessed non-vacuous. **NOT wired into `TileAt.lean`'s actual `Tri`/`CongruentDissection` structures** — that needs translating the project's vertex/edge representation into vector form and discharging the oriented-containment hypothesis from real dissection geometry (containment of `T_A`'s carrier in the target's corner wedge). Mechanical but unattempted. Label unchanged: this is one previously-missing general ingredient, not a closed route. **2026-09-12 (sequential step 6)**: the proof's boundary-clamp step — "a straddling edge would extend beyond `P` or `Q` along the line, and the extension leaves `ABC`" — is realised in coordinates for the `c`-corner of the `e = 1` target: `E1BaseWord.corner_ext_outside` (the open extension `(A, A')` of the corner tile's `b`-edge beyond its side-vertex `A` lies outside the target) fed to `RunPartition.not_straddles_of_outside` (clamp (c)) in `E1BaseWord.cslot_run_kill_outside`, which kills the corner overshoot without any filler. The proposition itself (the `b`-chord matched by *exactly one* tile; the parallelogram) is still not proved; the recorded dead end (`BaseBetaWalks.lean:797-808`, the parity argument) is a different route and is untouched. At `e = 1`, `f ≥ 3` the statement is now vacuous (no tiling exists, `e1_family_f_ge_3`); its content stands for general `(e, f)`. Label unchanged. |
...
| M `prop:eqspecint` | in a tiling, `s`, `t`, `q` are integers with `s ≡ t ≡ N (mod 2)` | `EqSpecInt.st_eq_three_N`, `.sq_identity`, `.conic_of_sides`, `.diff_of_sides`, `.eqspecint_core` | PROVED — **citation corrected 2026-09-10**: this row said `none` and the "16 genuinely uncited" scan (line ~4075) inherited that, but `Erdos634/EqSpecInt.lean` was built for this very row on 2026-09-04 and the scoping note at line ~4091 already says "citation changes from `none` to `EqSpecInt.*`" — only the table row was never updated. **Extended 2026-09-10**: `diff_of_sides` derives the difference relation `2N(a−b) = S(t−s)`, which `sq_identity`/`conic_of_sides` previously took as a hypothesis — multiply `X − Y = 2(a−b)` by `st`, use `sX = tY = 3S` and `st = 3N`, cancel `3` over `ℤ`; no nonvanishing hypothesis needed. `eqspecint_core` assembles the whole arithmetic half from exactly the data a tiling supplies (`XY = 3ab`, `X − Y = 2(a−b)`, `sX = tY = 3S`, `S² = N·ab`, `Sq = 2N(a+b)`) and outputs both relations `EquilateralConic.*` consumes. Blocker unchanged and named: that a tiling supplies `X ∣ 3S`, `Y ∣ 3S` and the area identity (**tile-placement layer**), and the positivity/parity clause `s ≡ t ≡ N (mod 2)`, which routes through `cor:int`. No flip. |
...
| M `thm:eq105` | no equilateral 105-tiling | none | PROVED — an exhaustive computation (`code/analysis/eq105_candidates.py`); formalizing it needs the search certified, which no format in this project supports **UPDATE 2026-09-07**: the proof has THREE branches (rational-angle, `π/3`, `2π/3`); one is now closed unconditionally with no search — `Eq105Rational.eq105_not_rational_branch` (`105 ≠ d·k²` for any `d∣6`), decidable finite arithmetic, zero axioms beyond the standard three. **The other two branches genuinely need the certified-search format** (Beeson Table 2's `π/3` rows, 1.9M/3.5M nodes; the `2π/3` triple enumeration, 25.7M nodes) — that format does not exist in this project, so **`thm:eq105` stays PROVED**: this closes one third of the case split, not the theorem. |
...
| O `prop:nogoauto` | the junction automaton is consistent | none | PROVED — a finite check over the automaton; not transcribed to Lean **UPDATE 2026-09-08**: the general automaton fact is now IN Lean — `JunctionAutomaton.only_forbidden_pair_is_gg` (the only inadmissible pair, over all 9 pairs of angle kinds, is `(γ,γ)`), decided directly against the two explicit figure multisets `{γ,α,β}` and `{3α,2β}`. Zero axioms beyond the standard three; two genuine non-vacuity witnesses checked (`(α,β)` admissible via `{γ,α,β}`; `(α,α)` admissible via `{3α,2β}` but **not** via `{γ,α,β}`, confirming the instance is not degenerate). **Label stays PROVED**: the row's stated corollary — the specific `(e,f)=(3,7)` count of 10,560 surviving configurations over all 120 words and `2¹¹` orientations — is a member-specific enumeration over actual boundary words, not attempted here. |
...
| O `rem:spectral` | spectral invariants of the dual graph | none | PROVED — spectral graph theory over the dual; no Lean development **UPDATE 2026-09-08**: the worked example's numeric claims are now KERNEL-VERIFIED against `Tiling44.tiles` — and they settle the 2026-09-04 TENSION row (line ~4255) against the paper. `DualGraph44.totalSharedEdges_eq_45` confirms 45 shared edges (matches the paper). `DualGraph44.degree_distribution` proves the ACTUAL distribution is `{1:12,2:18,3:14}`, and `.paper_distribution_wrong` proves the paper's claimed `{1¹¹,2²⁰,3¹³}` does **not** hold of this object — both by `decide` directly against the certified coordinates, zero axioms beyond `propext`. This upgrades `code/dualgraph44_count.py`'s finding from a Python computation to a kernel-checked fact: **the paper's worked example has an error**, not the script. **UPDATE 2026-09-08 (2nd pass)**: the connected-component count is now also KERNEL-VERIFIED — `DualGraph44.component_sizes` proves the actual sizes are `[2,4,8,14,16]` via a decidable BFS-reachability closure (`componentOf`, `stepClosure`) over the same certified `sharesEdge` relation, and `.paper_distribution_wrong`'s sibling `.paper_component_sizes_wrong` proves the paper's claimed `{4,8,16,16}` does not hold. Same TENSION row, same resolution direction: the paper's worked example, not the script. **Label stays PROVED**: `rem:spectral`'s decisive clause (a spectral criterion's constraints are exactly `prop:globalsys`'s vertex-figure counts, which admit prime solutions) is untouched by any of this, and is what any future Lean formalization should target — this file only settles the worked example's numbers. |
...
geometric argument. **Format built 2026-09-13** (see the dated section at the foot): `gen_tree.py`
+ `CertNode.lean` turn a refutation tree into one theorem per node; Lemma P's 13 nodes are
`LemmaPTree.node_0..12` and compose to `LemmaPTheorem.lemmaP_no_dissection` (standard axioms) —
the `a c b`-prefix exclusion at `N = 23` is kernel-checked with no search in the trust base. The
`aabbcca` tree (188 nodes) is generated by the same script into `Word_aabbcca0..8.lean`; its build status is recorded there.
...
|---|---|---|---|
| ~~C `lem:parity`~~ | ~~straight-figure parity~~ | — | **RESOLVED 2026-09-10, row removed from here — see the VERIFIED row above.** The census α-identity is now discharged directly (`CornerAnglePermCensus.census_alpha_sum`+fibre cards) without going through `lem:census`'s own PROVED status — `lem:parity`'s paper statement only needs the α-identity, not `lem:census`'s full four-clause conjunction, so it does not actually inherit `lem:census`'s blocker as this row previously assumed. |
| C `lem:pentagon` | the middle region of the `(0,e,2e)` walk admits no tiling | `Pentagon.no_partition` | the declaration is the arithmetic non-existence of a partition; "admits no tiling" quantifies over dissections of a region that is not a `Tri`. **Corrected 2026-09-09**: the corpus DOES have this notion — `RegionDissection` (`RegionDissection.lean:39`, built 2026-09-04, explicitly motivated by non-triangular regions like this one) — the real remaining gap is the nonexistence *proof* for this specific pentagon region using that notion, not the notion itself. |
...
| M `thm:admissible` | every `N`-tiling of the base-α isosceles target has scale `k = dew` | `ThinFamily.*`, `SolvCore.*` | PROVED — quantifies over tilings; the arithmetic is available, the passage from a tiling to its scale is not **UPDATE 2026-09-07**: the arithmetic half is now IN Lean, not merely available — `AdmissibleSpectrum.sqfree_scale_dvd` (`b∣k² ∧ b=de² ∧ Squarefree d ⟹ de∣k`, by prime-valuation comparison, no coprimality between `d,e` needed — witnessed at `b=8=2·2²`, non-coprime `d=e=2`) and `.count_eq` (`N = dw²(a+2b)` from the area identity). Numerically re-verified first (`private/rs/admissible_check.rs`): 3938 pairs `(b,k)` with `b<2000`, zero failures. Label **stays PROVED**: `cor:int`'s `e ∣ w(c−a−b)` and the parity clause are geometric facts about a real dissection's boundary sum, not arithmetic, and need the same tile-placement/boundary-word layer that blocks `prop:cornerpara`. |
...
| M `prop:otherspectra` | `F₁` forces `N = dw²(a+b)`; `F₂…F₄` force `N = N₀k²` | `InvariantProduct.*` | PROVED — the arithmetic is stated per shape and is formalizable; what is missing is the shape table as a Lean definition, so each clause has nothing to attach to **UPDATE 2026-09-07**: its equilateral squarefree-kernel clause ("`N` and `ab` have the same squarefree kernel") is now IN Lean as the general fact it actually is — `AdmissibleSpectrum.same_valuation_parity` (`x·y=z² ⟹` `x,y` agree in the parity of every prime valuation), which is `N·(ab)=S² ⟹` same squarefree part. Numerically re-verified first: 1509 pairs `(x,y)` with `x,y<400` and `xy` a perfect square, zero failures. Label **stays PROVED**: the `F₁`–`F₄` and tile-similar clauses still need the shape table as a Lean definition, and the equilateral divisibilities `(c+a-b)∣3S` still quantify over a real dissection's boundary flux. |
| M `prop:eqspec` | `XY = 3ab` and `s`, `t` positive integers | `EquilateralSpectrum.*` | VERIFIED. **Blocker text corrected 2026-09-10**: the previous note ("integrality of s,t comes from a tiling, quantifies over dissections") was misfiled from the neighbouring `prop:eqspecint` row (line 299) — `prop:eqspec` itself is pure algebra with no tiling quantifier, matching the other `prop:eqspec` row above (line 298) verbatim; the integrality clause is `prop:eqspecint`'s own separate, correctly-PROVED row. |
...
| O `prop:unify` | every ingredient of the corner chain holds at all members | `CChord.fill_beta`, `.fill_alpha`, `.fill_alpha_beta` (clause i), `MidTriangleGeneral.two_gamma_gt_pi` (clause ii), `TriLawSin.b_sin_gamma_eq_c_sin_beta`, `.sin_angleAt_mul_sideOpp` (clause iii, **new 2026-09-10**), `OrderForcing.partition_jb_gen` (clause iv), `SideNoB.*`, `Inflation.*` | PROVED — a survey over the chain's ingredients; its label is the minimum of theirs, and several are blocked by the placement layer. **Citation completed 2026-09-10**: the row previously carried three namespace globs and named none of the four declarations the paper's own statement cites by name. All four clauses' *identity halves* are in Lean and are genuinely member-independent — clause (i) is `fill_beta`/`fill_alpha`/`fill_alpha_beta` (the paper names these three verbatim); clause (ii)'s `2γ > π` is `MidTriangleGeneral.two_gamma_gt_pi`, proved from `3α+2β=π` and `γ=2α+β` alone, with no `(e,f)` anywhere; clause (iv) is `OrderForcing.partition_jb_gen`, general in `(e,f)` (the paper names it verbatim). **Clause (iii) had no Lean at all and now does**: `b sin γ = c sin β` was used only at `e = 1`, via `ScaleMap.scale_map`'s closed cosine forms — exactly the member-dependence clause (iii) denies. `TriLawSin.sin_angleAt_mul_sideOpp` states the sine rule in this project's `sideOpp`/`angleAt` notation for an **arbitrary** `Tri`, no hypotheses, no member; `b_sin_gamma_eq_c_sin_beta` is its `j=1` instance. **No label move**, and the reason is precise: three of the four clauses have geometric consequents that are not arithmetic — (ii) 'every reflected partner dies at a straight junction', (iii) 'so an `a`-up tile spans its strip', (i) 'the straight and full vertex classifications are unchanged' — and those are placement statements. What is now settled is that no clause's *identity* carries a hidden `e = 1`. |
| O `lem:avgen` | the `α`-vertex gap, general form | `alpha_vertex_gap_gen`, `OrderForcing.mirrored_foot_offset`, `.mirrored_foot_offset_cleared` (**new 2026-09-10**) | PROVED — the gap arithmetic is VERIFIED; 'a cover piece with its foot at' is a placement statement about a row structure that has no Lean definition. **Arithmetic gap closed 2026-09-10**: `alpha_vertex_gap_gen` proved the lemma's two *exclusions* (`¬ f²∣e²`, `¬ f∣e³`) but nothing produced the number `e³/f` they exclude. The lemma's opening step — `2 c uₓ − 3a = −e³/f`, with `uₓ = cos β = e(3f²−e²)/(2f³)`, `c = f²`, `a = ef` (the general-member closed forms recorded in `NoRightAngleTile`'s header) — is now `mirrored_foot_offset` in real form and `mirrored_foot_offset_cleared` over `ℤ`. At `e = 1` it degenerates to `−1/f`, the offset the `e = 1` argument `alpha_vertex_gap` uses, confirming the two are the same lemma. **No label move**: the remaining content is that this number *is* a mirrored cover piece's foot, which is the row structure. |
| O `lem:cornerstep` | the corner step, unconditional at every member | `CornerRule.*`, `SideNoB.*` | PROVED — same placement layer; the angle bookkeeping is available, the corner tile's edges are not **Blocker text corrected 2026-09-10 (stale, Protocol §6)**: 'the corner tile's edges are not [available]' has been false since 2026-09-09 — `prop:cornerfig`'s edge clause closed (`ApexEdgesReal.congruentDissection_apex_edges`) and `CongruentTileEdges.congruentDissection_lays_a` gives, for a real `CongruentDissection`, that a tile laying `b` and `c` at a vertex lays `a` on the remaining edge. **The row still does not move**, and the real blocker is a different one: the lemma's load-bearing step is *'the tile across the corner tile's `b`-edge lays a whole `b`'* — a statement about which tile sits across a given edge, which is edge-level placement (`CLAUDE.md` blocker (iii)'s surviving half), not about the corner tile's own edge lengths. |
...
| C `thm:ray` | the mismatch ray is completely determined | `ApexRay.*`, `PinRay.exit_not_integral_member` | PROVED — the ray arithmetic is available; 'along the ray' quantifies over the tiles it meets — placement layer. **Citation sharpened 2026-09-10**: the companion (`erdos-634-companion.tex:2348`) cites `PinRay.exit_not_integral`, whose hypotheses `IsCoprime N₀ f` and `p < N₀` were left for the reader — `PinRay.lean` held `coprime_f_N0` and `pin_index_small` as unjoined siblings. `exit_not_integral_member` (new) is the instantiated statement with `N₀ = 3f²-1` and both side conditions discharged from `2 ≤ f`, `1 ≤ p ≤ f-1`. Mechanical gap only; the row's blocker is unchanged |
...
| C `thm:e1reduce` | the `e=1` base walk is a permutation of `(a^f, b, c)` | `BaseCountsE1.base_counts`, `.base_counts_corner` | PROVED — the counts `n_b = 1` and the exclusion of `n_c = 2` are VERIFIED; `n_c ≥ 1` is `prop:gammatrap`, VERIFIED (`GammaTrap.congruentDissection_gammatrap`), and `SideWalk.lean`'s `_of_gammatrap` family (`equal_side_no_b_of_gammatrap`, `base_b_count_of_gammatrap`, `equal_side_shape_of_gammatrap`) already wires it through a real side's walk equation. **Corrected 2026-09-02**: the prior "not an open geometric input" phrasing was imprecise — checked `thm:walkstruct`'s row (line ~507) for the precise name: what remains is a *concrete* `CongruentDissection` witness realizing the base-β model/target with matching side lengths, which is structural blocker 1 (the tile-placement layer) — genuinely open, not mere bookkeeping **2026-09-12 (sequential step 6)**: the positional half now exists and is non-vacuous — `E1BaseWord.base_chain` (the base `[0, L]` is exactly partitioned into whole tile edges of `D`, each with two distinct tile vertices at its ends, lengths in `{f, f²−1, f²}`; from `chain_endpoints` with the functionals `−y`, `x`), `E1BaseWord.base_counts_of_chain` (`n_b = 1` and `(n_a, n_c) ∈ {(0,2), (f,1), (2f,0)}` as the *actual* filter cardinalities of that chain — the 2026-09-10 vacuity pattern repaired here, by exposing the chain rather than packing the counts), `first_edge_not_b`/`last_edge_not_b` (the corner tile presents `β`, so its base edge is `a` or `c`). **Not proved as stated**: clause (ii)'s "exactly one `c`" and "first and last are `a`" — instead the cases `n_c ∈ {0, 2}` and a `c` at either end are killed downstream (`E1BaseWord` §E), and clause (i) is untouched. Caution: `E1BaseWord.e1_family_f_ge_3` refutes the antecedent ("in any tiling") for `f ≥ 3`, so (i) and (ii) are now Lean theorems by ex falso; that is **not** claimed as VERIFIED here (it would be the label inflation the 2026-08-30 audit warned against). Label PROVED unchanged. |
...
| C `lem:offsets` | the offset congruence `Σεᵢ − j = −2q` | `OffsetForcing.forcing`, `.forcing_odd`, `.forcing_even` | PROVED — the congruence is elementary and formalizable; what it is *about* — the terminal column's base apex — has no Lean definition | **[citation corrected 2026-09-04 — this row read `none`, but the companion's own `rem:parity` names `lean/OffsetForcing.lean` as the forcing; the row was never updated.]**
...
| C `lem:sidenob` | the equal sides carry no `b`-edge | `SideNoB.side_no_b_uncond`, `.side_no_b_e_one` | VERIFIED — **blocker note withdrawn 2026-09-10.** Re-read the paper statement (`erdos-634-companion.tex:1249`): it quantifies over *walks* — "every side walk `P'a+Q'b+R'c=f³` obeying the γ-trap `R'≥1` has `Q'=0`" — and makes no claim about a tiling's side, so there is no edge-chain presupposition to bridge. `SideNoB.side_no_b_uncond` (`SideNoB.lean:110`) is that implication verbatim. Row 271 above already had this right; this row's note was the stale one. |
| C `prop:doublec` | the double-`c` kill at any initial block | `DoubleC.*` | PROVED — placement layer: initial blocks of a side walk |
| C `rem:reversal` / `rem:walls14` | the isosceles mirror reverses the base word, sending a route-1 word `a^i c a^j b a^{k'}` to one whose `b` is preceded only by `a`-letters | `MirrorKill.k_ge_two`, `.rev_b_position`, `.revLetters_eq`, `.rev_prefix_all_a`, `.rev_b_index`, `.mirror_reduces_route1` | PROVED — **previously untracked (no row at all) and outside the build gate; both fixed 2026-09-10.** The combinatorics is VERIFIED: `k' ≥ 2` is automatic from `thm:e1reduce`'s `b`-position constraint, and the reversal is now *computed* (`revLetters_eq`) rather than asserted, so `rev_prefix_all_a`/`rev_b_index` are statements about `W^R` itself. **CIRCULARITY FINDING 2026-09-10, repaired**: `mirror_reduces_route1` previously took `cascade_kills : ∀ v : Word f, 2 ≤ v.k → ¬ tiles v` — a condition *every* `Word f` satisfies by `k_ge_two`, so the hypothesis was equivalent to the conclusion universally quantified and the theorem was `H → H`; its `mirror` hypothesis (`∃ pre, pre = v.k ∧ 3 ≤ pre+1 ∧ pre+1 ≤ f-1`) was provable outright from `rev_b_position` and went unused in the proof. Machine-checked both ways before the repair. The statement now separates the two predicates — `mirror : tiles v → cascadeWord v.k` transports, `cascade_kills : ∀ pre, 2 ≤ pre → ¬ cascadeWord pre` kills — so the cascade is a genuine external obligation. **It is still an obligation**: `rem:walls14`'s cascade is not proved in Lean for general `f`, and `ThinClosed.lean` already records that this reversal is not new (it is part (L3) of `thm:e1cascade`, stated generally as `rem:reversal`) |
...
| C `thm:forkkill` | the row fork kill | `OrderForcing.straight_junction_cases`, `.straight_junction_gamma_bound`, `.east_cover_gap`, `.alpha_vertex_gap`, `.anti_brick_side` | PROVED — same brick/mate structure. **Phantom citation removed 2026-09-10**: this row cited "`ForkKill` lemmas", and the string `ForkKill` occurs **nowhere** in `lean/` or `paper/` — no file, no namespace, no declaration (grepped, zero hits). Same failure class as `thm:l2slot`'s 2026-09-04 correction. The theorem's own proof (`erdos-634-companion.tex:1893-1895`) is two lines and cites no Lean name of its own: it composes `lem:eastfan` with `prop:a2branch`, so the honest citation is the union of those two rows' citations, all five pre-existing in `OrderForcing.lean` (lines 51, 55, 438, 382, 411) and verified present this pass. `ForcedRow.*` also dropped: `ForcedRow.lean` exists but neither this theorem's statement nor its proof routes through it — it entered via the same vague "same brick/mate structure" boilerplate. **Label unchanged**: the arithmetic under both cited rows is real; the brick/mate configuration objects still have no `Dissection`-level definition, which remains the blocker |
...
| C `lem:wallclimb` | the wall climb: `Cⱼ` direct and `Mⱼ` forced | `WallChain.*` | PROVED — same structure. **Phantom citation removed 2026-09-10**: this row cited "`WallClimb` lemmas"; the string `WallClimb` occurs **nowhere** in `lean/` or `paper/` (grepped, zero hits) — no file, no namespace, no declaration. `WallChain.*` is retained and is real (`Dissection.lineChain`, `.wall_cover`, `.wall_partition`, `.wall_two_sided` — the interior-wall edge-chain machinery the lemma's "`Mⱼ`'s `c`-edge continues the interior wall" clause needs), but note that the lemma's own proof (`erdos-634-companion.tex:1948-1965`) cites **no** Lean name at all: it is a planar induction over the block tiles `Cⱼ`/mates `Mⱼ`, objects with no `Dissection`-level definition. So `WallChain.*` is ambient support, not a discharge of the statement. Blocker named: **tile-placement layer** (the brick/mate/block objects). **Label unchanged** |
...
| C `thm:secondc` | the second edge of an equal side is a `c` | `SecondEdge.admissible_ends_alpha`, `SideWalk.apex_edge_and_next_of_gammatrap` | PROVED — **blocker corrected 2026-09-09.** The old note ("'the edge immediately after' presupposes the side's ordered chain") was stale: the side's ordered chain is real and general (`WallEndpoints.chain_endpoints`, contiguity `edgeEast (E m) = edgeWest (E (m+1))` proved, not hypothesised), and `SideWalk.apex_edge_and_next_of_gammatrap` (new) now states, for a real `CongruentDissection`'s side `k`, that the apex-end edge is a `c` and **the edge immediately below it is an `a` or a `c`** — the `b` excluded by the real `lem:sidenob` (`Qc = 0`), now exposed edge-by-edge rather than only as a count. Axiom-clean, no `sorry`. What remains — the *whole* remaining content of `thm:secondc` — is the **exclusion of the `a`**: it needs the vertex figure at `J = edgeWest p` to be straight and `cor:noTP`'s tile identification, i.e. real geometry that does not exist. `SecondEdge.admissible_ends_alpha` is exactly that exhaustion with those two facts as hypotheses. So the gap is now a single named exclusion, not a missing chain |
...
| C `cor:pbound` | `n_c ≥ 2`, hence `pe + 2 ≤ f` | `ApexRigidity.side_p_bound`, `SideWalk.equal_side_no_b_of_gammatrap` | PROVED — **blocker corrected 2026-09-09.** "the apex `c`-edge, which is still open" was stale and contradicted this file's own row for `thm:walkstruct` (~line 494): the apex `c`-edge for a real dissection's equal side has been proved since 2026-09-02 inside `SideWalk.equal_side_no_b_of_gammatrap`, whose conclusion exhibits a `wallList` edge with `edgeEast = D.target.pts (k+1)` (the apex) and length `sideOpp D.model 2 = c`, via `TileAt.congruentDissection_apex_counts` + `EdgeType.edge_excludes_own_angle` + `Qc = 0`. What is actually missing is the **second** `c`: `n_c ≥ 2` needs the edge below `J` too, i.e. `thm:secondc`, and the residual there is the `a`-exclusion (row above). Arithmetic cannot supply it — `Rc = 1` is consistent with the walk equation whenever `e ∣ f-1` |
...
| C `prop:inflbdy` | the inflated boundary | `Inflation.*` | PROVED — **checked 2026-09-02**: same as `cor:inflcrux`/`prop:inflparity` — the scale map (`Realizable.scaleTri`) exists as an operation, but `Inflation.lean` has no geometric `Dissection` object of the inflated tile to apply it to yet; that construction is unstarted, not a citation fix. **Corrected 2026-09-09**: "no Dissection object" is now stale on its own terms — `RegionDissection`/`CongruentRegionDissection` (built 2026-09-04, two days later) are exactly such general dissection objects. What's actually still missing is CONSTRUCTING the specific inflated-tile region/dissection using that notion, not the notion itself. |
...
| C `prop:inflparity` | edge parity kills the `p=1` boundary | `Inflation.*` | PROVED — same as `cor:inflcrux` (checked 2026-09-02: `Inflation.lean` is pure combinatorics, no dissection object yet), plus an edge-to-edge hypothesis with no Lean definition. **Corrected 2026-09-09**: "no dissection object" is stale — `RegionDissection` (built 2026-09-04) supplies the general notion; the edge-to-edge hypothesis (the real remaining gap) is unaffected. |
...
| O `lem:charge` | direct-vs-mirrored cover pieces and feet displaced by `1/f`: **placement layer** |
| C `prop:orientmono` | passage from a real boundary run to its word: **bridge (c) beyond the base** |
| C `thm:walkstruct` | side words of a real tiling: **bridge (c) beyond the base** |
...
| M `prop:conicform` | a `2π/3` tile *requires* such a factorization, with the divisor/parity clauses spelled out | `EquilateralConic.conicform_2pi3`, `.conicform_pi3` | PROVED — the remaining clause after the split. **Citation refined 2026-09-10**: the row previously cited `EquilateralConic.*` generically without a declaration matching the paper's exact *divisor* shape (`uv=16N²`, `2∣(u+v)`, `(u+v)/2−5N=s²`, `s∣3N` for `2π/3`; the `π/3` analogue). `conicform_2pi3`/`.conicform_pi3` (new) state exactly this, as hypothesis-carried arithmetic — so the entire arithmetic half, divisor and parity clauses included, is now one named theorem per case, not scattered ingredients. Label stays PROVED: "requires" is necessity from a tiling, and that a tiling yields integers `s,t,q` of the stated parities is `prop:eqspecint`'s own separate tiling quantifier, untouched. |
...
| K3 | the `c`-slot fan's residual angle is outside `{Xα+Yβ : X,Y ≥ 0}` | **`MarchKillsFan.lean` (2026-09-12, later the same day)**: the residual is `β − α` at the far end `V` of the `c`-tile on the base, uniformly (`residual_L_BG`, `residual_L_GB`); `beta_sub_alpha_not_cone`, `beta_wedge_only_beta`; the kill as a configuration theorem `c_slot_kill`, `c_slot_kill_gb` | VERIFIED **conditional on R_c** — see the subsection below. The earlier note that the residual "depends on the fan's chirality bits" was wrong: the traces show it does not (every `P4c` leaf at the slot, every orbit checked, has the same residual `(−1,1)`); the chirality bits change the fan's coordinates, not its angles |
...
### K3, the `c`-slot fan, and the `c`-slot pockets (2026-09-12, later) — `MarchKillsFan.lean`

The one kill `MarchKills.lean` left open. Instrumentation traces (`uni_f12_b5c8` rerun through
`guard_run.sh`, byte-identical to room `data`'s tree; `uni_f12_b7c4`, `uni_f14_b5c8`; the 85-orbit
`P4c` ledger, exact `Q(√D)` arithmetic) locate every `c`-slot `P4c` leaf at one vertex, `V`, the far
end of the `c`-tile on the base, with residual `β − α` (basis `α↦(1,0)`, `β↦(0,1)`, `γ↦(2,1)`,
`π↦(3,2)`: `(−1,1)`) on all 85 orbits, zero leaves without a failing vertex; the five pin orbits
(`cp = bp−1`) add `2β − α` and `β − 2α` on `2/18` of their slot leaves each. The nine terminal
signatures at the slot are identical on `(5,8)`, `(7,4)` at `f=12` and `(5,8)` at `f=14`. R_c below is the open content; everything after it is a theorem.

| Atom | Statement | Lean declaration | Label |
|---|---|---|---|
| K3-wedge | the wedge at `V` after the `c`-tile and the next `a`-tile: `γ` (`cSlotTile`+`BG`), `β` (`cSlotTile`+`GB`), `3α` (`cSlotTile'`+`BG`), `α` (`cSlotTile'`+`GB`) | `MarchKillsFan.wedge_L_BG`, `.wedge_L_GB`, `.wedge_R_BG`, `.wedge_R_GB`, `.fans_agree` | VERIFIED (identities from `γ = 2α+β`, `3α+2β = π`) |
| K3-res | the fan's residual is `β − α` in both branches: `π − (4α+β)` and `π − (α+γ) − α` | `MarchKillsFan.residual_L_BG`, `.residual_L_GB` | VERIFIED |
| K3-arith | `β − α ≠ Xα + Yβ` for `X,Y ∈ ℕ`; any label with a negative coefficient is not a corner sum (covers the pin orbits' `2β − α`, `β − 2α`); a `β`-wedge admits exactly the figure `(0,1,0)`; at a straight point `nα ≤ 3`, and `nα ≤ 1` once a `γ` is present | `MarchKillsFan.beta_sub_alpha_not_cone`, `.neg_label_not_cone`, `.ledger_residuals_not_cone`, `.beta_wedge_only_beta` (via `Geometry.vertex_multiplicities`, irrationality not re-proved), `.pi_point_no_fourth_alpha`, `.pi_point_gamma_at_most_one_alpha` | VERIFIED |
| K3-model | at `e = 1`: `α < β` for `f ≥ 2`; `sin(α/2) = 1/(2f)`; `α/π ∉ ℚ` for natural `f ≥ 2`, at the model's own `modelAlpha` | `MarchKillsFan.modelAlpha_lt_modelBeta`, `.sin_half_modelAlpha`, `.modelAlpha_irrational`, `.angle_bundle_nonvacuous_f3` | VERIFIED — witnesses every angle hypothesis of the composite at `f = 3` |
| K3-fan | **the `c`-slot fan kill**: in a `CongruentDissection` of the `e=1` target (model sides `f, f²−1, f²`, angles `α,β,γ`), with `cSlotTile x f` a tile and two tiles presenting `α` at `V = (x+f², 0)`, no other tile presents `α` at `V`; with `cSlotTile x f` and `aTileGB (x+f²) f`, no other tile presents `α` at `V` | `MarchKillsFan.straight_point_no_fourth_alpha`, `.straight_point_gamma_no_second_alpha` (abstract, via `TileAt.congruentDissection_boundary_figure_cases`), `.c_slot_kill`, `.c_slot_kill_gb` (coordinates: `localAngle_of_oppSide`, `cSlotTile_localAngle_V`, `aTileGB_localAngle_left`, `base_point_mem_frontier`, `base_point_not_vertex`) | VERIFIED **conditional on R_c** (hypotheses `hi : D.tile i = cSlotTile x f`, `hk₁ hk₂ : localAngle V = α`, resp. `hj : D.tile j = aTileGB (x+f²) f`). Non-vacuity: `cslot_config_in_target` (the `c`-tile and the `BG` tile lie in the target for `0 ≤ x`, `x + f² + 2·dBG ≤ L`), `flushFiller_localAngle`/`offsetFiller_localAngle` (the fan tiles of the traces satisfy `hk`), `cslot_f3` |
| K3-forced | under R_c with the `BG` tile, exactly one further tile presents `β` at `V` — the `P2` branch | `MarchKillsFan.straight_point_three_alpha_two_beta`, `.c_slot_forced_beta` | VERIFIED conditional on R_c; that tile's kill is K2 (`b − a`, arithmetic VERIFIED), not realised at edge level |
| K3-pocket | the two `P1a` pockets at the slot: (R) isosceles `(a, a, 1)`, area `tile/b`; (L) isosceles `(b, b, b/f)`, area `(b/c)·tile`; neither ratio (nor `1 + b/c`) is in `ℕ` | `MarchKillsFan.overrun_on_ray`, `.pocketR_AC`, `.pocketR_VC`, `.pocketR_VA`, `.pocketR_det`, `.pocketR_not_tile_multiple`, `.pocketL_VK`, `.pocketL_VA`, `.pocketL_AK`, `.pocketL_det`, `.pocketL_not_tile_multiple`, `.pockets_f3` | VERIFIED as exact coordinate arithmetic. That the pocket is a union of tiles of a hypothetical dissection (the region-level content of `P1a`) is **not** built |

**R_c, exactly.** That a hypothetical tiling contains the `c`-tile in the `β`-left reflection on
`[x, x+f²]` and two `α`-corners at its far end `V` (the fan's first two tiles), resp. the `c`-tile
and a `GB` `a`-tile on the next letter. This is `rem:marchobl` (i)–(iii) carried into the `c`-slot
plus the reflection choice, and is not proved. The `cSlotTile'` reflection's kills are `P2` (`b−a`)
and the (R) pocket, whose region-level content is not assembled. **No label of `rem:marchobl` moves.**
...
### The `e = 1` family, unconditional in Lean for `f ≥ 3` (2026-09-12, sequential step 6) — `E1NormalForm.lean`, `E1BaseWord.lean`

Step 5 left `thm:e1family` (`f ≥ 3`) conditional on two things: (2) the normal-position and
model-labelling reduction, and (1) `thm:e1reduce`(ii) in positional form.  Both are closed.

**(2) `E1NormalForm.lean`.** `normalFormC D₀ g σ hT` moves a `CongruentDissection` into normal
position by the isometry `g` of a target congruence `(g, σ)` and replaces its model by any congruent
tile `T`; tiles are `g`-images vertex for vertex (`rfl`).  `normal_form`: from
`D₀.target.Congruent (baseBetaTarget 1 n)` and `D₀.model.Congruent (modelTri n)` (`n ≥ 2`) produce
`D` with `D.target = baseBetaTarget 1 n` on the nose, `D.model = modelTri n`, `ModelData`,
`AngleData` at the model angles, and `(D.tile i).pts m = g ((D₀.tile i).pts m)`.  `angleData_model`
(every natural `n ≥ 2`), `modelData_of_model_eq`, `congruent_modelTri_of_sides` (SSS from the three
side lengths, via `SssCongruent`), `base_point_iff` (the intrinsic reading of "`g p` is on the
base"), `e1_family_of_word` (`base_word_dies` transported to `D₀`).  Chirality is irrelevant:
everything downstream is stated through vertex sets and `localAngle`s.

**(1) `E1BaseWord.lean`.**  §A `base_chain`: for `D` with `htgt`/`ModelData`, junctions
`0 = X 0 < … < X n = L` with every `[X m, X (m+1)]` a tile edge of `D` (two distinct vertices at
its ends) of length `f`, `f²−1` or `f²` — `chain_endpoints` on the base line with `g = −y`,
`dir = x`, the base as `segment (0,0) (L,0) ⊆ frontier`, `hthird` from `WallThird`.  §B
`base_counts_of_chain`: `n_b = 1`, `(n_a, n_c) ∈ {(0,2), (f,1), (2f,0)}` as the actual filter
cardinalities of the chain (`ChainWalk.chain_walk` + `BaseCountsE1.base_counts`).  §C
`first_edge_not_b`, `last_edge_not_b`: the corner tile presents `β` (`TileAt.
congruentDissection_base_corner_tile_vertex`, `beta_eq_modelBeta` by the law of cosines on the
model's own vertex), so its base edge is `a` or `c`.  §D the corner `c`-slot: no filler exists
there, and the two placements `junction_c_a`/`junction_c_b` leave at `V = (f², 0)` die differently
— `corner_bCapM_dies` (the cap's far vertex `(1/2, h_b)` violates the left-side functional) and
`corner_bOverM_dies` (`cslot_run_kill_outside`: `RunPartition.cslot_run_kill` with the filler
clamp at `A` replaced by clamp (c), `corner_ext_outside` — the extension `(A, A')` of the corner
tile's `b`-edge beyond its side-vertex `A` is outside the target).  §E the four words the counts
and §C leave outside `base_word_dies`: `word_aba_dies` (`a^p b a^q`: `first_run`, `junction_a_b`,
`bslot'_then_run_dies`), `word_c_p0_dies` (`c a^q b a^r`: corner tile is `cSlotTile 0`
(`corner_c_tile`), then `junction_c_a`/`junction_c_b` with the §D kills on the `GB`/`bSlotTile`
branch and the far-corner kills on the other), `word_cbc_dies` (`c b c`: `cbc_bslot_dies` for the
`bSlotTile` reflection, `reflCD` for the other), `word_bfirst_r0_dies` (`a^p b a^q c`, the
reflection of `c a^q b a^p`; `laysARun_reflCD` transports laid runs).  §F `e1_family_f_ge_3`:
the chain's letter sets partition `range n`; the unique `b` is at `ib ∉ {0, n−1}`; in each of the
three count cases the word is read off the junction sequence (`X_run`, `laysARun_of_chain`) and
is one of `base_word_dies`'s two shapes or one of §E's four.  §G `e1_family_f_ge_3_congruent`,
`e1_family_f_ge_3_of_sides`.

**The theorem, exactly.**  `E1BaseWord.e1_family_f_ge_3_congruent {N} (D₀ : CongruentDissection
N) {n : ℕ} (hn3 : 3 ≤ n) (htgt : D₀.target.Congruent (baseBetaTarget 1 n _)) (hmod :
D₀.model.Congruent (modelTri n _)) : False`.  Hypotheses individually witnessed; axioms standard;
no `sorry`; `lake build Erdos634.All` clean; `code/novelty_check.sh`: no prior statement.

**What it is not.**  `f = 2` (`N = 11`) is outside: `base_counts` needs `f ≥ 3` (at `f = 2`,
`n_b = 3` also solves the walk equation); that member is the 135-node search.  `thm:e1reduce`(ii)
is not proved *as stated* (its `n_c = 1` and first/last-`a` clauses are bypassed, the cases being
killed downstream), and `prop:cornerpara` is not proved (only its boundary clamp is realised at one
end of one corner).  Every conditional about a tiling of the `e = 1`, `f ≥ 3` target is now
provable by ex falso; no label is moved on that account.  `e ≥ 2` is untouched.

**Labels.**  `thm:e1family`: not moved — the paper statement as written includes `f = 2`, and the
Lean theorem does not; the paper's CONJECTURE tag is contradicted for `f ≥ 3` and re-tagging is the
author's call.  `thm:e1reduce`, `prop:cornerpara`: unchanged, notes updated.

**Which /goal outcome.**  Not (1).  Not (2): the prime case needs `e ≥ 2` as well, and that branch
stands exactly where it did (`N = 83` first; the crossing question).  What is now a theorem: node
B1 of `private/GOAL_PRIMES.md` — the `(1, f)` instance admits no tiling, every `f ≥ 3` — closed in
Lean without conditions; for the primes `3f² − 1` with a unique base-β representation
(`47, 107, 191, 431, 587, …`) the exclusion rests on VERIFIED Lean plus `thm:mod12`/`prop:repunique`
(VERIFIED) and `thm:main` (PROVED).  Whether that is outcome (3) is the user's judgement; by density
it is `≈ 1.1 %` of the base-β primes, and the wall for the rest has not moved.
...
| R1-over | a `b`- or `c`-edge from `V` strictly contains `E` (arithmetic only) | `RouteOne.overshoot_dichotomy`, `.E_interior_of_long` | VERIFIED — **label corrected 2026-09-09**: the cited Lean proves only that `f < L` puts `E` strictly between the endpoints. The clause "tile-interior blocking, branch dies" was **not** in the cited declarations, and `paper/erdos-634-companion.tex:1667` (`prop:overshoot`, marked VERIFIED) asserts it on their strength. `E` interior to a long edge is not by itself a contradiction — a tile may lay a long edge along the wall, and does at the end of a march run |
| R1-step | **`LocalMarchRun.StepDatum.hfoot` is unsatisfiable for negatively-oriented (reflected) tiles** — a real bug found and repaired 2026-09-09, previously untracked (`RouteOneStepBridge.lean` has no PAPER_MAP row at all). `no_such_corner` exhibits the concrete witness (`revTri`) proving `hfoot` fails for a reflected tile, stranding the `m+2` branch of every flank theorem built on the biased `StepDatum`. Repair: `Run'`/`StepDatum'` generalize with a per-step offset `{1,2}` instead of assuming `hfoot`; `Run'.ofRun` embeds every original `Run`, and `witness_run'` instantiates a real one against the kernel-verified 44-tile dissection (`Tiling44Bridge.dissection`), so the repair is not vacuous. `march_from_wall`'s own hypotheses are separately, honestly flagged in the file as jointly unsatisfiable (lines 309-319) — the right handling, not a defect. `StepDatum` is used only inside `LocalMarchRun.lean`/`RouteOneStepBridge.lean`, so no other VERIFIED row rested on the pre-repair biased version. | `RouteOneStepBridge.escape_flank_dichotomy`, `.advance_vector`, `.escape_flank_advance`, `.no_such_corner`, `LocalMarchRun.Run'`, `.StepDatum'`, `.Run'.ofRun`, `.witness_run'` | VERIFIED (the repair) |
| R1-block | **tile-interior blocking as a theorem**: an edge-interior point of a horizontal edge of a tile that keeps weakly above that line owns a whole *upper half-neighbourhood* inside the tile's **interior**; so the overshoot branch dies as soon as another tile has points strictly above the wall arbitrarily near `E` | `OvershootBlocking.upper_nbhd_of_edge_interior`, `.coord_apex_height`, `.overshoot_blocks`, `.blocked_of_overshoot`, `.escape_flank_advance_of_deviation`, `.upper_nbhd_witness` | VERIFIED **conditional on `hdev`** — `hdev`, that a tile other than the serving one carries points strictly above the wall arbitrarily near `E`, is the escape word's own deviating chord at `E` and is **not** derived: it stays part of `rem:routeoneopen`'s attachment obligation. `upper_nbhd_witness` exhibits a triangle meeting every hypothesis of the half-neighbourhood lemma, so that lemma is not vacuous |
...
### The `none`-citation scan, completed (2026-09-04)

All 26 rows citing `none` were partitioned. **10 were stale bookkeeping** — a later row for the same
label already carried a real citation — and were marked `[STALE 2026-09-04]` in place:
`prop:nogocensus`, `prop:norm`, `prop:globalsys`, `lem:cancel`, `lem:value`, `cor:int`,
`prop:conic`, `lem:filler`, `thm:lattice`, `prop:interfacefloor`. (`prop:conic` is in fact
**VERIFIED** in the paper text via `EquilateralConic.*`; its `none` row was simply never updated.)
**REMOVED 2026-09-07** (sync-debt cleanup, Rule 18): all 10 of the above, plus 7 more
duplicate/stale rows found the same pass (`cor:ladder`, `thm:lattice` again, `rem:norightangle`,
`lem:anglethreshold` ×2, `prop:orientmono`, `lem:ccornerside`) — 17 rows total, each verified to
have a later accurate row before deletion. Sync debt from PAPER_MAP.md duplicate rows: 0.

**16 are genuinely uncited**: `rem:nilptower`, `rem:closepairbase`, `prop:eqspecint`, `thm:eq105`,
`prop:selfsim`, `lem:rowwords`, `lem:rowp0`, `lem:rowp1`, `cor:rowinduction`, `prop:nogoauto`,
`rem:spectral`, `prop:threecostumes`, `prop:ninetools`, `thm:decidable`, `lem:offsets`,
`rem:anglethreshold`.

Triage of those 16: five (`prop:selfsim`, `lem:rowwords`, `lem:rowp0`, `lem:rowp1`,
`cor:rowinduction`) are the row-induction chain and share the certified-search blocker; four
(`rem:spectral`, `prop:threecostumes`, `prop:ninetools`, `prop:nogoauto`) are meta-statements about
what the tools cannot do; the tractable arithmetic remainder is **`prop:eqspecint`**, **`lem:offsets`**
and **`thm:decidable`**.

**`thm:lattice` audited under Rule 5 — label stays PROVED.** `SpectrumLattice.admissible_set_eq` is
...
### `lem:offsets` citation corrected; `UnionDissection` audited (2026-09-04)

**`lem:offsets` was not uncited.** The companion's own `rem:parity` names
`lean/OffsetForcing.lean` (`forcing`, `forcing_odd`, `forcing_even`) — general in `f`, both
parities, axiom-clean — as the lemma's forcing step. Only the `PAPER_MAP` row said `none`. Read and
confirmed: `forcing` derives `q = 0` from `p + q < f` together with `f ∣ 2q` (odd `f`) or `2f ∣ 2q`
(even `f`), which is exactly the lemma's arithmetic. Citation fixed.

**Label stays PROVED.** The Lean covers the forcing; the lemma's *statement* also asserts the base
apex's position formula `[j(3f²−1) + Σεᵢ]/(2f) + kf` and concludes "every level is `(γ,β)`-oriented
and the apexes lie at consecutive multiples of `f`". Those are geometric, about a real column.
Blocker named: **tile-placement layer**. This drops the genuinely-uncited count from 16 to 15.

**`UnionDissection` audit (Rule 5), on the file dated 2026-09-02.** `unionDissection` and
`unionCongruentDissection` are real, non-vacuous, and prove what they claim: given two
`Dissection`s (resp. same-model `CongruentDissection`s) whose targets have disjoint interiors and
carriers unioning to a `Tri`, they produce a genuine dissection of size `M + N`, with
`unionDissection_tile_left/right` identifying the tiles. That is the union primitive
`Compose.compose` (a refinement composer) never supplied.

**But it does not yet reach the collar step, and the reason is structural.** Both `hcov` and the
result type are stated against a single `Tri`: the glued region must be a *triangle*, and so must
each piece. The collar decomposition of `Δ_{m+2}` into `Δ_m` plus two parallelogram columns and a
corner triangle cannot be routed through this primitive by iteration, because the intermediate
unions are not triangles — cutting `Δ_{m+2}` by a line parallel to the base leaves a **trapezoid**,
which is not a `Tri`, so there is no `Dissection` to feed back in. Binary cevian splits keep every
piece a triangle, but nothing says a cevian split of `Δ_{m+2}` yields pieces admitting same-model
congruent dissections.

So the recorded blocker "no composition map on dissections" is **half discharged**: the union of two
triangle-targeted dissections exists; a union over **non-triangular regions** does not, and that is
...
## 2026-09-12: Target B (thick `e = f−1`) — the `(0,e,2e)` column's arithmetic, the survivor lists, and a vacuity on `hyp:walls` there

New file `lean/Erdos634/EpredColumnKill.lean` (in `Erdos634.All`, axiom-clean on the standard three,
no `sorry`, `lake build Erdos634.All` clean). Rows touched: `thm:efminus1`, `prop:n1fromwalls`,
`thm:basebeta-full`; `WallsCircular.lean`'s header corrected in place (dated).

**Ledger first (what was already in Lean).** `p = 0` at `e = f−1` had *no* Lean at all beyond the
general `side_p_bound`; its geometric premise `n_c ≥ 2` is `thm:secondc`, whose Lean residual is the
single `a`-exclusion below the apex `c` (`SideWalk.apex_edge_and_next_of_gammatrap`, 2026-09-09).
`lem:ccorner` CONJECTURE, `prop:n1fromwalls` CONJECTURE, both with no Lean. The base-column lists at
`e = f−1` were complete (`EpredBaseColumns`, `EpredWedgeClearance`).

**Added.**
* `side_p_zero_of_two_c`, `epred_side_no_a` — `thm:efminus1`'s arithmetic tail from the real side
  walk: `n_c ≥ 2 ⟹ P = Q = 0` on `e = f−1`.
* `column_zero_side_word`, `column_zero_dies_of_two_c` — `prop:n1fromwalls` on the family: a side
  with `P > 0` reads exactly `a^f c`, dead under `n_c ≥ 2`. Hypotheses `hPpos` (placement link) and
  `hR` (`thm:secondc`) are the two open geometric facts, named, witnessed satisfiable
  (`witness_column_zero_side_23`).
* `prefix_ec` (via `Inflation.a_side_no_b`/`a_side_words` at `k = f`), `no_prefix_of_length_fa`,
  `no_suffix_of_length_fa`, `no_junction_at_fa` — on `e = f−1`, a base word beginning and ending
  with `a` and carrying `≤ f` letters `a` has no prefix or suffix of length `f·a = e·c`.
* `west_block_never_complete_epred`, `east_block_never_complete_epred`, `walls_refuted_epred` —
  `hyp:walls` (both halves) fails on the family; see the `thm:basebeta-full` row.
* `survivors_23`, `survivors_59`, `survivors_83` — the exact surviving base columns after every
  filter (`z ≥ 1`, `x+z ≥ 4`, `x ≥ 2` from `p = 0`, wedge): `N=23`: `(3,2,2)`; `N=59`: `(5,4,4)`,
  `(4,9,3)`, `(3,14,2)`; `N=83`: `(6,5,5)`, `(5,11,4)`, `(4,17,3)`, `(3,23,2)`.

**Consequence for `thm:align`/`thm:ray` (flagged above, 2026-09-10, not relabelled).** Their
framing premise "the mismatch ray has inner length `f·b`" means the wall reaches the base, at
coordinate `e·f² = f·a` from a corner (`thm:farregion`, `OM-align`). `no_junction_at_fa` says no
`e = f−1` base word has a junction there. So on the whole family the premise fails in every
hypothetical tiling; the theorems are vacuous there, and the wall-based apex toolkit (`rem:live`)
...
## 2026-09-13: blocker (iv) — the certified-search format exists; Lemma P is a theorem

Design (B) of the brief: a generator, `lean/gen_tree.py`, re-runs the constructor's branching rule
(`build23.py`'s `pick_point`/`free_dirs`/six placements) in **exact rational arithmetic in the
rescaled coordinates `(X, Y) ↦ (X, Y·√Dn)`** — every vertex of every node lives in `ℚ × ℚ√Dn`, so
every membership, escape, overlap and covering fact is a rational-*linear* inequality — and emits a
Lean file in which each node is a theorem about an arbitrary `CongruentDissection`. The
hand-written trust base is `Erdos634/CertNode.lean` (≈ 640 lines, generic in `Dn`, standard
axioms), on top of `PlacementCompleteness` and `SixPlacements`:

* `mem_rtri` / `rtri_ef_nonneg` / `mem_interior_rtri` / `not_mem_rtri_of_edge{0,1,2}` — closed and
  open membership in a coordinate triangle as three rational-linear inequalities (`ef`);
* `placeThird_rp` — `SixPlacements.placeThird` evaluated in `ℚ(√Dn)`; `carrier_eq_rtri_of_pts`;
* `hlex_of` + `halfR` + `lexLE_rp` — `hlex` from a closed set `C ⊇ U`; the generator supplies
  `C = target ∩ {Y ≥ yv} ∩ ({X ≥ xv} ∪ ⋃ closed BSP cells)` and proves `U ⊆ C` by a binary space
  partition over tile-edge lines (leaf: in a placed tile / infeasible / a cell that avoids the
  lex-below set, each a `linarith` call);
* `hfree_of` + `pt_ccw_rp` — the curve `t ↦ v + t(d + t√Dn·d⊥)`, polynomial in `t`, in `U`;
* `hblocked_of_wedge` (via `PlacementCompleteness.wedge_interior`, on the placed tile if `v` is
  its vertex, on a sub-triangle if `v` is a T-junction on its edge) / `hblocked_base`;
* the kills: `escape_of`, `overlap_of` (explicit interior witness, `Dissection.interiors_disjoint`),
  `base_edge_clash` (two tiles with base edges `[x₀,x₁]`, `[x₀,x₁']`, `x₁ ≠ x₁'`, overlap — the
  base-word kill, consuming `HasBaseEdge`);
* `disj_of_sep_edge`, `rtri_subset` — non-vacuity plumbing.

**Generated and built, standard axioms, no `sorry`:**

```
LemmaPTheorem.lemmaP_no_dissection {N} (D : CongruentDissection N)
...
`HasBaseEdge D i x₀ x₁ := ∃ k₀ k₁, k₀ ≠ k₁ ∧ (D.tile i).pts k₀ = (x₀,0) ∧ (D.tile i).pts k₁ = (x₁,0)`;
"base word begins `a c b`" is this three-block predicate, *stated* (not derived from
`CornerBaseEdgesReal`, which gives only the corner tile's edge). `LemmaPTree.node_0..12` are the 13
nodes of `report_ramanujan.md` §5 (same tree, same coordinates; `node_12` is `LemmaPNode2.node2_jam`'s
configuration, now subsumed). Non-vacuity: `T*_sides`, `T*_inside`, `disj_*`, `node_*_config`,
`baseword_hyp_config` — every node's placed tiles are congruent, inside, pairwise interior-disjoint,
and the base-word hypothesis is realised by node 4's configuration. Cost: 13 nodes ≈ 63 s wall,
5.5 GB max RSS (183 `linarith` + 575 `norm_num`).

**Controls.** Accept: `Tiling44Nodes.node_0..2` (generated, `Dn = 15`, conclusion "some unplaced
tile is one of the live placements") instantiated on `Tiling44Bridge.dissection` in
`Tiling44NodesControl.control_root/_node_1/_node_2`, with the real next tile shown to be the named
child each time (`real_tile0/1/2`). Reject: `gen_tree.py lemmaP --flip 3 2` corrupts one overlap
witness (`(3574/729, 230/729·r)` → `(76474/729, …)`); the file fails to build at exactly that
line (unsolved `0 < ef …`), nothing else changes.

**What this does and does not move.** Formalization debt: the `a c b` half of the canonical `N = 23`
certificate (four of the five class representatives of `report_ramanujan.md`) is now a theorem
about dissections. `/goal`: none of the three outcomes — `N = 23` as a whole still needs `aabbcca`
(188 nodes, generated by the same script into `Word_aabbcca0..8.lean`; build status: AABBCCA_STATUS), `N = 83` untouched, the prime case untouched.
