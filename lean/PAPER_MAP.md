...
| R1-tang | the tangential-approach step: points on the line just right of `V` drag a tile whose corner cone contains `(1,0)` | `RouteOne.affine_nonneg_at_zero`, `.lower_flank_horizontal` (the arithmetic); pigeonhole + cone passage OPEN | REDUCED — `lower_flank_horizontal` collapses the cone core to four lines: `cross(e₁,(1,0)) = −e₁.y`, so cone membership plus weakly-upward forces the lower flank horizontal outright, and positive orientation gives it rightward; `affine_nonneg_at_zero` passes the cone inequality to `u = 0` with no topology. What remains is the pigeonhole (finitely many closed tiles over a sequence) and `carrier ⊆ cone` in barycentric form — bookkeeping on existing machinery, no further geometric content |
