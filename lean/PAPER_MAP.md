...
| C `lem:pgram` | the unit parallelogram | `PgramTiling22.pgram22_certificate` | PROVED — one member's certificate is VERIFIED by `decide`; the general parallelogram is a region with no Lean notion of dissection. `PgramTiling22Bridge.lean` (2026-09-02) now has real-coordinate groundwork for this one member — carrier, covering theorem (generic), (C1) congruence, (C2) containment, (C3) pairwise-disjoint interiors, (C4) exact-arithmetic area-sum, and now `dEdge`/`dDet_eq_Ddet` (the determinant half of the volume identity) all proved; instantiating the covering theorem still needs the volume identity itself (a reference square in `Plane` with volume 1, scoped precisely in `VERIFY_PLAN.md`) — so still short of even a one-member VERIFIED |
