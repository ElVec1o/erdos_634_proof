...
| M `prop:cornerfig` | base corner is a single β-tile, apex exactly three α-tiles, with the edge pattern | `VertexFigureReal.corner_angle_sum`, `.base_corner_figure`, `.apex_figure_real` | PROVED — the two figures are now VERIFIED from a real corner; the edge-pattern clause is not formalized |
