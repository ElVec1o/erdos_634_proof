...
| C `lem:pgram` | the unit parallelogram | `PgramTiling22.pgram22_certificate` | PROVED — one member's certificate is VERIFIED by `decide`; the general parallelogram is a region with no Lean notion of dissection. `PgramTiling22Bridge.lean` (2026-09-02) now has real-coordinate groundwork for this one member — carrier, covering theorem (generic), (C1)-(C4), the full volume identity `volume carrier = ENNReal.ofReal |Ddet|`, and now `AreaDet.volume_stdCarrier_half : volume stdCarrier = 1/2` all proved — every geometric fact `covers_of_volume` needs is now in the corpus; only arithmetic assembly (matching `toR area2target` against `2 * Ddet`, scoped in `VERIFY_PLAN.md`) and the actual instantiation remain, so still short of even a one-member VERIFIED |
