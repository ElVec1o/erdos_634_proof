...
| M `prop:gammatrap` | every side of a base-β target carries at least one c-edge | `AngleArithmetic.pi_vertex`, `.gamma_trap`, `GammaCascade.cascade`, `EdgeType.exists_matching_side` | PROVED — **narrowed 2026-09-02**. The combinatorial induction itself is already a theorem (`GammaCascade.cascade`, abstract: an injective `place : ℕ → ℕ` landing each index at itself or its predecessor, missing both ends, is impossible) — this was already true before tonight and had not been connected to anything real. `EdgeType.exists_matching_side` (new) gives every tile edge one of the model's three lengths, closing the classifier half of `cascade`'s first geometric input ('each `a`- or `b`-edge places one `γ`'). What remains, now precisely: (i) the other half of that first input — WHICH endpoint of an `a`/`b`-edge gets `γ`, from the tile's actual local angles there; (ii) `cascade`'s `hinj` (no junction gets two `γ`s) as a real fact about the chain's interior junctions, which needs knowing which tiles meet at each interior junction — genuinely new geometric content, not a citation; `cascade`'s `hends` (corners get none) is now available from `prop:cornerfig`, VERIFIED, and `TileAt.congruentDissection_endpoints`'s chain-to-corner identification (tonight, for `lem:endpoints`) |
