...
| O `prop:nogocensus` | the census contributes one relation | none | PROVED — linear algebra over the census; not transcribed | **[STALE 2026-09-04 — superseded: a later row for `prop:nogocensus` carries the real citation; this `none` is bookkeeping, not a gap.]**
...
| O `prop:norm` | the area ratio cannot exclude primes | none | PROVED — an arithmetic argument; formalizable, simply not done | **[STALE 2026-09-04 — superseded: a later row for `prop:norm` carries the real citation; this `none` is bookkeeping, not a gap.]**
| O `prop:globalsys` | the global angle-Euler system admits prime solutions | none | PROVED — an exhibited solution set at N=11; formalizable as a finite check, not done | **[STALE 2026-09-04 — superseded: a later row for `prop:globalsys` carries the real citation; this `none` is bookkeeping, not a gap.]**
...
| M `lem:cancel` | the tile values sum to the boundary flux `Φ_f(∂ABC)` | none | PROVED — needs the **flux functional Φ** on a dissection boundary, and the grid-direction cancellation argument; no Lean development exists | **[STALE 2026-09-04 — superseded: a later row for `lem:cancel` carries the real citation; this `none` is bookkeeping, not a gap.]**
| M `lem:value` | `C_{f_α}(t) = ±(c+a−b)` for every placement | none | PROVED — same flux development, plus a notion of oriented tile placement | **[STALE 2026-09-04 — superseded: a later row for `lem:value` carries the real citation; this `none` is bookkeeping, not a gap.]**
| M `cor:int` | `M_α`, `M_β` integral and `≡ N (mod 2)` | none | PROVED — depends on `lem:cancel` and `lem:value`; blocked by the same flux development | **[STALE 2026-09-04 — superseded: a later row for `cor:int` carries the real citation; this `none` is bookkeeping, not a gap.]**
...
| M `prop:conic` | the conic form of the equilateral condition | none | VERIFIED — a reformulation of `prop:eqspec`'s conditions; blocked by the same tiling quantifier, and no declaration exists | **[STALE 2026-09-04 — superseded: a later row for `prop:conic` carries the real citation; this `none` is bookkeeping, not a gap.]**
...
| C `lem:filler` | the column and its fillers tile the inter-level strip | none | PROVED — split off `lem:fillerid` on 2026-08-31; the strip, the column and its fillers have no Lean definition | **[STALE 2026-09-04 — superseded: a later row for `lem:filler` carries the real citation; this `none` is bookkeeping, not a gap.]**
...
| C `prop:interfacefloor` | short interfaces carry the same multiset on both sides | none | PROVED — the remaining clause after the split; maximal straight interfaces of a dissection are not objects of the development | **[STALE 2026-09-04 — superseded: a later row for `prop:interfacefloor` carries the real citation; this `none` is bookkeeping, not a gap.]**
...
All 26 rows citing `none` were partitioned. **10 are stale bookkeeping** — a later row for the same
label already carries a real citation — and are now marked `[STALE 2026-09-04]` in place:
