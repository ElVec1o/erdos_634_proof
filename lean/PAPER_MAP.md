...
| M `prop:gammatrap` | every side of a base-β target carries at least one c-edge | `AngleArithmetic.pi_vertex`, `.gamma_trap`, `GammaCascade.cascade`, `EdgeType.exists_matching_side`, `.gamma_at_one_endpoint`, `TileAt.congruentDissection_no_double_gamma` | PROVED — **gap (ii) also closed 2026-09-02**. `congruentDissection_no_double_gamma` (new): two distinct tiles cannot both present `γ` at the same interior boundary point of a `CongruentDissection` — composes `VertexFigureReal.localAngle_mem`/`.gamma_boundary_figure_real` (already existed, connecting real angle multiplicities to a dissection at one named point) with `congruentDissection_hcorners` (new, trivial: every tile's corner angles are the model's three, via `congruent_corner_angles`). This is `cascade`'s `hinj` in substance (its `place` values are always adjacent to their own index, so injectivity reduces exactly to 'the two tiles meeting at one junction don't both present γ', which is what this proves). Both of `cascade`'s hard geometric inputs are now real theorems: gap (i) (`gamma_at_one_endpoint`, closed earlier tonight) and gap (ii) (this). What remains is assembly only: instantiating `cascade`'s abstract `place`/`hplace`/`hinj`/`hends` against a concrete real side's chain (`SideWall`/`BridgeC`), not new geometric content. Label stays PROVED pending that assembly |
