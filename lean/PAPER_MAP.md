...
| Paper | Statement | Evidence | Status |
|---|---|---|---|
| C `cor:wfamily` (erdos-634.tex) | The cevian spectrum at `(1,2)` is complete: `N=7m²` realized for every `m≥2`, no other `m` | `CevianTiling28`/`CevianTiling63` (VERIFIED, the two certified seeds), `code/engine/gen_cevian.py` exhaustions (14 at 678 nodes, 46 at 4,685, 56 at 4,890,763, 62 at 12,872,188 nodes) | **Blocked, recorded precisely**: the W-addition propagation step (composing two W-tilings via a connecting parallelogram, "no scale or composition map on dissections" — one of the four standing recurring blockers) is not built, and the `m=1` exclusion rests on the certified `N=11` exhaustion (the certified-search-format blocker, also standing). Two of the four recurring blockers apply simultaneously. |
| C `thm:wfamily` (erdos-634-companion.tex) | W-addition: `S_W` closed under addition; `W_m` tileable iff `m≠1` | `Primitives.w_slab_counts`/`.w_counts` (VERIFIED, but trivial `ring` identities — no geometric content), same certified exhaustions as `cor:wfamily` | **Same blockers as `cor:wfamily`** (its own W-addition machinery, stated in full here): the composition/parallelogram-bridge layer and the certified-search format. `w_slab_counts` only checks the slab's arithmetic census (`2f²−1 = f·a+b`), not that the parallelogram interfaces are actually full straight segments or that the bricks assemble into a genuine `CongruentDissection`. |

Neither is flippable without new machinery (a composition/gluing layer for dissections) that does
not exist yet in this project — this is a genuinely new blocker class, distinct from the four
already named, though it overlaps with "no scale or composition map on dissections". No label
change; this is debt-tracking hygiene, not a proof.
