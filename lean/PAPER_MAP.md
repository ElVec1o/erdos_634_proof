...
| C `thm:walks` | the boundary walks of the base-β target | `BaseBetaWalks.*`, `WalkEquation.walk_equation` | PROVED — the walk equation is VERIFIED; enumerating the *boundary* walks of a tiling needs the edge chain on all three sides — bridge (c) at that side, which `SideWall.side_chain_junctions` supplies; what remains is the orientation word on it |
...
| C `thm:chain` | the `b`-run orientation lemma | `Inflation.orient_monotone`, `RunOrientation.*` | PROVED — the alphabet lemmas are VERIFIED; the passage from a real run to the word is bridge (c) at that side, which `SideWall.side_chain_junctions` supplies; what remains is the orientation word on it |
