...
| Paper | Statement | Lean declaration | Blocker |
|---|---|---|---|
| O `lem:endpoints` | the last edge's top angle is `α`, the first edge's bottom angle is `β` | `OrientBridge.endpoints_avoid_alpha`, `BridgeC.avoid_alpha_of_multiset` | PROVED — the angle arithmetic is VERIFIED. **Note corrected 2026-09-02** (stale since `SideWall.lean` was added): bridge (c) is no longer base-only — `SideWall.side_chain_junctions` supplies the ordered boundary chain at *any* side. The remaining blocker is the proof's actual dependency, `lem:census`'s corner uniqueness ('the apex fills uniquely as `{3α}`', 'each base corner uniquely as `{β}`'), which is placement-layer, not bridge (c) |
