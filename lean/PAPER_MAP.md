...
| R1-tang | the tangential-approach step: points on the line just right of `V` drag a tile whose corner satisfies `horizontal_flank`'s hypotheses | none | OPEN — the covering/pigeonhole argument (finitely many closed tiles, a sequence approaching `V` tangentially); with it, "some tile lays an edge from `V` along the line" becomes a theorem and the trichotomy closes cases (a)+(b) |
