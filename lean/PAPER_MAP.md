...
| M `thm:ladder` | `kT` is cut into `k²N` copies | none | PROVED — needs the **scale map on dissections**. Half of it now exists: `DissectionMap.mapDissection` transports a dissection along any affine equivalence, so the *transport* is done. What remains is the *construction* — subdividing `kT` into `k²` copies of `T` |
