...
| C `prop:orientmono` | a boundary `a`-run's orientations are monotone | `Inflation.BG_GB_forbidden`, `.orient_monotone`, `.AAB_iff_transition` | PROVED — the alphabet lemmas are VERIFIED. **Note updated 2026-09-02**: bridge (c) is side-agnostic (`SideWall.*`, `OrientWord.word_isChain`). `EdgeType.edge_excludes_own_angle` (new) is a piece of the remaining gap — for a real tile edge whose length matches a model side `j`, neither flanking corner carries `j`'s own opposite angle; combined with `exists_matching_side` this gives the length-based classifier per edge. Still missing: connecting that per-edge classification to the *chain* of a real boundary run (which requires `chain_endpoints`/`word_isChain` instantiated on this specific side with an actual `a`-run), i.e. the same 'readings at that side' gap as `thm:walks`/`thm:chain`. Not closed. |
