...
| M `prop:gammatrap` | every side of a base-β target carries at least one c-edge | `AngleArithmetic.pi_vertex`, `.gamma_trap`, `GammaCascade.cascade`, `EdgeType.exists_matching_side`, `.gamma_at_one_endpoint` | PROVED — **gap (i) closed 2026-09-02**. `EdgeType.gamma_at_one_endpoint` (new): for a non-`c` edge, exactly one of its two endpoints presents `γ` — derived by sharing one congruence witness between the edge-length match and both endpoint-angle matches (so the two endpoints can be told apart individually, not just as a flanking pair), `EuclideanGeometry.angle_comm` for order-symmetry, and the model's scalene-ness (`sideOpp` injective) to identify the edge's own type-index uniquely. Six attempts; the first five failed on small Fin-3/`if`-reduction tactic mismatches (`norm_num` does not reduce `if (0:Fin 3)=0 then...`, `simp`/explicit `if_pos`/`if_neg` does) — none left a `sorry`. `cascade`'s first geometric input ('each `a`- or `b`-edge places one `γ`') is now fully closed: both the classifier (`exists_matching_side`) and the endpoint-specific placement (`gamma_at_one_endpoint`). Two gaps remain: (ii) `cascade`'s `hinj` (no interior junction gets two `γ`s) as a real fact about which two tiles meet at each interior chain junction — genuinely new geometric content; `hends` is already available from `prop:cornerfig` (VERIFIED) and `TileAt.congruentDissection_endpoints`'s chain-to-corner identification |
...
| M `thm:ladder` | `kT` is cut into `k²N` copies | none | PROVED — needs the **scale map on dissections**. Half of it now exists: `DissectionMap.mapDissection` transports a dissection along any affine equivalence, so the *transport* is done. What remains is the *construction*. `Subdivision.P` defines the lattice and `up_sides`, `down_sides` show both shapes of small triangle are congruent to `T` side for side; `cell_index` places every point of `kT` in a cell with `i + j ≤ k − 1`, and `mem_up`/`mem_down` place it in one of that cell's two triangles, so the **covering** is assembled up to bookkeeping. `lattice_coords` supplies the passage to the plane and `up_count`, `down_shift`, `total_count` the count — the upward and downward cells number `k(k+1)/2` and `k(k−1)/2`, summing to `k²`. **Disjointness, corrected 2026-09-02**: the file previously had only `cells_disjoint_i` (pinning the *first* lattice index) and `up_down_disjoint` (same-cell shape separation) — the second index was never pinned, so full pairwise disjointness across the `k²` family was *not* actually assembled, contrary to the previous note's claim. `cells_disjoint_j`/`up_pins_j`/`down_pins_j` (mirroring the `i` versions) and `cell_pinned` (combining all four into one dichotomy: any two open cell triangles meeting at a point share both indices) close that gap. What remains, now precisely: connecting the lattice-coordinate `Prop`s (`openUp`/`openDown`) to actual `Tri.carrier`/`interior` membership for constructed cell triangles, and indexing the `k²` result by `Fin (k²)` to fill `Dissection`'s two fields. This is real remaining work, not bookkeeping — the previous 'only packaging' characterization was optimistic |
...
| C `thm:walks` | the boundary walks of the base-β target | `BaseBetaWalks.*`, `WalkEquation.walk_equation` | PROVED — the walk equation is VERIFIED; enumerating the *boundary* walks of a tiling needs the edge chain on all three sides — bridge (c) at that side, which `SideWall.side_chain_junctions` supplies; and `SideWall.side_junction_frontier_nonvertex` supplies the frontier and non-vertex inputs there. `OrientWord.word_isChain` and `BridgeC.base_word_isChain` are themselves side-agnostic — they take the junction function as a parameter — so what remains is the readings at that side, i.e. which of its edges are `a`-edges |
...
| C `thm:chain` | the `b`-run orientation lemma | `Inflation.orient_monotone`, `RunOrientation.*` | PROVED — the alphabet lemmas are VERIFIED; the passage from a real run to the word is bridge (c) at that side, which `SideWall.side_chain_junctions` supplies; and `SideWall.side_junction_frontier_nonvertex` supplies the frontier and non-vertex inputs there. `OrientWord.word_isChain` and `BridgeC.base_word_isChain` are themselves side-agnostic — they take the junction function as a parameter — so what remains is the readings at that side, i.e. which of its edges are `a`-edges |
...
| C `lem:anglecalc` | the five vertex-figure facts: no right angle, apex, base corner, γ-trap, `α+β` wedge | `AngleArithmetic.no_right_angle`, `.apex_forced`, `.beta_corner_forced`, `.gamma_trap`, `.alpha_beta_corner`, `VertexFigureReal.apex_figure_real`, `.base_corner_figure`, `.boundary_figure_cases`, `TilePlacement.base_corner_counts`, `.apex_counts`, `.corner_tile_edges` | VERIFIED — all five clauses, and clauses (2)–(4) now hold of **real** vertex figures, not only of multiplicities. The geometric consequence of clause (1) was split into `rem:norightangle` |
...
| C `prop:orientmono` | a boundary `a`-run's orientations are monotone | `Inflation.BG_GB_forbidden`, `.orient_monotone`, `.AAB_iff_transition` | PROVED — the alphabet lemmas are VERIFIED. **Note corrected 2026-09-02** (same staleness as `lem:endpoints`, row 455): bridge (c) is no longer base-only — `SideWall.side_chain_junctions` and `.side_junction_frontier_nonvertex` supply it, and `OrientWord.word_isChain` is side-agnostic, at any side. What remains, as in `thm:walks`/`thm:chain`, is the readings at that side — which of its edges are `a`-edges |
