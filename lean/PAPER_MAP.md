...
| M `thm:ladder` | `kT` is cut into `k²N` copies | none | PROVED — needs the **scale map on dissections**. Half of it now exists: `DissectionMap.mapDissection` transports a dissection along any affine equivalence, so the *transport* is done. What remains is the *construction*. `Subdivision.P` defines the lattice and `up_sides`, `down_sides` show both shapes of small triangle are congruent to `T` side for side; `cell_index` places every point of `kT` in a cell with `i + j ≤ k − 1`, and `mem_up`/`mem_down` place it in one of that cell's two triangles, so the **covering** is assembled up to bookkeeping. `openUp`/`openDown`, `up_pins_i`, `down_pins_i`, `cells_disjoint_i` and `up_down_disjoint` give the **disjointness** in lattice coordinates: a point strictly inside a cell triangle pins the cell, and within a cell the diagonal separates the two shapes. `lattice_coords` supplies the passage to the plane and `up_count`, `down_shift`, `total_count` the count — the upward and downward cells number `k(k+1)/2` and `k(k−1)/2`, summing to `k²`. What remains is only the packaging: indexing the `k²` triangles by `Fin (k²)` and filling `Dissection`'s two fields from the covering and disjointness already proved |
