...
| Paper | Statement | Lean declaration | Blocker |
|---|---|---|---|
| C `thm:walkstruct` | the walk structure at `m=1` | `equal_side_no_b`, `equal_side_shape`, `base_b_count` | PROVED — the walk arithmetic is VERIFIED; the clauses about what a *tiling*'s sides carry need the side edge chain — bridge (c) instantiated at that side (`SideWall` supplies its wall data). **Note 2026-09-02**: `SideWalk.side_walk_of_dissection` now supplies exactly the real-side-to-walk-equation connection this row's proof cites as its first step ("a side of length L is exactly partitioned by the tile edges on it"). Closing this row needs composing that with `equal_side_no_b`/`equal_side_shape`/`base_b_count` (a real-to-`ℤ` cast bridge plus the specific side-length instantiation) — not yet done, but the missing input now exists. |
