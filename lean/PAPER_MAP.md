...
| Paper | Statement | Lean declaration | Blocker |
|---|---|---|---|
| C `thm:walkstruct` | the walk structure at `m=1` | `equal_side_no_b`, `equal_side_shape`, `base_b_count` | PROVED — the walk arithmetic is VERIFIED. **Note 2026-09-02 (fourth pass, major)**: `SideWalk.equal_side_no_b_of_gammatrap` (new) connects the already-VERIFIED `GammaTrap.congruentDissection_gammatrap` directly through to clause (i)'s conclusion (`Qc=0`, no `b`-edge on a real equal side) — `hnc1` is no longer a hypothesis, it's derived (gammatrap's `c`-edge witness converted to a chain index via `chain_endpoints`'s newly-exposed surjectivity, then `ChainWalk.count_pos`). Every hypothesis beyond the standard wall/line setup, real corner angles, one calibration fact (`hiso`: `dir` measures real distance along the wall), and number-theoretic `(e,f)` conditions is now derived, not assumed. **The one genuinely remaining gap**: `hA`,`hB`,`hC`,`hLen` — the concrete numeric realization (an actual `CongruentDissection` whose model has sides `ef,f²-e²,f²` and whose target's side `k` has length `f³`). `IsoTri.isoTri`/`SssTri.sssTri` (new, general-purpose `Tri`-from-side-lengths constructors, first in this project) can build the individual triangles, but a full `CongruentDissection` needs `N` tiles actually covering the target — a trivial `N=1` witness is impossible here (the model is scalene, the target isosceles, so they can't coincide) — this is unavoidably the placement layer, structural blocker 1, not a quick add. This is the closest any row got to a real closure this session. |
