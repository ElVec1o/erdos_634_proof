...
| M `prop:cornerfig` | base corner is a single β-tile, apex exactly three α-tiles, with the edge pattern | `VertexFigureReal.corner_angle_sum`, `.base_corner_figure`, `.apex_figure_real`, `TilePlacement.corner_multiplicities`, `.base_corner_single_tile` | PROVED — both figures are VERIFIED from a real corner of a real dissection, including that **no** tile contributes a straight angle there and every tile but one contributes zero. The edge-pattern clause (`a` and `c` at the base corner) needs `LaysOn` developed further |
