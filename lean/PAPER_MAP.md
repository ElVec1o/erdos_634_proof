...
| M `thm:ladder` | `kT` is cut into `k²N` copies | none | PROVED — needs the **scale map on dissections**. Half of it now exists: `DissectionMap.mapDissection` transports a dissection along any affine equivalence, so the *transport* is done. What remains is the *construction*. `Subdivision.P` defines the lattice and `up_sides`, `down_sides` show both shapes of small triangle are congruent to `T` side for side; `cell_index` places every point of `kT` in a cell with `i + j ≤ k − 1`, and `mem_up`/`mem_down` place it in one of that cell's two triangles, so the **covering** is assembled up to bookkeeping. What is still missing is the **disjointness** of the `k²` interiors, and the packaging as a `Dissection` |
