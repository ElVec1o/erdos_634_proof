...
| C `prop:orientmono` | the orientation of a boundary `a`-run is monotone, **on the inflated tile's side** (companion `prop:orientmono`, distinct from the already-VERIFIED `prop:orientmonobdy` on the *target's own* side — the latter's proof explicitly needs no boundary-to-word passage) | `Inflation.BG_GB_forbidden`, `.orient_monotone`, `.AAB_iff_transition` | PROVED — the alphabet lemmas are VERIFIED. **Note updated 2026-09-02 (third pass, corrected)**: `AEdgeReading.a_edge_west_east`/`.tile_orient_BG_east_gamma`/`.tile_orient_GB_west_gamma`/`.aRun_word_monotone` (new) build the real boundary-to-word passage `prop:orientmonobdy`'s proof says is unnecessary there but which `prop:orientmono` (companion's own text) says is exactly the missing piece for the inflated-tile side: "the inflated tile's side to carry the straight-vertex figure, which is the scale map" (structural blocker 4, `DissectionMap`/`Subdivision`) "and... the tile-corner fact... available in `TilePlacement.incident_sides` but not composed with `cornerAngle`". `aRun_word_monotone` supplies that composition and the full `word_isChain → orient_monotone` assembly, generic in the ambient `Dissection` — it is stated against `D.target`'s frontier here (so it does not yet need the scale map itself), conditional on the run's junction incidence (`hjunc`/`hne`/`hfront`/`hvert`, still hypotheses: `MarchRunObject.aRun`'s contiguity is `BaseChain.base_chain_consecutive_meet`'s unclosed business, and `BridgeC.chain_junctions` only covers the *unfiltered* `wallList`, not the filtered run). To actually close `prop:orientmono` as written, the same assembly must be re-run with the *inflated tile's* Dissection standing in for `D`, once the scale map exists — comparable in scope to `GammaTrap.lean`. Not closed. |
