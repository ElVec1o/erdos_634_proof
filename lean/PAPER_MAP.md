...
| C `lem:pgram` | the unit parallelogram | `PgramTiling22.pgram22_certificate` | PROVED — one member's certificate is VERIFIED by `decide`; the general parallelogram is a region with no Lean notion of dissection. `PgramTiling22Bridge.lean` (2026-09-02) now has real-coordinate groundwork for this one member — carrier, covering theorem (generic), (C1)-(C4), and now the full volume identity `volume carrier = ENNReal.ofReal |Ddet|` (first non-triangular covering-measure identity in the corpus) all proved; instantiating `covers_of_volume` needs one more numeric fact, `volume AreaDet.stdCarrier = 1/2` (the reference right triangle is half the unit square), scoped in `VERIFY_PLAN.md` — so still short of even a one-member VERIFIED |
