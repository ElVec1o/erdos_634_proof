...
| M `thm:ladder` | `kT` is cut into `k²N` copies | none | PROVED — needs the **scale map on dissections**. Half of it now exists: `DissectionMap.mapDissection` transports a dissection along any affine equivalence, so the *transport* is done. What remains is the *construction*. `Subdivision.P` defines the lattice and `up_sides`, `down_sides` show both shapes of small triangle are congruent to `T` side for side; `cell_index` places every point of `kT` in a cell with `i + j ≤ k − 1`, and `mem_up`/`mem_down` place it in one of that cell's two triangles, so the **covering** is assembled up to bookkeeping. `lattice_coords` supplies the passage to the plane and `up_count`, `down_shift`, `total_count` the count — the upward and downward cells number `k(k+1)/2` and `k(k−1)/2`, summing to `k²`. Disjointness (`cells_disjoint_i`/`_j`, `up_pins_i`/`_j`, `down_pins_i`/`_j`, `cell_pinned`, `up_down_disjoint`) is fully assembled. **Note 2026-09-02 (second pass)**: both cell shapes are now real `Tri` objects. `Subdivision.cellUp` is a literal translate of `T` (`AffineIndependent.vadd`). `Subdivision.cellDown` is a point-reflection of the reversed triple `(C,B,A)` — `AffineIndependent.reverse_of_three` (existing, fast Mathlib lemma) plus a hand-built reflection `AffineMap` composed via `.map'`. Landing `cellDown` needed real debugging: `affineIndependent_iff_not_collinear_of_ne`'s `.mp` direction is pathologically slow to elaborate on `Plane` for an abstract hypothesis (confirmed by isolated scratch-file testing, 2+ minute hangs on that single line), so it had to be avoided entirely in favor of the injective-affine-map route. What remains: connecting `openUp`/`openDown` membership to `Tri.carrier`/`interior`, and indexing by `Fin(k²)` to fill `Dissection`'s two fields — this is now genuinely just that, no more hidden geometric content. |
