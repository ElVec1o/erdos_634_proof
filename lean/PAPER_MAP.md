...
| Paper | Statement | Lean declaration | Blocker |
|---|---|---|---|
| C `thm:walkstruct` | the walk structure at `m=1` | `equal_side_no_b`, `equal_side_shape`, `base_b_count` | PROVED — the walk arithmetic is VERIFIED; the clauses about what a *tiling*'s sides carry need the side edge chain — bridge (c) instantiated at that side (`SideWall` supplies its wall data). **Note 2026-09-02 (second pass)**: `SideWalk.int_walk_of_dissection` and `.equal_side_no_b_of_dissection` (new) fully close the real-to-`ℤ` cast bridge and compose it with `equal_side_no_b` — clause (i) ("no `b`-edge on an equal side") now follows for a real dissection from exactly two remaining, cleanly isolated inputs: (1) `hA`,`hB`,`hC`,`hLen` — a concrete real-coordinate realization of the base-β target with its actual numeric side lengths, confirmed by search to not exist anywhere in this project (the tile-placement layer, structural blocker 1); (2) `hnc1` — that an edge of type `c` occurs on this side, `prop:gammatrap`'s content, not yet connected to this specific side's chain. Both are real, separately-scoped gaps, not a vague "bridge (c)" anymore. **Note 2026-09-02 (third pass)**: `IsoTri.lean` (new) starts on input (1) — an explicit isosceles coordinate placement (`isoPts L h`, base on the x-axis) with its three pairwise distances proved (`dist01`, `dist02`, `dist12`, given `h² = s² - (L/2)²`). Still missing: `AffineIndependent` for this placement (needed to package it as a `Tri`), which needs `h ≠ 0` and is not yet proved — Mathlib's `Collinear`/`affineIndependent_iff_linearIndependent_vsub` API not yet navigated successfully. |
