...
| C `prop:orientmono` | a boundary `a`-run's orientations are monotone | `Inflation.BG_GB_forbidden`, `.orient_monotone`, `.AAB_iff_transition` | PROVED — the alphabet lemmas are VERIFIED. **Note updated 2026-09-02 (second pass)**: `AEdgeReading.a_edge_west_east`/`.tile_orient_BG_east_gamma`/`.tile_orient_GB_west_gamma` (new) discharge, unconditionally for a real `a`-edge of a `CongruentDissection`, exactly the two hypotheses (`hw`,`he`,`hne`) that `OrientBridge.orient_reading`/`.orient_BG_east_gamma`/`.orient_GB_west_gamma` previously took as given — closing the reading `OrientBridge.lean`'s own docstring names as the last unbuilt geometric input to bridge (c). What remains, not yet built: (1) `MarchRunObject.aRun`'s contiguity along the line (`BaseChain.base_chain_consecutive_meet`'s job, taken as hypothesis there), and (2) assembling `BridgeC.chain_junctions`-style junction incidence *for the filtered `aRun` sublist specifically* (the existing `chain_junctions` runs over the unfiltered `wallList`) so that `OrientWord.word_isChain`'s `hBG`/`hGB` can be discharged by the readings above at every junction of a real run, then `Inflation.orient_monotone` applied to get the `GB^j BG^k` form. That assembly is the actual `prop:orientmono` closure and is comparable in scope to `GammaTrap.lean`. Not closed. |
