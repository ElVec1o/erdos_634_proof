...
theorem base_chain_consecutive_meet {N : ℕ} (hN : 0 < N) (D : Dissection N)
  ...
  -- an edge of the chain lies in the base, so its shadow lies in the base's shadow
  have hedge_sub : ∀ p : Fin N × Fin 3, p ∈ wallList D g c →
    ...
    rintro y ⟨x, hx, rfl⟩
    refine ⟨x, ?_, rfl⟩
    -- `x` is in the tile, hence in the target, and `g x = c` since the edge lies on the wall
    have hxt : x ∈ (D.tile p.1).carrier :=
      (D.tile p.1).edge_subset_carrier p.2 (by rw [Tri.edge]; exact hx)
    have hxc : g x = c := by
      obtain ⟨u, v, hu, hv, huv, rfl⟩ := hx
      have := (AffineMap.lineMap_apply (k := ℝ) ((D.tile p.1).pts p.2) ((D.tile p.1).pts (p.2+1)) v)
  ...
  · -- the covering, transported along the enumeration
