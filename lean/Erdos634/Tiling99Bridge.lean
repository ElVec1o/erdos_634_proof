...
/-!
# `Tiling99`, assembled into a genuine `CongruentDissection`

Erdős #634. The `cor:elevenm` half needing `Tiling99` as a positive witness (`3 ∈ S`). Same
pattern that closed `thm:44`/`thm:63` (`Erdos634.Tiling44Bridge`/`Erdos634.CevianTiling63Bridge`):
`Tiling99`'s `Z15`/`Pt`/`cross`/`dist2`/`znonneg` are literally `Z15Real`'s through `toZPt`, so
every general lemma applies unchanged. `Tiling99` has a named `target` field, like `Tiling44`.

Not a paper-row flip until checked against `cor:elevenm`'s exact statement.

Axiom-clean; no `sorry`.
-/

namespace Erdos634.Tiling99Bridge

open Erdos634.Z15Real Erdos634.Geometry
open Erdos634.Tiling44Bridge (affNeg_apply lineFun_ne_zero_of_sq_ne)

def toZPt (p : Tiling99.Pt) : ZPt := p

/-- **The real determinant of a `Tiling99.Tri`'s three vertices**, transferred from its `ℤ[√15]`
cross product. -/
theorem det3_eq_toR_cross (t : Tiling99.Tri) :
    Erdos634.CertCoord.det3
      (toR (zx (toZPt (Tiling99.t1 t)))) (toR (zy (toZPt (Tiling99.t1 t))))
      (toR (zx (toZPt (Tiling99.t2 t)))) (toR (zy (toZPt (Tiling99.t2 t))))
      (toR (zx (toZPt (Tiling99.t3 t)))) (toR (zy (toZPt (Tiling99.t3 t))))
    = toR (zcross (toZPt (Tiling99.t1 t)) (toZPt (Tiling99.t2 t)) (toZPt (Tiling99.t3 t))) :=
  toR_zcross _ _ _

/-- **The target's determinant is `4224√15`**, positive. -/
theorem target_det_pos :
    (0:ℝ) < Erdos634.CertCoord.det3
      (toR (zx (toZPt (Tiling99.t1 Tiling99.target))))
      (toR (zy (toZPt (Tiling99.t1 Tiling99.target))))
      (toR (zx (toZPt (Tiling99.t2 Tiling99.target))))
      (toR (zy (toZPt (Tiling99.t2 Tiling99.target))))
      (toR (zx (toZPt (Tiling99.t3 Tiling99.target))))
      (toR (zy (toZPt (Tiling99.t3 Tiling99.target)))) := by
  rw [det3_eq_toR_cross]
  exact toR_pos (z := zcross (toZPt (Tiling99.t1 Tiling99.target))
    (toZPt (Tiling99.t2 Tiling99.target)) (toZPt (Tiling99.t3 Tiling99.target))) (by decide)
...
noncomputable def targetTri : Tri :=
  Erdos634.CertCoord.mkTri
    ...
    (toR (zy (toZPt (Tiling99.t2 Tiling99.target))))
    (toR (zx (toZPt (Tiling99.t3 Tiling99.target))))
    (toR (zy (toZPt (Tiling99.t3 Tiling99.target))))
    target_det_pos.ne'
...
theorem piece0_det_pos :
    (0:ℝ) < Erdos634.CertCoord.det3
      (toR (zx (toZPt (Tiling99.t1 (Tiling99.tiles.headI)))))
      (toR (zy (toZPt (Tiling99.t1 (Tiling99.tiles.headI)))))
      (toR (zx (toZPt (Tiling99.t2 (Tiling99.tiles.headI)))))
      (toR (zy (toZPt (Tiling99.t2 (Tiling99.tiles.headI)))))
      (toR (zx (toZPt (Tiling99.t3 (Tiling99.tiles.headI)))))
      (toR (zy (toZPt (Tiling99.t3 (Tiling99.tiles.headI))))) := by
  rw [det3_eq_toR_cross]
  exact toR_pos (z := zcross (toZPt (Tiling99.t1 (Tiling99.tiles.headI)))
    (toZPt (Tiling99.t2 (Tiling99.tiles.headI))) (toZPt (Tiling99.t3 (Tiling99.tiles.headI))))
    (by decide)

-- Every one of the 99 pieces is positively oriented.
set_option maxRecDepth 4000 in
theorem all_pieces_pos :
    ∀ t ∈ Tiling99.tiles,
      zpos (zcross (toZPt (Tiling99.t1 t)) (toZPt (Tiling99.t2 t)) (toZPt (Tiling99.t3 t)))
        = true := by
  decide
...
noncomputable def pieceTri {t : Tiling99.Tri} (ht : t ∈ Tiling99.tiles) : Tri :=
  Erdos634.CertCoord.mkTri
    (toR (zx (toZPt (Tiling99.t1 t)))) (toR (zy (toZPt (Tiling99.t1 t))))
    (toR (zx (toZPt (Tiling99.t2 t)))) (toR (zy (toZPt (Tiling99.t2 t))))
    (toR (zx (toZPt (Tiling99.t3 t)))) (toR (zy (toZPt (Tiling99.t3 t))))
...
/-- `Tiling99.cross` is literally `Z15Real.zcross` read through `toZPt` — the two files define the
same operations on the same underlying type, so this is `rfl`. -/
theorem cross_eq_zcross (o a b : Tiling99.Pt) :
    Tiling99.cross o a b = zcross (toZPt o) (toZPt a) (toZPt b) := rfl

/-- `Tiling99.dist2` is literally `Z15Real.zdist2` read through `toZPt` — again `rfl`. -/
theorem dist2_eq_zdist2 (p q : Tiling99.Pt) :
    Tiling99.dist2 p q = zdist2 (toZPt p) (toZPt q) := rfl

/-- **A piece's vertex, named by index.** -/
def vertexOf (t : Tiling99.Tri) (i : Fin 3) : Tiling99.Pt :=
  ![Tiling99.t1 t, Tiling99.t2 t, Tiling99.t3 t] i

/-- **`pieceTri`'s vertices are exactly `vertexOf`, read as real points.** -/
theorem pieceTri_pts (t : Tiling99.Tri) (ht : t ∈ Tiling99.tiles) (i : Fin 3) :
    (pieceTri ht).pts i
...
`dist2`.** -/
theorem pieceTri_dist_sq (t : Tiling99.Tri) (ht : t ∈ Tiling99.tiles) (i j : Fin 3) :
    dist ((pieceTri ht).pts i) ((pieceTri ht).pts j) ^ 2 = toR (Tiling99.dist2 (vertexOf t i) (vertexOf t j)) := by
...
theorem sep_of_sepBy {P Q : Tiling99.Pt} {A B : Tiling99.Tri}
    ...
    (hsep : Tiling99.sepBy P Q A B = true) (hA : A ∈ Tiling99.tiles) (hB : B ∈ Tiling99.tiles) :
  ...
  have hpts : ∀ (t : Tiling99.Tri) (ht : t ∈ Tiling99.tiles) (k : Fin 3),
      (pieceTri ht).pts k
        = Erdos634.CertCoord.mkPt (toR (zx (toZPt (![Tiling99.t1 t, Tiling99.t2 t, Tiling99.t3 t] k))))
            (toR (zy (toZPt (![Tiling99.t1 t, Tiling99.t2 t, Tiling99.t3 t] k)))) := by
    intro t ht k; fin_cases k <;> rfl
  simp only [Tiling99.sepBy, List.all_eq_true, List.mem_cons, Bool.and_eq_true,
...
def edgeCands (A B : Tiling99.Tri) : List (Tiling99.Pt × Tiling99.Pt) :=
  [Tiling99.edgeOf A 0, Tiling99.edgeOf A 1, Tiling99.edgeOf A 2,
   Tiling99.edgeOf B 0, Tiling99.edgeOf B 1, Tiling99.edgeOf B 2]
...
def sqCond (pq : Tiling99.Pt × Tiling99.Pt) : Bool :=
...
def pairOK (A B : Tiling99.Tri) : Bool :=
  (edgeCands A B).any (fun pq => Tiling99.sepBy pq.1 pq.2 A B && sqCond pq)

-- Every one of the 99*99 ordered pairs of distinct pieces has a working separating edge.
set_option maxRecDepth 8192 in
set_option maxHeartbeats 4000000 in
theorem all_pairs_ok : ∀ A ∈ Tiling99.tiles, ∀ B ∈ Tiling99.tiles, A ≠ B → pairOK A B = true := by
  decide

/-- `sqCond`'s `Bool` and `sep_of_sepBy`'s `Prop` side condition agree. -/
theorem sqCond_iff (pq : Tiling99.Pt × Tiling99.Pt) :
    sqCond pq = true ↔
...
theorem pieces_interiors_disjoint {A B : Tiling99.Tri} (hA : A ∈ Tiling99.tiles)
    (hB : B ∈ Tiling99.tiles) (hne : A ≠ B) :
...
theorem headI_mem_tiles : Tiling99.tiles.headI ∈ Tiling99.tiles := by decide
...
def congOK' (t : Tiling99.Tri) : Bool :=
  decide (∃ σ : Equiv.Perm (Fin 3), ∀ i j : Fin 3,
    Tiling99.dist2 (vertexOf t i) (vertexOf t j)
      = Tiling99.dist2 (vertexOf Tiling99.tiles.headI (σ i)) (vertexOf Tiling99.tiles.headI (σ j)))

-- Every one of the 99 pieces has a matching permutation.
set_option maxRecDepth 4000 in
theorem all_pieces_cong : ∀ t ∈ Tiling99.tiles, congOK' t = true := by decide

/-- **(C1) fully assembled**: every piece of `Tiling44`, as a real `Tri`, is congruent to the
model tile. -/
theorem pieceTri_congruent {t : Tiling99.Tri} (ht : t ∈ Tiling99.tiles) :
  ...
  rw [pieceTri_dist_sq t ht, pieceTri_dist_sq Tiling99.tiles.headI headI_mem_tiles]
...
theorem cross_cyclic (o a b : Tiling99.Pt) : Tiling99.cross o a b = Tiling99.cross a b o := by
  simp only [Tiling99.cross, Tiling99.zsub, Tiling99.zmul, Tiling99.px, Tiling99.py, Prod.ext_iff]
  constructor <;> ring

-- Every one of the 99 pieces' vertices lies in the closed target.
set_option maxRecDepth 4000 in
theorem all_pieces_inside : ∀ t ∈ Tiling99.tiles, Tiling99.insideOK t = true := by decide

/-- Abbreviations for the target's three vertices. -/
theorem insideOK_vertex {t : Tiling99.Tri} (ht : t ∈ Tiling99.tiles) (i : Fin 3) :
    Tiling99.znonneg (Tiling99.cross (Tiling99.t1 Tiling99.target) (Tiling99.t2 Tiling99.target)
        (vertexOf t i)) = true
    ∧ Tiling99.znonneg (Tiling99.cross (Tiling99.t2 Tiling99.target) (Tiling99.t3 Tiling99.target)
        (vertexOf t i)) = true
    ∧ Tiling99.znonneg (Tiling99.cross (Tiling99.t3 Tiling99.target) (Tiling99.t1 Tiling99.target)
        (vertexOf t i)) = true := by
  have hins := all_pieces_inside t ht
  simp only [Tiling99.insideOK, Bool.and_eq_true, List.all_eq_true] at hins
  have hv : vertexOf t i ∈ ([Tiling99.t1 t, Tiling99.t2 t, Tiling99.t3 t] : List Tiling99.Pt) := by
...
theorem vertexOf_mem_targetTri {t : Tiling99.Tri} (ht : t ∈ Tiling99.tiles) (i : Fin 3) :
  ...
  refine Erdos634.CertCoord.mem_carrier_of_dets (x₀ := toR (zx (toZPt (Tiling99.t1 Tiling99.target))))
    (y₀ := toR (zy (toZPt (Tiling99.t1 Tiling99.target))))
    (x₁ := toR (zx (toZPt (Tiling99.t2 Tiling99.target))))
    (y₁ := toR (zy (toZPt (Tiling99.t2 Tiling99.target))))
    (x₂ := toR (zx (toZPt (Tiling99.t3 Tiling99.target))))
    (y₂ := toR (zy (toZPt (Tiling99.t3 Tiling99.target))))
  ...
  · -- h0 : 0 ≤ det3 a b x1 y1 x2 y2 = cross(v, T2, T3) = cross(T2, T3, v) [cyclic]
    rw [show Erdos634.CertCoord.det3 (toR (zx (toZPt (vertexOf t i)))) (toR (zy (toZPt (vertexOf t i))))
        (toR (zx (toZPt (Tiling99.t2 Tiling99.target)))) (toR (zy (toZPt (Tiling99.t2 Tiling99.target))))
        (toR (zx (toZPt (Tiling99.t3 Tiling99.target)))) (toR (zy (toZPt (Tiling99.t3 Tiling99.target))))
      = toR (zcross (toZPt (vertexOf t i)) (toZPt (Tiling99.t2 Tiling99.target))
          (toZPt (Tiling99.t3 Tiling99.target))) from toR_zcross _ _ _,
      ← cross_eq_zcross, cross_cyclic (vertexOf t i) (Tiling99.t2 Tiling99.target) (Tiling99.t3 Tiling99.target)]
    exact Erdos634.Z15Real.toR_nonneg h2
  · -- h1 : 0 ≤ det3 x0 y0 a b x2 y2 = cross(T1, v, T3) = cross(T3, T1, v) [two cyclic rotations]
    rw [show Erdos634.CertCoord.det3 (toR (zx (toZPt (Tiling99.t1 Tiling99.target))))
        (toR (zy (toZPt (Tiling99.t1 Tiling99.target)))) (toR (zx (toZPt (vertexOf t i))))
        (toR (zy (toZPt (vertexOf t i)))) (toR (zx (toZPt (Tiling99.t3 Tiling99.target))))
        (toR (zy (toZPt (Tiling99.t3 Tiling99.target))))
      = toR (zcross (toZPt (Tiling99.t1 Tiling99.target)) (toZPt (vertexOf t i))
          (toZPt (Tiling99.t3 Tiling99.target))) from toR_zcross _ _ _,
      ← cross_eq_zcross,
      cross_cyclic (Tiling99.t1 Tiling99.target) (vertexOf t i) (Tiling99.t3 Tiling99.target),
      cross_cyclic (vertexOf t i) (Tiling99.t3 Tiling99.target) (Tiling99.t1 Tiling99.target)]
    exact Erdos634.Z15Real.toR_nonneg h3
  · -- h2 : 0 ≤ det3 x0 y0 x1 y1 a b = cross(T1, T2, v), direct match
    rw [show Erdos634.CertCoord.det3 (toR (zx (toZPt (Tiling99.t1 Tiling99.target))))
        (toR (zy (toZPt (Tiling99.t1 Tiling99.target)))) (toR (zx (toZPt (Tiling99.t2 Tiling99.target))))
        (toR (zy (toZPt (Tiling99.t2 Tiling99.target)))) (toR (zx (toZPt (vertexOf t i))))
        (toR (zy (toZPt (vertexOf t i))))
      = toR (zcross (toZPt (Tiling99.t1 Tiling99.target)) (toZPt (Tiling99.t2 Tiling99.target))
...
theorem pieceTri_subset_target {t : Tiling99.Tri} (ht : t ∈ Tiling99.tiles) :
...
/-- Every piece of `Tiling44`, indexed by `Fin Tiling99.tiles.length` (rather than a fixed literal
`44`, so no length-cast bookkeeping is needed anywhere in this section). -/
noncomputable def pieceAt (i : Fin Tiling99.tiles.length) : Tri :=
  pieceTri (t := Tiling99.tiles[i.val]) (List.getElem_mem i.isLt)

theorem detTri_pieceAt (i : Fin Tiling99.tiles.length) :
    Erdos634.AreaDet.detTri (pieceAt i) = toR (Tiling99.area2 (Tiling99.tiles[i.val])) := by
  unfold pieceAt pieceTri
  rw [Erdos634.CertCoord.detTri_mkTri, det3_eq_toR_cross]
  rfl

theorem foldl_zadd_eq (l : List Tiling99.Z15) (acc : Tiling99.Z15) :
    l.foldl Tiling99.zadd acc = acc + l.sum := by
  induction l generalizing acc with
  | nil => simp
  | cons a t ih =>
    simp only [List.foldl_cons, List.sum_cons]
    rw [ih]; apply Prod.ext <;> simp [Tiling99.zadd] <;> ring

theorem zsum_eq_sum (l : List Tiling99.Z15) : Tiling99.zsum l = l.sum := by
  simp only [Tiling99.zsum]; rw [foldl_zadd_eq]; simp

theorem tiling99_zadd_eq_zreal (u v : Tiling99.Z15) : Tiling99.zadd u v = zadd u v := rfl

theorem toR_list_sum (l : List Tiling99.Z15) : toR l.sum = (l.map toR).sum := by
  induction l with
  | nil => simp [toR]
  | cons a t ih =>
    simp only [List.sum_cons, List.map_cons]
    show toR (a + t.sum) = toR a + (List.map toR t).sum
    rw [show (a + t.sum : Tiling99.Z15) = Tiling99.zadd a t.sum from rfl,
      tiling99_zadd_eq_zreal, toR_add, ih]

theorem area_sum_transfer :
    ∑ i : Fin Tiling99.tiles.length, toR (Tiling99.area2 (Tiling99.tiles[i.val]))
      = toR (Tiling99.zsum (Tiling99.tiles.map Tiling99.area2)) := by
  rw [zsum_eq_sum, toR_list_sum, List.map_map,
    ← List.ofFn_getElem_eq_map Tiling99.tiles (toR ∘ Tiling99.area2)]
  rfl

/-- **The (C4) area sum, transferred to `ℝ`** — matching `Tiling44`'s own `checkAll` equality. -/
theorem area_sum_eq_target :
    ∑ i : Fin Tiling99.tiles.length, toR (Tiling99.area2 (Tiling99.tiles[i.val]))
      = toR (Tiling99.area2 Tiling99.target) := by
  rw [area_sum_transfer]
  apply congrArg toR
  have h := Tiling99.tiling99_certificate
  simp only [Tiling99.checkAll, Bool.and_eq_true, beq_iff_eq] at h
  exact h.2
...
theorem abs_detTri_sum_eq_target :
    ∑ i : Fin Tiling99.tiles.length, |Erdos634.AreaDet.detTri (pieceAt i)|
      = |Erdos634.AreaDet.detTri targetTri| := by
  have htarget : Erdos634.AreaDet.detTri targetTri = toR (Tiling99.area2 Tiling99.target) := by
    unfold targetTri
    rw [Erdos634.CertCoord.detTri_mkTri, det3_eq_toR_cross]
    rfl
...
-- Tiling99's certificate, assembled into a genuine CongruentDissection.
set_option maxRecDepth 4000 in
theorem tiles_getElem_inj : ∀ i j : Fin Tiling99.tiles.length, i ≠ j →
    Tiling99.tiles[i.val] ≠ Tiling99.tiles[j.val] := by decide

noncomputable def dissection : CongruentDissection Tiling99.tiles.length where
...
theorem targetTri_pts_eq (k : Fin 3) :
    targetTri.pts k = Erdos634.CertCoord.mkPt
      (toR (zx (toZPt (![Tiling99.t1 Tiling99.target, Tiling99.t2 Tiling99.target,
        Tiling99.t3 Tiling99.target] k))))
      (toR (zy (toZPt (![Tiling99.t1 Tiling99.target, Tiling99.t2 Tiling99.target,
        Tiling99.t3 Tiling99.target] k)))) := by
  fin_cases k <;> rfl

theorem targetTri_dist_sq (i j : Fin 3) :
    dist (targetTri.pts i) (targetTri.pts j) ^ 2
      = toR (Tiling99.dist2
          (![Tiling99.t1 Tiling99.target, Tiling99.t2 Tiling99.target, Tiling99.t3 Tiling99.target] i)
          (![Tiling99.t1 Tiling99.target, Tiling99.t2 Tiling99.target, Tiling99.t3 Tiling99.target] j)) := by
  rw [targetTri_pts_eq i, targetTri_pts_eq j, Erdos634.CertCoord.dist_sq_mkPt, dist2_eq_zdist2]
  exact toR_zdist2 _ _

theorem targetTri_sides :
    dist (targetTri.pts 0) (targetTri.pts 1) ^ 2 = 278784
    ∧ dist (targetTri.pts 1) (targetTri.pts 2) ^ 2 = 147456
    ∧ dist (targetTri.pts 2) (targetTri.pts 0) ^ 2 = 147456 := by
  have e01 : Tiling99.dist2 (Tiling99.t1 Tiling99.target) (Tiling99.t2 Tiling99.target)
      = ((278784:ℤ),(0:ℤ)) := by decide
  have e12 : Tiling99.dist2 (Tiling99.t2 Tiling99.target) (Tiling99.t3 Tiling99.target)
      = ((147456:ℤ),(0:ℤ)) := by decide
  have e20 : Tiling99.dist2 (Tiling99.t3 Tiling99.target) (Tiling99.t1 Tiling99.target)
      = ((147456:ℤ),(0:ℤ)) := by decide
  refine ⟨?_, ?_, ?_⟩
...
theorem model_sides :
    (dist ((pieceTri headI_mem_tiles).pts 0) ((pieceTri headI_mem_tiles).pts 1) ^ 2 = 1024
      ∨ dist ((pieceTri headI_mem_tiles).pts 0) ((pieceTri headI_mem_tiles).pts 1) ^ 2 = 2304
      ∨ dist ((pieceTri headI_mem_tiles).pts 0) ((pieceTri headI_mem_tiles).pts 1) ^ 2 = 4096)
    ∧ (dist ((pieceTri headI_mem_tiles).pts 1) ((pieceTri headI_mem_tiles).pts 2) ^ 2 = 1024
      ...
      ∨ dist ((pieceTri headI_mem_tiles).pts 1) ((pieceTri headI_mem_tiles).pts 2) ^ 2 = 4096)
    ∧ (dist ((pieceTri headI_mem_tiles).pts 2) ((pieceTri headI_mem_tiles).pts 0) ^ 2 = 1024
      ∨ dist ((pieceTri headI_mem_tiles).pts 2) ((pieceTri headI_mem_tiles).pts 0) ^ 2 = 2304
      ∨ dist ((pieceTri headI_mem_tiles).pts 2) ((pieceTri headI_mem_tiles).pts 0) ^ 2 = 4096) := by
  have e01 : Tiling99.dist2 (vertexOf Tiling99.tiles.headI 0) (vertexOf Tiling99.tiles.headI 1)
      = ((1024:ℤ),(0:ℤ)) := by decide
  have e12 : Tiling99.dist2 (vertexOf Tiling99.tiles.headI 1) (vertexOf Tiling99.tiles.headI 2)
      = ((2304:ℤ),(0:ℤ)) := by decide
  have e20 : Tiling99.dist2 (vertexOf Tiling99.tiles.headI 2) (vertexOf Tiling99.tiles.headI 0)
      = ((4096:ℤ),(0:ℤ)) := by decide
  refine ⟨Or.inl ?_, Or.inr (Or.inl ?_), Or.inr (Or.inr ?_)⟩
  · rw [pieceTri_dist_sq _ headI_mem_tiles 0 1, e01]; norm_num [toR]
  · rw [pieceTri_dist_sq _ headI_mem_tiles 1 2, e12]; norm_num [toR]
  · rw [pieceTri_dist_sq _ headI_mem_tiles 2 0, e20]; norm_num [toR]
...
theorem targetTri_eq_mkTri :
    targetTri = Erdos634.CertCoord.mkTri
      (toR (zx (toZPt (Tiling99.t1 Tiling99.target)))) (toR (zy (toZPt (Tiling99.t1 Tiling99.target))))
      (toR (zx (toZPt (Tiling99.t2 Tiling99.target)))) (toR (zy (toZPt (Tiling99.t2 Tiling99.target))))
      (toR (zx (toZPt (Tiling99.t3 Tiling99.target)))) (toR (zy (toZPt (Tiling99.t3 Tiling99.target))))
...
theorem exists_dissection_mul_sq (m : ℕ) (hm : 0 < m) :
    ∃ E : CongruentDissection (m * m * Tiling99.tiles.length), E.model = pieceTri headI_mem_tiles :=
  ⟨(Erdos634.Ladder.ladder
      (A := Erdos634.CertCoord.mkPt (toR (zx (toZPt (Tiling99.t1 Tiling99.target))))
        (toR (zy (toZPt (Tiling99.t1 Tiling99.target)))))
      (B := Erdos634.CertCoord.mkPt (toR (zx (toZPt (Tiling99.t2 Tiling99.target))))
        (toR (zy (toZPt (Tiling99.t2 Tiling99.target)))))
      (C := Erdos634.CertCoord.mkPt (toR (zx (toZPt (Tiling99.t3 Tiling99.target))))
        (toR (zy (toZPt (Tiling99.t3 Tiling99.target)))))
...
theorem tiles_length_eq_99 : Tiling99.tiles.length = 99 := by decide

/-- **Clause 2, restated in the paper's own numeral form**: `99 * m ^ 2` is a tile count for every
`m ≥ 1`. -/
theorem exists_dissection_99_mul_sq (m : ℕ) (hm : 0 < m) :
    ∃ E : CongruentDissection (99 * m ^ 2), E.model = pieceTri headI_mem_tiles := by
  obtain ⟨E, hE⟩ := exists_dissection_mul_sq m hm
  have hnum : m * m * Tiling99.tiles.length = 99 * m ^ 2 := by
    rw [tiles_length_eq_99]; ring
  exact hnum ▸ ⟨E, hE⟩


end Erdos634.Tiling99Bridge
